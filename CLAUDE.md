...
**No resuelto, a propósito, documentado para más adelante:** self-signup abierto en Login (mitigado por RLS real, `is_app_admin()` bloquea a cualquier no-admin — es fricción de UX, no fuga de datos; sacarlo es una decisión de producto, no un bug); límite de tamaño de archivo en Bóveda ya agregado (20MB) pero sin compresión/redimensionado client-side; timeout/progreso granular por agente en Mesa de Diálogo (el debate puede tardar minutos, hoy solo hay un spinner genérico) — quedó fuera por ser una mejora de UX más grande, no un bug de confiabilidad como el resto de esta lista.
