...
| **CI (`ci.yml`)** | **lint + tsc bloqueantes** — 0 errores (6 warnings preexistentes), ESLint acotado a `src/**` (Deno fuera de scope). test + build corriendo. `permissions: contents:read` + `concurrency` (cancela el CI de un commit ya superado). `vitest.config` con `testTimeout` 15s + `retry: 1` (mata el falso rojo por sobrecarga). Los 20 workflows con `permissions` + `timeout-minutes` (2026-09-03) | 2026-09-03 (pase "mejorar") |
