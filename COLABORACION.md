...
- **`npm audit` — FALSA ALARMA, ya está todo parcheado.** El primer `npm audit` reportó "5 vulns high" (`lodash`/`ws`/`browserslist`/`@remix-run/router`) pero era **caché de advisories vieja del registro**. Verificado por inspección directa del `package-lock.json` commiteado + `node_modules`: `lodash@4.18.1` (advisory `<=4.17.23`), `ws@8.21.3` (`<=8.20.1`), `browserslist@4.28.8` (`<=4.28.6`), `@remix-run/router@1.23.4` (`<=1.23.2`) — **los 4 por encima del techo del advisory, o sea parcheados**. `npm audit fix` hizo 0 cambios (no hay nada que arreglar). El árbol de deps está limpio.
