...
- **CI + workflows.** `permissions` explícito en los 20 workflows (los 6 cron-curl con `permissions: {}`, `deploy-functions` con `contents: read` + `cancel-in-progress: false` para no matar un deploy a mitad); `concurrency` en `ci.yml` (cancela el CI de un commit ya superado), `deploy-migrations` / `sync-history` / `autopilot` (serializadas, nunca cortar a mitad). Commits `17afbf3` + `dce9ac7`.
