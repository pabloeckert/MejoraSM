...
**Fix aplicado, en los tres lugares:** `GROQ_MODEL`/`model` actualizado de `llama-3.3-70b-versatile` a `openai/gpt-oss-120b` en `scripts/lib/claude.mjs`, `supabase/functions/orchestrator/index.ts` y `supabase/functions/copilot/index.ts`. Las dos Edge Functions se redeployaron (`deploy-functions.yml`, disparado por el push). Catálogo de proveedores de IA (tabla más arriba en este archivo) actualizado para no quedar desactualizado otra vez sin que nadie lo note.

**Lo que queda pendiente de verdad, no resuelto por código:**
- Para las stories **con foto** (la mayoría, dado que Hub ya tiene fotos reales pendientes cuando existan), sigue sin haber fallback — no hay un modelo de visión de Groq verificado como confiable. Van a seguir sin generarse hasta que se resuelva el límite de Anthropic de un lado o del otro, aunque ahora sí exista el secret y el modelo de texto esté al día.
- **Confirmación real pendiente, no fabricada:** el fix del modelo se aplicó y deployó, pero no se disparó `daily-story.yml` a mano para no forzar una publicación real (mismo criterio de siempre) — la corrida real de mañana (2026-08-19, 13:00 UTC) es la primera prueba end-to-end real del modelo nuevo en ese flujo. Para `orchestrator`/`copilot`, el fallback a Groq solo se ejercita cuando Anthropic falla — no hay forma de confirmarlo en producción sin esperar a que ocurra o sin armar una llamada de prueba puntual (mismo patrón ya usado en otras verificaciones de este documento, no se hizo en esta vuelta).
- La solución de fondo del límite en sí sigue siendo del lado de la cuenta de Anthropic, no del código: subir el plan, agregar créditos, o esperar al 2026-09-01. Es una decisión de cuenta/facturación que le corresponde a Pablo.
- `classify-photo` (sugerencia de dimensión) tampoco va a devolver sugerencias reales mientras dure el límite de Anthropic — el flujo de Subir material está diseñado para degradar bien (si la sugerencia falla, el humano elige la dimensión a mano y confirma igual, no bloquea el upload), así que **subir fotos sigue funcionando**, solo sin la ayuda de la IA por ahora.
