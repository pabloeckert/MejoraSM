...
| **Plan de publicación 2026** | 🟢 7 fases completas. **2026-09-03:** dominio propio activado, IG/FB limpio + `sync-history` reactivado (cron cada 6h de nuevo), LinkedIn y FB-Ads conectados en Zernio. **Falta solo:** Pablo carga el secret `ZERNIO_LINKEDIN_ACCOUNT_ID` = `6a99f21e77555aae01d50cf5` (identificador, no credencial) para que Fase 5 se active. Ver "Cierre de pendientes del plan de publicación" en la bitácora | cerrado 2026-09-01, pendientes cerrados 2026-09-03 |
...
- **LinkedIn conectado en Zernio.** Probe temporal (`/v1/accounts`, borrado apenas confirmó) — las 3 cuentas de "Mejora Continua" `enabled+active`: `facebook 6a5640333ecd8aa344fadb4b`, `instagram 6a56405a3ecd8aa344faecae`, **`linkedin 6a99f21e77555aae01d50cf5`**. **Pendiente único de Pablo:** cargar `ZERNIO_LINKEDIN_ACCOUNT_ID = 6a99f21e77555aae01d50cf5` como secret de GitHub Actions (`gh secret set ZERNIO_LINKEDIN_ACCOUNT_ID --body "6a99f21e77555aae01d50cf5"` o por la UI de Settings → Secrets). Es un identificador, no una credencial (igual que los IDs de IG/FB, que están en texto plano en `inbox/index.ts`) — el clasificador de seguridad del entorno bloqueó que lo cargara la sesión. Apenas exista, `publishPost()` suma `linkedin` a las plataformas sin tocar nada más, y `sync-history` (que ya pasa ese env) trae el historial de LinkedIn.
