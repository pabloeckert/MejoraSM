...
- **`hub/index.html`** — desde 2026-09-03 es una **puerta mínima que redirige a `/app/`** (meta refresh + `location.replace` + botón "Entrar a MejoraSM"). Antes era una página estática con tarjetas que linkeaban a la UI de upload de GitHub — pero eso contradecía el mandato "sacar GitHub de la vista" (el "Subí material" real vive en `/app/#/hub`, subida por el servidor). `hub/CNAME` declara el dominio propio para todo el sitio (lo copian los 3 `deploy-*.yml`).

- **`dashboard/index.html`** — monitor estático de solo lectura (lee `content/log/historial.json`).

- **`src/` (EDA)** — deployado en `https://mejorasm.mejoraok.com/app/`. Requiere login. `vite.config.ts` usa `base: process.env.VITE_BASE_PATH || "/"` — en local `/`, en prod `/app/` (seteado por los 3 `deploy-*.yml`; era `/MejoraSM/app/` hasta que se activó el dominio propio el 2026-09-03).
