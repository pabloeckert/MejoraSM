...
- **Confirmación real pendiente para el flujo exacto de `daily-story`, no fabricada:** el mecanismo de fallback ya se confirmó en vivo contra `copilot` (mismo modelo, mismo endpoint de Groq), pero no se disparó `daily-story.yml` a mano para no forzar una publicación real (mismo criterio de siempre) — la corrida real de mañana (2026-08-19, 13:00 UTC) es la primera prueba end-to-end real del modelo nuevo en ese flujo específico.
