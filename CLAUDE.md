...
## Auditoría real y 4 fixes — 2026-08-31

Pablo pidió una auditoría completa (bugs, mejoras UX/UI, optimización, prueba E2E). Se investigó primero contra evidencia externa real (APIs de Meta/Groq/Anthropic/Supabase, tendencias UX 2026) — sin hallazgos que requirieran cambio de código (Groq `openai/gpt-oss-120b` sigue vigente, nada deprecado) — y se confirmaron los 6 cron jobs de automatización (Daily Story, Publish Scheduled Posts, Sync History, Metrics Collector, Rule Engine, Copilot Advice) corriendo solos y en verde contra el historial real de GitHub Actions. De la auditoría salieron 4 fixes reales, más otros dos reportados por Pablo probando la app en vivo el mismo día.

**1. CI nunca corría tests/build reales, más un crash local sin `.env`** (`531c4fc`) — `ci.yml`: el step "Run tests" no tenía `VITE_SUPABASE_URL`/`VITE_SUPABASE_PUBLISHABLE_KEY` como env (solo "Build" los tenía), y el step "Lint" (42 errores preexistentes, ya documentados como deuda aceptada) abortaba el job antes de llegar a tests/build — resultado real: hacía tiempo que nadie veía si tests o build pasaban en CI de verdad, solo se sabía por corridas locales. Lint ahora usa `continue-on-error` (sigue visible en su propio step, ya no bloquea el resto) y "Run tests" recibe las mismas env vars que "Build". De paso: `services/supabase.ts` — `createClient(supabaseUrl ?? "", ...)` tiraba `"supabaseUrl is required"` en frío si faltaba `.env`, rompiendo `Dashboard.test.tsx` (y cualquier checkout fresco sin `.env` corriendo `npm test`) antes de llegar a ningún test real. Fallback a un placeholder con formato de URL válido; el aviso real en consola (variables no configuradas) sigue intacto.

**2. Post huérfano en Monitor: el "pendiente" del 27/08 quedó resuelto de verdad** (`5d9b808`) — la sección "Borrar manualmente del Monitor" de arriba había documentado que `historialApi.removePost()` solo saca la fila de la corrida puntual, y que `sync-history` la reescribe entera desde Zernio (cada 15 min vía `publish-scheduled-posts`, o cada 6h por su propio cron), así que un borrado manual podía volver a aparecer. Pasó exactamente eso con `6a8ef3ac...` — Pablo pidió borrarlo de nuevo. Fix real: `EXCLUDED_POST_IDS` en `sync-history.mjs` filtra el post en el origen, antes de escribir `historial.json`/`historial_cache` — ya no vuelve a aparecer sin importar cuántas veces corra el sync. No borra nada real de Zernio/Instagram (Instagram no soporta despublicar por API, ya documentado); si hace falta que desaparezca de Instagram también, sigue siendo a mano en la app.

**3. Monitor: Stories publicadas sin fallas no tenían ningún botón de acción** (`696a587`) — Pablo probó "Ya lo hice a mano" en `/monitor` sobre 3 Stories reales, todas publicadas OK, y notó que "faltaba algo": tenía razón. `GestionPostDeFeed` tenía `if (post.kind !== "post" || !post.proposalId) return null` — excluía Stories a propósito porque originalmente solo existía para posts de feed, así que una Story sin fallas no mostraba ni Reintentar, ni Despublicar, ni "marcar a mano", ni siquiera el link a GitHub Actions. `manageTarget()` ya distinguía bien story/post (workflow/input key correctos para cada caso), pero `handleAction()` nunca lo usaba — hardcodeaba `manage-post.yml` + `proposal_id` sin importar el tipo. Fix: renombrado a `GestionPublicacion`, sin guard de `kind`, usa `manageTarget()` de verdad para elegir workflow/input key dinámicamente — Stories y posts publicados sin fallas tienen ahora el mismo panel completo, cada uno contra el workflow correcto.

**4. Mesa de Diálogo: mandar feedback tras un rechazo no actualizaba la pantalla** (`d56bc8f`) — Pablo probó una ronda real: el Crítico rechazó, mandó feedback, y la pantalla quedó "todo lo mismo, como si no hubiera tomado nada de lo que hablamos". Causa real, doble: (a) `useContinueDialogue` solo invalidaba `["dialogue-sessions"]` (la lista), nunca `["dialogue-messages", sessionId]` (la query real del chat) — y como `useDialogueMessages` corta su polling de 5s apenas `session.status` deja de ser `"active"` (fix de perf del 2026-08-25), sin esa invalidación explícita los turnos nuevos de la segunda vuelta nunca llegaban a la pantalla, quedaba congelada hasta un F5 manual; (b) ni `handleContinue` ni `handleStart` tenían `onError` — un fallo real (el timeout de 150s, o un error de red/Edge Function) no mostraba nada, indistinguible en pantalla de un envío exitoso. Se agregó la invalidación que faltaba y un toast de error en los dos casos.

Verificado en los cuatro: `tsc --noEmit` limpio, lint sin regresión (42 errores preexistentes), 64/64 tests, build limpio.
