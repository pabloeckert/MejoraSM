...
| **Dominio propio** | `mejorasm.mejoraok.com` — CORS de las 11 Edge Functions + canonical ya apuntan ahí. **Falta:** Pablo agrega el registro DNS (`CNAME mejorasm → pabloeckert.github.io`), después se agrega el archivo `CNAME` + se cambia `VITE_BASE_PATH` a `/app/` y se redespliega | preparado 2026-09-01 |
...
1. Agregar `https://pabloeckert.github.io/MejoraSM/app/reset.html` a **Redirect URLs** del proyecto Supabase (Authentication → URL Configuration). Sin eso, el link del email de blanqueo rebota.
