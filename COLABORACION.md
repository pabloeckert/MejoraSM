...
**Estado `[03]` a esta altura:** auditado y arreglado — infra/migraciones, pipeline de contenido (los 5 workflows), CORS/CSP, código muerto, docs stale, tests. Baseline verde (tsc/lint 0/66 tests/build, `node --check` los 17 scripts). Sigo con: revisión fina de `scripts/render-*`/`publish-*`, y otra pasada a la doc.
