...
**Lo que queda para una decisión de Pablo, no ejecutado:** el pedido de "editor propio dentro del MejoraSM" — investigado el punto real de fricción: el pipeline autónomo de posts/carruseles (Mesa de Diálogo → auto-agenda → cron) no tiene ningún momento donde un humano vea la foto real antes de que se publique (la foto se elige recién al renderizar, en el cron de 15 min) — agregar edición ahí exigiría sumar un gate de revisión, que es una decisión de diseño distinta a la autonomía total ya vigente desde 2026-08-02, no un ajuste técnico. El único lugar que ya tiene un momento supervisado real es "Publicar ahora" (`PublishNowCard`, `/hub`) — pero es el flujo de **Stories**, no de posts de feed, y hoy solo usa `story-template.html` (sin variante de collage todavía). Planteado a Pablo como pregunta concreta antes de construir a ciegas: si el editor va ahí (Stories, ya con checkpoint humano) o si vale la pena sumar un checkpoint nuevo al pipeline de posts.
