...
- Para las stories **con foto** (la mayoría, dado que Hub ya tiene fotos reales pendientes cuando existan), sigue sin haber fallback — no hay un modelo de visión de Groq verificado como confiable. Van a seguir sin generarse hasta que se resuelva el límite de Anthropic de un lado o del otro, aunque ahora sí exista el secret.
- La solución de fondo sigue siendo del lado de la cuenta de Anthropic, no del código: subir el plan, agregar créditos, o esperar al 2026-09-01. Es una decisión de cuenta/facturación que le corresponde a Pablo — no algo que "Claude Code disponga" solo.
