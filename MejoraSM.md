...
Con todo eso arreglado y documentado, llegó un archivo comprimido con la propuesta de la herramienta de diseño: una auditoría de producto en formato de diario impreso, un mockup del panel principal, y un sistema de diseño con estética de periódico que no aplica a este proyecto, porque la marca ya tiene el suyo. La herramienta de diseño había leído el código a mitad de las nueve tandas de arreglos, así que buena parte de lo que marcaba ya estaba resuelto: los controles muertos de la pantalla de configuración, el texto viejo del pie del menú, el menú de once ítems planos, el arrastre del calendario en pantalla táctil, la subida de varios archivos en el manual de marca. Lo que sí aportaba, y la auditoría de código no tenía, era otro lente: contrastar el estado actual contra el brief de rediseño que se había cerrado el dieciséis de agosto. De los seis puntos de ese brief, tres estaban hechos — la reconstrucción de propuestas, la del calendario, el ruteo automático de modelo de inteligencia artificial — y tres no: fusionar la mesa de diálogo con el laboratorio en una sola herramienta, renombrar la bóveda a Manual de Identidad de Marca con carga de un comprimido entero y organización por categoría, y el motor de análisis con inteligencia artificial para el panel principal, que seguía siendo seis tarjetas de texto fijo congeladas desde principios de agosto. Esas tres no eran ideas nuevas ni funcionalidades a futuro: eran decisiones ya tomadas que el código nunca ejecutó, y la auditoría de código las había clasificado mal como parte del roadmap.

La herramienta de diseño sumó además dos ideas que no estaban en la auditoría de código: conectar los templates de render que ya existen — los que se usan para publicar — a la vista previa de la ventana de detalle, lo que cerraría de una sola vez el "falta ver cómo queda la pieza" en propuestas, calendario y mesa de diálogo; y un rol de solo lectura para Sindy sobre propuestas y el panel principal, ver y comentar sin tocar la configuración ni el motor de publicación, dado que Sindy ya participa de decisiones reales de contenido sin ningún acceso al sistema.

Se armó entonces un plan de continuación en lenguaje sencillo, con ejemplos de antes y después para cada parte, publicado como página visual. Cinco fases: el motor de análisis con inteligencia artificial primero, después la fusión de la mesa de diálogo con el laboratorio, después el renombre del manual de marca con carga de comprimidos, después un puñado de ajustes finos que quedaron de las dos auditorías, y por último el rol de revisor para Sindy. Antes de todo eso, una decisión que bloquea la última fase: hoy cualquiera con el enlace entra al sistema y puede publicar o borrar, y para que Sindy tenga un rol de verdad hace falta una puerta de acceso — la recomendación es una capa de control de acceso simple adelante de la aplicación, alrededor de una hora de configuración, sin tocar la base de datos.

Sobre lo que queda fuera de ese plan, llegó una pregunta directa: qué limita hoy sumar otra red social, video corto, la bandeja de comentarios y mensajes directos, y el reciclado de contenido. El hilo común es que todo lo social pasa por una sola plataforma externa de publicación, y hoy solo se le pide publicar imágenes y traer números agregados, nunca el texto de un comentario. Sumar otra red social depende de si esa plataforma la soporta y de si hay una cuenta conectada ahí — una pregunta de quince minutos, y si la respuesta es sí, son unos días de trabajo. El video corto está bloqueado porque no existe ningún motor de armado de video: el sistema renderiza imágenes estáticas, y un video corto necesita convertir fotos, texto y música en un archivo de video, lo cual es infraestructura nueva. La bandeja de comentarios y mensajes directos está bloqueada porque la plataforma de publicación casi seguro no expone el texto de los comentarios ni los mensajes, lo que obliga a ir a la interfaz de la red social directa, y en el caso de los mensajes directos eso requiere una revisión formal de aplicación que lleva semanas; además haría falta un modelo de datos nuevo entero para guardar y analizar conversaciones. El reciclado de contenido no tiene ningún bloqueo técnico — el sistema ya sabe qué funcionó, tiene las piezas ya renderizadas guardadas, y sabe reprogramar solo — el único freno real es que todavía hay muy pocas piezas reales publicadas para que "reciclá lo que funcionó" tenga de dónde elegir; en un mes o dos de pipeline corriendo, deja de ser un problema.

La decisión final: cerrar acá, dejar todo lo que queda fuera del plan para cuando esté terminado lo del plan, documentar cada parte en el archivo de fuente de verdad del proyecto para no perder nada, y mantener la copia local sincronizada con la de la nube.

Arrancó entonces la ejecución del plan de corrido, en modo autónomo, con la instrucción de solo interrumpir a Pablo si hacía falta una intervención humana real.

La Fase A fue el motor de insights con inteligencia artificial. Hasta ahí, el Dashboard mostraba seis conclusiones fijas escritas a mano en el código, sacadas del análisis real de redes de principios de agosto. La Fase A las convirtió en algo vivo: se creó una tabla que cachea los insights por semana y otra que guarda si cada insight le sirvió o no a Pablo, y una función nueva que una vez por semana toma esas seis conclusiones semilla y las contrasta contra los datos reales acumulados — métricas de las últimas seis semanas, rendimiento por formato, por horario, las cinco mejores piezas — más el contexto de marca. La función le pide al modelo que devuelva cada insight con un estado: se mantiene igual, se refinó, se actualizó, o es nuevo, siempre con la evidencia numérica real detrás y un nivel de confianza. Si no hay datos suficientes, mantiene las semillas y lo dice — no inventa. Se probó en producción: con solo cuatro métricas reales, el modelo mantuvo las seis semillas intactas con confianza media, exactamente el comportamiento correcto. En el frontend, una sección nueva reemplazó las conclusiones fijas, con botones de "me sirve" / "no aplica" que alimentan el recálculo de la semana siguiente, y un botón para generar un informe imprimible eligiendo qué secciones incluir, que sale a PDF por la impresión del navegador sin ninguna librería nueva. De paso se encontró que el workflow que despliega todas las funciones tenía una lista incompleta — le faltaban dos — y se completó.

La Fase B fusionó la Mesa de Diálogo con el Laboratorio y agregó dos cosas que pedía el brief de rediseño. El Laboratorio se discontinuó: era la versión "dame el resultado sin ver el debate", y no justificaba ser una pantalla aparte. Su página se borró, su ruta ahora redirige a la Mesa de Diálogo, y los enlaces que apuntaban ahí se redirigieron. La primera cosa nueva es el modo libre: hasta ahora había que escribir el tema a mano; ahora el diálogo de nueva sesión ofrece también "proponeme un tema", y el sistema elige uno solo — mira lo que ya funcionó, los buyer personas y el criterio de marca, y los últimos quince temas debatidos para no repetir, y devuelve un tema concreto. Se probó en producción: eligió "por qué la nueva generación tiene ideas que nunca llegan a ninguna decisión, y cómo pedir un rol con nombre y peso real", un tema coherente y no repetido; el debate corrió los tres agentes y el Crítico no aprobó en primera ronda, así que no se creó ninguna propuesta ni hubo riesgo de publicación. La segunda cosa nueva es el preview visual real de la pieza: antes, al aprobar algo en la Mesa de Diálogo o al abrir el detalle de una propuesta, solo se veía texto. Ahora se ve cómo va a quedar la imagen — un componente nuevo trae la plantilla de render real, le mete el mismo texto que le metería el pipeline al publicar, y lo muestra a escala dentro de un marco. Muestra la variante sin foto, porque la foto real se elige recién al publicar. Todo verificado: tipos limpios, cero errores de lint, sesenta tests en verde, build limpio, y los tres workflows de despliegue en verde.

La Fase C convirtió la Bóveda en el "Manual de Identidad de Marca". El cambio de nombre es solo texto visible — la dirección web, el archivo y la tabla no se tocaron, porque era un problema de vocabulario, no de estructura, el mismo criterio que se usó antes al cambiar "oferta" por "dimensión del servicio". Lo de fondo son dos cosas nuevas. La primera: ahora se puede subir un archivo comprimido con todo el kit de marca de una vez — se descomprime en el navegador y cada documento adentro se sube por separado al mismo procesador de siempre; lo que no sea un documento de texto se ignora sin drama. La segunda: cuando se procesa un documento, después de sacarle el texto, el sistema le pregunta al modelo de qué tipo es — manual y criterio, buyer persona, guía de tono, ejemplo de pieza, u otro — con un llamado corto, y si el llamado falla lo marca como "otro" sin frenar nada. Se agregó una columna nueva a la base para guardar ese tipo, y la pantalla ahora muestra los documentos agrupados por categoría, con un menú por documento para corregir la clasificación si el sistema se equivocó. Se probó contra producción con dos documentos reales: uno de buyer persona lo clasificó como buyer persona, una guía de tono como tono, los dos bien. Los diecinueve documentos que ya estaban cargados quedan sin clasificar por ahora — reclasificarlos gasta llamadas de embeddings, así que se hace cuando haga falta.

La Fase D fue un puñado de ajustes finos que habían quedado de las dos auditorías. En el Monitor, los avisos nativos de "¿estás seguro?" del navegador para despublicar o sacar una pieza pasaron a usar el mismo cuadro de diálogo que el resto de la aplicación, mediante un gancho reutilizable nuevo; y los botones de acción de las tarjetas se agrandaron a cuarenta y cuatro píxeles de alto para que se puedan tocar bien desde el celular. En Configuración se agregó una tarjeta que muestra las últimas doce decisiones reales del sistema: por cada sesión de la Mesa de Diálogo, la fecha, el tema (marcando si lo eligió el sistema en modo libre), el veredicto del Crítico y si la pieza se agendó sola. La pantalla dejó de ser solo para ajustar los textos de los agentes y pasó a ser también de supervisión. En Auditoría se agregó un filtro de rango de fechas para los exports. En Propuestas se sacó la pestaña "Video", que nunca tuvo nada porque el sistema no genera video y la base ni siquiera permite ese valor. Y en el tablero, los números clave muestran un guión mientras cargan en vez de un cero, que se confundía con un cero real.

La Fase E era el rol de revisor para Sindy, y acá el trabajo se dividió en dos. Lo que se pudo hacer sin depender de nada, se hizo: comentarios anclados a cada pieza, para que Pablo y Sindy discutan una propuesta dentro del sistema en lugar de por afuera. Se creó la tabla, la interfaz aparece al final del detalle de cualquier propuesta, y como todavía no hay usuarios reales, cada uno firma con su nombre, que el navegador recuerda. Se probó contra producción y anda. Pero el rol de revisor de verdad — que Sindy entre y vea todo sin poder tocar nada — no se puede construir sin una puerta de acceso que sepa quién es cada persona, y hoy el sistema está abierto para cualquiera con el enlace por decisión de Pablo. Poner esa puerta es una decisión suya más una hora de configuración de infraestructura por fuera de este repositorio, exactamente el tipo de cosa que esta sesión no resuelve sola. Así que la Fase E queda a mitad de camino, con la parte de comentarios lista y funcionando, y la restricción de acceso esperando que Pablo decida con qué herramienta cerrar la puerta.

Con el plan cerrado, Pablo definió la puerta de acceso que la Fase E había dejado pendiente. Pidió textualmente un solo usuario y contraseña, con el mail de registro para el blanqueo si se pierde u olvida la contraseña, y aclaró que el rol de solo lectura para Sindy ya no hace falta. O sea: una sola credencial compartida, sin roles, con recupero por email. Se descartó la capa de infraestructura tipo Cloudflare que se había recomendado antes, porque Pablo quiere el login adentro de la aplicación.

Se revirtió entonces la apertura del 25 de agosto. La aplicación vuelve a estar envuelta por la puerta de acceso; la pantalla de ingreso es email más contraseña, sin alta de cuenta y sin código por email, con un modo "olvidé la contraseña" que dispara el mail de blanqueo. El link del mail lleva a una página HTML suelta, fuera del sistema de rutas de la aplicación, que toma el token y guarda la contraseña nueva con una llamada directa a la API de autenticación — se hizo así porque el token viene en el fragmento de la URL y eso choca con el ruteo por hash que usa la aplicación en GitHub Pages. Las funciones del backend vuelven a exigir un token real de la cuenta autorizada o la clave de servicio para los procesos automáticos; se sacó la excepción que aceptaba la clave pública pelada. Y una migración nueva vuelve a cerrar el acceso a nivel de base de datos en las quince tablas y en el bucket de documentos, revirtiendo la que lo había abierto.

Se verificó contra producción: con la clave pública, la base ya no devuelve nada y las funciones responden "no autorizado"; el proceso automático diario, que usa la clave de servicio, sigue corriendo y escribiendo su registro sin problema; y la pantalla de login y la de blanqueo cargan bien en el sitio real. Lo único que no se pudo probar es el ingreso real y el mail de blanqueo, porque hacen falta la contraseña de Pablo y su casilla. Quedan dos cosas para que él haga: sumar la dirección de la página de blanqueo a la lista de URLs permitidas en el panel de Supabase, y tener o establecer la contraseña de la cuenta. La función de comentarios en cada pieza, que se había armado como parte de la Fase E, se mantiene, porque sirve igual con una cuenta compartida para dejar notas ancladas a una publicación.

Cerrado el plan, se le planteó a Pablo el problema real de fondo: el sistema casi no tiene datos, porque publica poco, y publica poco porque elegir el tema de cada post sigue siendo manual. Se le ofreció automatizar eso. Eligió el modo libre automático tres veces por semana, lunes miércoles y viernes, pero con un aviso por email antes de cada publicación, en modo veto: la pieza se publica salvo que él la cancele a tiempo, no espera su aprobación.

Se armó un script y un flujo de trabajo nuevos para eso. Cada corrida llama al orquestador en modo libre con la clave de servicio: el sistema elige un tema que no haya tocado hace poco, los tres agentes debaten, y si el Crítico aprueba, la pieza queda agendada. El script entonces empuja la hora de publicación a las ocho de la noche hora argentina del mismo día — una ventana de veto de unas diecisiete horas — y manda un email con el tema, el texto completo y un link para revisar o cancelar. Si el email no se puede mandar por lo que sea, la pieza se baja a pendiente y no se publica: nunca sale algo sin haber avisado. Si el Crítico no aprueba, no pasa nada y se reintenta en la corrida siguiente.

Falta que Pablo se registre en Resend, el servicio de envío de mails, que es gratis, cree una clave, y la pase para cargarla como secreto. Recién con eso se puede probar el circuito completo y activar el horario automático, que por ahora quedó comentado.

Pablo pidió además una forma de publicar al instante: sacar una foto, subirla, que el sistema trabaje, y apretar un botón para que salga ya. Se le ofrecieron dos variantes y eligió la de ver antes de publicar. Quedó así, dentro de "Subir material": subís la foto eligiendo la dimensión, tocás "Preparar", y el sistema genera el copy con Claude y arma la imagen de marca — sin el freno de una-por-día, tomando la foto más reciente de esa carpeta. A los uno o dos minutos aparece en pantalla la story ya armada, con la imagen y el texto. Si te gusta, tocás "Publicar ahora en Instagram y Facebook" y sale. Si no, la descartás.

Por dentro son dos disparos separados de un flujo de trabajo nuevo: uno prepara y renderiza sin publicar, escribiendo un archivo de estado que la aplicación consulta cada pocos segundos hasta que está listo; el otro publica vía Zernio. Los archivos de trabajo son propios de este modo, con nombres distintos a los del proceso diario, así que los dos pueden convivir sin pisarse, y comparten un candado para no correr nunca al mismo tiempo. La única condición es que el token de GitHub tenga permiso de ejecutar flujos de trabajo, además del de escritura de contenido.

Se probó la fase de preparación con una foto de prueba: el flujo terminó bien, el estado quedó marcado como preparado con el título y el texto, y la imagen se renderizó correctamente con el diseño de marca. La fase de publicación no se probó para no mandar una story real de prueba a las redes, pero usa el mismo publicador de siempre.

Después de cerrar el plan, Pablo pidió probar todo el sistema como un usuario real y, sobre todo, que en ninguna pantalla apareciera la palabra "GitHub" — que todo se sintiera parte de MejoraSM y no de una herramienta pegada con cinta. La validación se hizo leyendo el código de punta a punta, porque hay una regla que impide escribir contraseñas, incluso las suyas en su propio sistema.

Se encontraron tres capas distintas de "GitHub a la vista" y se atacó cada una. La más profunda: hasta ese momento, para que la aplicación pudiera guardar fotos y disparar publicaciones, Pablo tenía que generar un token en la web de GitHub y pegarlo en la app, que lo guardaba en el navegador. Eso se movió entero al servidor: se creó una función de backend que guarda ese token como secreto y hace las llamadas a GitHub por su cuenta; la aplicación ahora habla con esa función usando la misma sesión de siempre. Se sacó el diálogo de "conectar con GitHub", el cartel de "GitHub conectado", el enlace para crear el token, todo. La segunda capa eran textos y enlaces sueltos que nombraban "GitHub Actions" en el Monitor y en el detalle de una propuesta — se reemplazaron por lenguaje genérico. La tercera es la dirección del navegador, que muestra "github punto io": para eso se preparó un dominio propio, mejorasm punto mejoraok punto com, pero falta que Pablo agregue un registro de DNS antes de activarlo.

También pidió dejar todo en blanco para empezar a usarlo en serio, manteniendo el Manual de Marca y el registro de lo que ya se publicó en las redes. Se borraron las sesiones viejas de prueba de la Mesa de Diálogo, cientos de registros de actividad, consejos cacheados y cuatro borradores que nunca salieron. Se mantuvieron los diecinueve documentos de marca, la configuración de los agentes, el login, las métricas reales y las doce piezas publicadas. Instagram y Facebook no se tocaron.

Y por último, la Biblioteca. Pablo la abrió, la vio llena de datos de ejemplo, y preguntó por qué era una cosa aparte del sistema. La respuesta honesta es que se construyó antes que todo lo demás, como herramienta suelta, y cuando se armó el panel único se metió con un iframe en vez de reescribirla, para no romperla. Pero quedó como la oveja negra: su propio flujo de token, sus propios datos falsos, y dos de sus cuatro columnas nunca tuvieron backend real. La recomendación fue sacarla, y Pablo estuvo de acuerdo: "no encaja en el objetivo del proyecto hoy". Se quitó del menú, la ruta redirige a Propuestas, y toda la carpeta se borró — queda en el historial de git por si algún día hace falta. Las fuentes de marca que vivían ahí se movieron a la carpeta de plantillas. Lo único que se rescató fue la vista de línea de tiempo, que se rehízo como una pestaña dentro de Propuestas: el ciclo completo de cada pieza, de en revisión a publicada, en una sola vista.

Pablo grabó la pantalla navegando por la app y notó que el Monitor tenía las doce piezas publicadas en agosto, que en la ronda anterior se habían dejado a propósito. Cambió de idea: quería borrar todo y dejar solo el Manual de Marca. Se hizo el wipe total de la base — quedaron los diecinueve documentos de marca, sus embeddings, la configuración de los tres agentes y el login; todo lo demás a cero. En el repositorio se vaciaron las setenta y tres imágenes renderizadas y los archivos de registro.

Pidió lo mismo en Zernio, y que borrar en MejoraSM borrara también en Zernio. Acá hubo un límite: despublicar en masa las publicaciones reales de Instagram y Facebook es una acción irreversible sobre las redes públicas de la marca, y hay una regla que lo impide, incluso con Pablo pidiéndolo. Además Instagram no deja despublicar por su interfaz de programación. Así que se hizo lo que sí corresponde: el proceso automático que sincroniza el historial desde Zernio se pausó, para que no vuelva a traer el contenido viejo al Monitor mientras Pablo lo limpia. Y el botón de tacho de cada tarjeta del Monitor pasó a hacer, en un solo gesto, tres cosas: despublicar de Facebook de verdad, marcar Instagram como pendiente de borrado manual, y sacar la fila del historial. El borrado real de las publicaciones vivas lo hace Pablo, desde las apps o pieza por pieza con ese botón.

## Parte 13 — El desafío sobre Zernio y Anthropic, y la bandeja de conversaciones (2026-09-01)

Pablo lanzó un desafío antes de seguir: preguntó si Zernio era de verdad la mejor herramienta para publicar y medir vía interfaz de programación, o si valía la pena que se dedicara doce horas a investigar algo mejor. Y de paso, si Anthropic era la mejor inteligencia artificial para esto. La investigación se hizo en serio, con fuentes, y la conclusión fue que las dos elecciones se sostienen. Zernio cubre dieciséis plataformas y expone en una sola interfaz la publicación, las analíticas, la bandeja de comentarios y mensajes, y hasta la parte de anuncios pagos, todo con un solo token. Las alternativas reales o son más caras para el mismo alcance, o no tienen bandeja de entrada, o exigen que uno se hospede el servidor. Migrar significaría rehacer toda la integración por un beneficio que no aparece. Con Anthropic pasó algo parecido: sigue siendo el mejor siguiendo un criterio de marca largo y en español rioplatense, que es de lo que dependen el agente Crítico y el copiloto; el modelo de Groq ya está de respaldo donde importa. Lo valioso que salió de la investigación fue otra cosa: el cuello de botella no era el proveedor, era que no se estaba usando todo lo que Zernio ya ofrecía. La bandeja de conversaciones y los anuncios estaban sin tocar. De ahí salió el plan de fases.

Pablo dio la orden de ejecución: fase uno, luego dos, tres, cinco, cuatro, seis y siete, en ese orden. Obsesivo al detalle, arreglar todo, no parar hasta terminar, y si se agota la sesión, documentar y quedar listo para continuar apenas se restablezca. Solo molestarlo si hacía falta intervención humana de verdad; todo lo demás, resolverlo solo.

La fase uno es la bandeja de conversaciones. El sistema publicaba y medía números, pero nunca veía lo que la gente dice. Se creó una tabla nueva que guarda cada comentario y cada mensaje directo de Instagram y Facebook, con una marca de si es entrante o saliente y una etiqueta de sentimiento: positivo, neutral, negativo o pregunta. Se agregó una función de servidor nueva, llamada bandeja, con tres acciones. La de sincronizar recorre las dos cuentas, trae los mensajes directos y los comentarios de los posts recientes (los viejos devuelven un error de plataforma y se saltean), los guarda sin duplicar, y después le pide a la inteligencia artificial que clasifique el sentimiento de todos los entrantes que todavía no tienen etiqueta, no solo los nuevos, así los que fallan se reintentan en la corrida siguiente. La de redactar toma un mensaje y devuelve una respuesta sugerida en la voz de la marca, apoyada en el Manual de Marca, sin enviar nada. La de responder manda esa respuesta a la red a través de Zernio y marca el mensaje como respondido. Un proceso automático corre cada tres horas.

En la interfaz hay una pantalla nueva, Conversaciones, en el menú, dentro del grupo de publicar y gestionar. Muestra los hilos agrupados, con filtros por plataforma, por sentimiento y por si están sin responder. El flujo para contestar es: se toca redactar, aparece la sugerencia de la inteligencia artificial en un campo editable, se corrige si hace falta, y al enviar hay una confirmación explícita antes de que salga a la red. El tablero principal ahora muestra, en la franja de cosas que necesitan atención, cuántas conversaciones hay sin responder.

Se probó de verdad contra producción. La sincronización trajo ciento setenta y tres mensajes reales y clasificó los cincuenta entrantes con texto: cuarenta neutrales, en su mayoría publicidad ajena que le mandan a la cuenta, seis positivos, dos preguntas y dos negativos. Los negativos eran una queja de audio cortado y alguien que no pudo registrarse ni pagar un evento; las clasificaciones estaban bien. La acción de redactar sobre un mensaje directo real devolvió una respuesta que usaba el nombre de la persona y ofrecía ayuda sin vender. La acción de responder no se probó contra una persona real, porque mandarle un mensaje a alguien solo para testear es exactamente lo que este proyecto viene evitando; quedó verificada por revisión del código, con las formas de la interfaz de Zernio ya confirmadas por las pruebas de esta misma sesión. Un detalle que quedó anotado: alrededor de un quince por ciento de las tandas de clasificación fallan el análisis del texto en una corrida dada, porque la inteligencia artificial no siempre devuelve el formato limpio, pero el reintento en cada sincronización las levanta después, así que el número converge a cero sin romper nada.

Todo verificado en cada cambio: revisión de tipos limpia, cero errores de estilo, sesenta y dos pruebas en verde, compilación limpia, y los tres despliegues automáticos en verde.

## Parte 14 — El plan de publicación de punta a punta: higiene, reciclado, aprendizaje, LinkedIn, Reels y pauta (2026-09-01)

Con la bandeja de conversaciones cerrada, siguió el resto del plan en el orden que pidió Pablo: dos, tres, cinco, cuatro, seis y siete. Todo autónomo, todo a producción directo, sin frenar salvo lo que realmente necesita que él haga algo con las manos.

La fase dos fue higiene. Había un error viejo del sistema de historias: si alguien volvía a disparar el flujo a mano el mismo día en que ya se había publicado, el primer paso salía sin generar nada y el segundo se caía porque buscaba un archivo que no existía. Se arregló en tres lugares: ahora ese primer paso deja un archivo vacío al salir, y los dos pasos siguientes toleran que el archivo falte o venga vacío, sin romperse. Probado en las tres situaciones. Lo otro fue la recolección de métricas: cuando una publicación queda trabada del lado de Zernio, sin que sus números lleguen nunca, cada corrida del proceso automático dejaba un error registrado para siempre. Ahora, pasada una semana desde publicado, se marca como una anomalía conocida de Zernio y deja de contar como falla del sistema; el registro solo queda en rojo si hay un error de verdad. Quedaron pendientes de Pablo dos cosas, las únicas de todo el plan que necesitan intervención humana: agregar el registro de dominio para que la app viva en su propia dirección, y borrar a mano las publicaciones viejas de Instagram y Facebook para poder reactivar la sincronización del historial.

La fase tres es reciclado de contenido. Una función nueva busca las piezas publicadas hace más de noventa días que rindieron por encima de la mediana y todavía no se reciclaron, deja que la inteligencia artificial reescriba el gancho y el llamado a la acción manteniendo el ángulo, y al confirmar entra como propuesta pendiente para agendar a mano. Hay una pestaña "Reciclar" nueva en Propuestas. El mecanismo está listo pero dormido: hoy no hay historial porque la base se vació.

La fase cinco es LinkedIn. El código quedó listo: cuando exista la cuenta de LinkedIn conectada en Zernio y su identificador cargado como secreto, los posts de feed salen también a LinkedIn sin tocar nada más. Hasta entonces todo sigue igual, Instagram y Facebook. Es lo único que falta hacer del lado de Pablo para activarlo.

La fase cuatro es el loop de aprendizaje activo. Hasta ahora el sistema analizaba a qué hora convenía publicar mirando lo que ya había pasado, pero como tendía a mandar todo al mismo horario, no había con qué comparar. Ahora, mientras no haya una regla de horario aprendida con confianza alta, el sistema explora: rota entre los tres bloques horarios preferidos eligiendo el menos usado, y registra cada elección como un experimento. Cuando llegan las métricas, el motor de reglas completa el resultado de cada experimento. Así la comparación es real: misma marca, mismo pipeline, distinta hora. Hay una tabla nueva y una fuente de exportación nueva en Auditoría.

La fase seis son los Reels armados de una foto. El flujo: toma la foto más nueva de una dimensión, la inteligencia artificial arma un gancho y un subtexto mirándola, se renderiza un cartel de marca transparente, y con ffmpeg se compone un video vertical de nueve segundos con un zoom lento sobre la foto, el cartel encima, y una pista de audio en silencio porque Instagram lo exige. El video no se guarda en el repositorio, se sube directo a Zernio, que lo publica como Reel. Se probó de punta a punta el armado del video con una foto genérica de prueba y salió un archivo válido; la publicación real no se probó porque sería un Reel público de verdad. Se descubrió en el camino que ffmpeg ya no viene instalado en los servidores de GitHub, así que el flujo lo instala.

La fase siete es la pauta de Facebook, pero solo de lectura. Una función lee las campañas de anuncios que haya en Zernio, hoy ninguna porque no hay cuenta conectada, y detecta qué posts orgánicos recientes rindieron bien y valdría la pena promocionar, con un consejo en la voz de la marca. Nunca gasta un peso: no crea ni modifica campañas. Pautar de verdad lo hace Pablo desde el administrador de negocios. Hay una tarjeta desplegable nueva en el tablero.

Un tropiezo real en el medio, que quedó anotado como aprendizaje: al querer ordenar el estado de git después de una prueba, un comando de recuperación de cambios guardados trajo por error un guardado viejísimo de meses atrás y pisó el archivo principal de documentación con una versión antigua. Se detectó enseguida y se restauró todo desde el último estado bueno, que estaba a salvo. Nada se perdió. La lección: nunca recuperar cambios guardados sin mirar primero qué hay en la lista.

Todo verificado en cada cambio: revisión de tipos limpia, cero errores de estilo, sesenta y dos pruebas en verde, compilación limpia, y los despliegues automáticos en verde. Con esto el plan de publicación 2026 quedó cerrado de punta a punta.

## Parte 15 — Forzar aprobación, un fix real de la Mesa de Diálogo, y la primera coordinación entre dos sesiones de Claude Code (2026-09-02/03)

Se pidió, en tono amplio, revisar bugs y mejoras posibles vía investigación web, UX/UI y optimización de código, probar el sistema de punta a punta y después trabajar activamente en las correcciones — reportando con claridad qué está listo para usar, qué no, y confirmando qué está genuinamente automatizado. En el medio se preguntó también si migrar el hosting a Hostinger traería un beneficio real; la respuesta, con investigación de por medio, fue quedarse en GitHub Pages y Actions.

Con el sistema ya revisado, se probó una publicación real de punta a punta: un post y una historia, generados por Mesa de Diálogo y publicados solos por el pipeline autónomo, sin ningún gate humano en el medio. En una de esas pruebas apareció un problema real: se mandó feedback después de que el Crítico rechazara una propuesta, y la pantalla quedó exactamente igual, como si no hubiera pasado nada. La causa tenía dos partes. La mutación de continuar el diálogo solo invalidaba la lista de sesiones, nunca los mensajes de esa sesión puntual — y la consulta de mensajes corta su actualización automática apenas la sesión deja de estar activa, así que los turnos nuevos del Creativo y el Crítico de la segunda vuelta nunca llegaban a la pantalla sin recargar a mano. Se corrigió invalidando también la consulta de mensajes de esa sesión, y de paso se agregó un aviso de error real a los dos flujos de inicio y continuación, que hasta ahora fallaban en silencio.

Con ese fix ya en producción, apareció el segundo problema, más de fondo: el Crítico aprobó en una ronda de revisión y no pasó nada — la pieza no se agendó. La causa real es que la función que crea la propuesta y la agenda solo existía en el camino de la primera ronda, nunca en el de una revisión con feedback — una aprobación inicial autoagendaba, una aprobación después de corregir algo no hacía nada, sin ningún aviso de que fuera una limitación de diseño y no una falla. Se sacó esa lógica a una función compartida y se aplicó también al camino de revisión, que ahora agenda igual y deja el mismo registro que la primera ronda.

En la misma conversación se hizo una pregunta directa: si al final la última palabra la tiene él, cómo hace para forzar una aprobación cuando quiere publicar algo que el Crítico frenó. Se construyó esa función: toma el último texto real que escribió el Creativo en la sesión, apruebe o no el Crítico, y lo agenda igual que si hubiera sido aprobado — dejando un registro explícito, un mensaje de auditoría más una marca en la sesión, de que fue una decisión humana y no del sistema. En la pantalla apareció un botón nuevo, visible solo cuando el Crítico rechazó, que pide una confirmación explícita antes de saltear el chequeo automático de marca.

A mitad de ese desarrollo, al ir a subir el cambio, apareció que el repositorio remoto había cambiado enormemente desde el último pull — otra sesión de Claude Code, corriendo en paralelo, había reinstaurado el login, vaciado la base de datos, sacado Biblioteca y Laboratorio del sistema, y cerrado por completo un plan de siete fases de publicación. Hubo un conflicto real de fusión en el mismo archivo que se estaba editando. En vez de resolverlo a ciegas, se guardó el trabajo propio en una rama de respaldo local, se abortó la fusión, se sincronizó todo contra el estado real del repositorio, se leyó la documentación para entender qué había cambiado, y se rehizo el trabajo del botón de forzar aprobación desde cero, contra el código real y actual — sin tocar ni revertir nada de lo que la otra sesión había hecho. Se le presentó la situación completa, sin resolver nada por cuenta propia, y la respuesta fue clara: toda la reestructuración era suya, era correcta, no había que modificar nada, y había que seguir con lo que ya se venía haciendo. Se probó el resultado final contra producción real — los dos despliegues automáticos, el de las funciones de servidor y el del sitio, corrieron en verde sobre el mismo cambio.

Aparte de ese trabajo, se pidió una versión visual con identidad de marca de un documento de cierre de ciclo que se había compartido, pensado como traspaso a gestión de proyecto. Se armó un HTML con la paleta y la tipografía del manual de marca real y se publicó como una página compartible. Al ir a dejar una copia en la carpeta de entregables del repositorio, apareció que la otra sesión ya había subido su propia versión del mismo documento — las dos habían recibido el mismo archivo de origen y habían generado, cada una por su cuenta y sin saberlo, su propio HTML. Sin ninguna sesión en vivo con la que coordinar en ese momento, se compararon las dos versiones en detalle antes de tocar nada: la propia tenía el isotipo real del repositorio, liviano y transparente, contra una foto de 122 kilobytes con metadata de certificación de contenido pegada adentro sin ningún uso ahí, y una barra de navegación con enlaces a cada sección que la otra no tenía; la otra tenía, a la vista del lector y no escondida en un comentario invisible, la aclaración de que la tipografía paga de la marca se había reemplazado por dos fuentes abiertas, y una etiqueta de categoría en cada tarjeta de pendientes que la propia no tenía. Se armó una sola versión final con lo mejor de las dos, en vez de dejar dos compitiendo o pisar una a ciegas.

Mientras se hacía ese trabajo, la otra sesión subió un archivo nuevo: un tablero vivo de coordinación entre sesiones de Claude Code, con una fila por sesión, una lista de archivos que las dos suelen tocar, y un espacio de mensajes entre ellas, con uno ya escrito y dirigido a esta sesión. De ahí salió un protocolo duro, documentado también en el archivo principal: sincronizar siempre por reorganización y nunca por fusión antes de tocar el repositorio, nunca recuperar cambios guardados sin mirar antes qué hay en la lista (una lección real de esa misma tarde: un comando de recuperación trajo por error un guardado viejísimo, de meses atrás, y pisó el archivo principal de documentación con una versión vieja — se detectó enseguida y se restauró sin perder nada), hacer commits chicos y frecuentes, avisar antes de tocar un archivo caliente, y coordinar cualquier cambio de esquema de base de datos o despliegue de funciones de servidor.

Se respondió en ese tablero confirmando que las funciones de servidor habían quedado verdes después del cambio, y avisando el único cruce real con el trabajo de la otra sesión — la consolidación del documento de cierre de ciclo — explicando qué se había hecho y por qué. Las dos sesiones quedaron con su carril libre, sin trabajo pendiente entre ellas.

Del lado de la sesión que había cerrado el plan de publicación, la coordinación se vio así: se pidió ponerse de acuerdo con la sesión nueva. No había canal en vivo — la otra no aparecía como sesión alcanzable —, así que se leyó lo que había hecho, se confirmó que su trabajo sobre la Mesa de Diálogo se integraba limpio con los experimentos de horario ya existentes sin ningún conflicto, y se armó el tablero de coordinación con el reparto de carriles y un mensaje dejado ahí para que lo leyera al abrir. Sobre el documento de cierre de ciclo, se pidió dejar un solo enlace compartible. No hay forma de borrar una página publicada desde la herramienta, y la de la otra sesión estaba en otra cuenta, fuera de alcance. Se resolvió al revés: se volvió a publicar la página propia con la versión consolidada del repositorio —revisada entera antes: contenido fiel al documento de texto, sin scripts, solo el isotipo embebido—, de modo que el enlace que ya estaba en circulación pasó a mostrar la versión buena, idéntica a la del repositorio. Se dejó constancia en el tablero para que la otra sesión pudiera dar de baja la suya. Con eso, y las dos mitades de la documentación al día, la sesión se cerró: todo committeado y pusheado, sin trabajo en curso, los dos carriles libres.

## Parte 16 — El mandato "arreglar todo" a las dos sesiones en simultáneo (2026-09-03)

Pablo les dio a las dos sesiones el mismo mensaje: obsesivos al detalle, arreglar todo, no parar hasta terminar, coordinar entre ellas dentro del trabajo, trabajar en paralelo, y molestarlo solo si de verdad hace falta que él haga algo con las manos. La sesión que había cerrado el plan de publicación armó el reparto en el tablero de coordinación —el backend de diálogo y sus pantallas para una, el pipeline de contenido y la infraestructura para la otra, el frontend común para el que lo agarre primero avisando— y arrancó una auditoría.

El primer hallazgo grande fue el historial de migraciones de la base de datos. La herramienta de línea de comandos mostraba solo las seis primeras migraciones como aplicadas y las diecinueve siguientes como pendientes, aunque estaban todas realmente aplicadas —verificado columna por columna y tabla por tabla contra la base real—. Se habían aplicado con un comando que corre el SQL pero no lo registra en la tabla de historial. El riesgo era que el flujo de despliegue de migraciones, si alguien lo disparaba, iba a reintentar las diecinueve, incluido el par que abre y vuelve a cerrar el control de acceso. Se reparó el historial con el comando específico para eso, que solo escribe la tabla de registro sin tocar el esquema. Después de eso, la simulación de empuje da "al día" y ese flujo dejó de ser una bomba.

El segundo hallazgo fue peor: el sistema de historias diarias venía fallando desde el día anterior y nadie lo había visto. La historia se generaba y se dibujaba bien, pero el paso que la guarda en el repositorio hacía referencia a una carpeta —la de fotos entrantes— que dejó de existir cuando se vació la base y nadie subió fotos nuevas. El comando de guardado sale con error cuando una de las rutas no existe, y eso mataba el paso: la historia nunca se guardaba ni se publicaba. Se arregló poniendo un archivo marcador en esa carpeta y las seis sub-carpetas para que siempre existan en el repositorio, y de paso se emparejaron los cinco flujos que guardan contenido: todos filtran ahora las rutas que no existen y todos usan el mismo bucle de reintento con sincronización antes de empujar, para no fallar cuando otra sesión o un proceso automático empuja al mismo tiempo. Se le agregó al flujo de historias diarias una opción para dispararlo a mano sin publicar, para poder probar el circuito sin generar una publicación real.

Después vinieron los arreglos más chicos, todos verificados con la batería de pruebas en verde: se sacó de tres funciones de servidor un comodín de origen que devolvía permiso de acceso entre orígenes a cualquier dirección terminada en un dominio de hosting que el proyecto nunca usó —superficie de ataque muerta—, y se avisó a la otra sesión que en su lane quedaban cinco funciones con el mismo residuo. Se borró un archivo de ayuda de reintentos de red que no lo importaba nadie. Se apretó la política de seguridad de contenido de la página: la lista de destinos de conexión bajó de seis a tres, porque el navegador nunca llama a los modelos de inteligencia artificial directo, todo pasa por las funciones de servidor. Se reescribió el archivo de ejemplo de variables de entorno, que describía un ruteo de modelos viejo de hace un mes. Se cerraron varias notas de documentación que daban por pendientes cosas ya resueltas —una función retirada que en realidad ya no estaba en el proyecto, una expresión de detección de símbolos que ya estaba ampliada—. Se agregaron pruebas de humo para dos pantallas del plan de publicación que no tenían ninguna. Y se sacó del proyecto un componente de formulario que no usaba ninguna página, junto con las tres bibliotecas que solo ese archivo traía.

En todo el tramo, cada hallazgo con su arreglo se fue anotando en el tablero de coordinación, marcado con la sigla de la sesión, para que la otra pudiera ver el estado sin preguntar y para dejar señaladas las cosas que caían en su lane. Las dos sesiones trabajaron sobre la misma rama principal, sincronizando antes de cada empuje y haciendo commits chicos y frecuentes.

## Parte 17 — La otra sesión responde al mandato: auditoría del backend de diálogo (2026-09-03)

Del lado de esta sesión, el reparto quedó aceptado tal cual estaba propuesto en el tablero, sin cambios: backend de diálogo (el orquestador de Mesa de Diálogo, el procesador de la Bóveda, el copiloto, el motor de insights, el clasificador de fotos) y las pantallas que dependen de ellos.

Primero los tres hallazgos que había dejado marcados la otra sesión, todos mecánicos: el mismo comodín de origen muerto sacado de las cinco funciones que faltaban; un aviso de estilo sobre un efecto que reseteaba un formulario de edición, documentado como falso positivo real (depender del objeto completo en vez de solo su identificador hubiera pisado una edición en curso) en vez de silenciado a ciegas; y una decisión sobre un script viejo de carga masiva de documentos a la Bóveda — se queda, es una herramienta operativa documentada, sin ningún riesgo mientras no se ejecute a mano.

Después, una auditoría obsesiva del resto de la lane, leyendo cada función y cada pantalla de punta a punta en vez de dar por buenas las partes ya tocadas antes. Salieron cinco hallazgos reales, cada uno con su causa confirmada, no supuesta.

El más serio: una función de saneamiento de tema —recortar a un largo máximo, exigir un mínimo de caracteres, sacar bytes nulos— que existía hace tiempo en el orquestador pero que el camino real de arranque de una sesión nunca llamaba. Un tema escrito a mano pasaba directo, sin ningún filtro. Y peor todavía: si alguna vez se hubiera llamado, esa función tiraba un tipo de error que no estaba definido en ningún lado del archivo — habría reventado con un error genérico de referencia en vez de devolver el mensaje real. Los dos problemas se cerraron juntos: la función quedó conectada al camino real, y el error corregido a uno común, con el mismo criterio de código de estado que ya usa el resto de esa función.

El segundo: el clasificador de tipo de documento de la Bóveda perdía categorías por un detalle de limpieza de texto. Si la inteligencia artificial respondía con un espacio en vez del guion bajo exacto que pedía la instrucción, la limpieza de la respuesta sacaba el espacio junto con la puntuación y el resultado ya no coincidía con ninguna categoría real — caía siempre en "otro", en silencio. Se corrigió normalizando los espacios a guion bajo antes de sacar el resto.

El tercero: en el tablero de insights del panel principal, marcar una lectura como útil o no aplica pintaba el botón como elegido antes de que el guardado terminara, y si el guardado fallaba, el botón se quedaba marcado igual — quedaba mintiendo sobre lo que realmente se había guardado.

El cuarto: la pantalla de la Bóveda tiene dos casilleros reales para elegir archivo — el del botón de subir y otro, independiente, adentro del área de arrastrar y soltar. Solo el primero se vaciaba después de cada subida. Volver a elegir el mismo archivo desde el segundo no disparaba ningún evento —el navegador no vuelve a avisar si el valor del casillero no cambió— así que un reintento por ese camino no hacía absolutamente nada, sin ningún error a la vista.

El quinto conecta con el trabajo del tramo anterior: el panel nuevo de "últimas decisiones del sistema" en Configuración no contemplaba el botón de forzar aprobación agregado unos días antes — una sesión aprobada así no tiene el mismo campo de evaluación que una aprobada por el Crítico real, así que caía en la rama por defecto y mostraba el texto crudo de la base en vez de una etiqueta real, sin ningún indicio de que había sido una decisión humana. Se agregó la rama que faltaba.

Cada arreglo, verificado con revisión de tipos limpia, cero errores de estilo, sesenta y seis pruebas en verde y compilación limpia, y confirmado real en producción: los dos despliegues automáticos —el de las funciones de servidor y el del sitio— corrieron en verde sobre el commit final. Todo anotado en el tablero de coordinación a medida que se encontraba, con la sigla de esta sesión, cerrando la lane sin dejar nada pendiente propio.

## Parte 18 — Segunda pasada, sobre el frontend compartido: dos falsos éxitos reales (2026-09-03)

Pablo repitió el mismo mandato a las dos sesiones, casi palabra por palabra: obsesivos al detalle, investigar y mejorar y arreglar todo, no parar hasta terminar, conversar entre ellas dentro del propio trabajo y resolver solas, y molestarlo solo si hacía falta que él pusiera las manos.

Con la lane propia ya cerrada, esta sesión tomó la parte del frontend que ninguna de las dos había reclamado todavía —el tablero principal, propuestas, calendario, el monitor de publicaciones, la bandeja de conversaciones, la subida de material, la auditoría exportable, la barra de navegación— mientras la otra sesión, que había vuelto a estar activa, seguía con el pipeline y la infraestructura. Se avisó en el tablero de coordinación antes de arrancar.

La revisión fue línea por línea, no superficial. En la capa de datos y en los formularios de propuestas no apareció nada — ya habían pasado por rondas de auditoría anteriores y se notaba: cada mutación real chequea su error, cada acción que falla avisa. Pero en dos lugares apareció el mismo patrón de bug, con la misma forma exacta: una acción real que termina mostrando éxito sin haber comprobado si de verdad lo fue.

El primero, más serio, en el monitor de publicaciones. El botón de "Borrar esta pieza" hace, en un solo gesto, tres cosas: despublica de Facebook de verdad, marca Instagram como gestionado a mano (esa red no permite despublicar por API), y saca la fila de la vista. El problema estaba en el paso del medio: la función que dispara esas dos primeras acciones nunca deja que el error se propague — lo atrapa ella misma, muestra su propio aviso, y devuelve simplemente verdadero o falso. El botón de borrar no miraba ese resultado. Si el despublicado de Facebook fallaba por cualquier motivo real —un permiso vencido, la red, un límite de la plataforma— el código seguía de largo igual, sacaba la pieza de la vista, y decía "listo, borrada", aunque Facebook nunca se hubiera tocado de verdad. Se confirmó, revisando el resto del mismo archivo, que los otros cuatro lugares que usan la misma función sí comprobaban bien el resultado — el bug era puntual a este botón. Se corrigió para que, si cualquiera de los dos pasos falla, no se toque la vista y quede claro que hay que reintentar.

El segundo, más chico pero de la misma familia, en la pantalla de propuestas: el botón de copiar el texto completo de una pieza no esperaba la confirmación real del navegador al escribir en el portapapeles —esa operación puede fallar, por ejemplo si el sitio no corre en un contexto seguro o el permiso está bloqueado— y mostraba el tilde de "copiado" igual, sin importar si algo se había copiado de verdad. El detalle de una propuesta, en otra pantalla, ya tenía resuelto el mismo botón con el patrón correcto; se replicó ahí.

En el medio, la otra sesión revisó por su cuenta el trabajo del tramo anterior y encontró un error propio, real: el comentario que silenciaba un aviso del linter había quedado varias líneas más arriba de donde hacía falta, separado por un bloque de comentario explicativo en el medio — no estaba tapando nada, y de paso sumaba un aviso nuevo de "comentario sin usar". Lo corrigió, confirmado con el conteo real de avisos antes y después.

Todo, en los dos casos, verificado con la misma batería de siempre —tipos, estilo, sesenta y seis pruebas, compilación— y confirmado en producción real con el despliegue automático del sitio en verde. Cada hallazgo, cada arreglo y cada confirmación se fue dejando en el tablero de coordinación a medida que pasaba, para que la otra sesión pudiera seguir el estado sin tener que preguntar.

## Parte 19 — El otro pase: la bandeja que nunca terminaba de clasificar, y timeouts en todo el pipeline (2026-09-03)

La sesión del pipeline volvió a activarse con el mismo mandato y encaró su lane con criterio de mejorar, no solo de tapar bugs conocidos.

Lo primero fue la bandeja de conversaciones. La documentación decía que quedaba un pendiente no bloqueante: alrededor del quince por ciento de las tandas de clasificación de sentimiento fallaban el parseo del JSON en una corrida dada, pero el reintento de la corrida siguiente las levantaba, así que convergía a cero sin fallar. Al mirarlo de verdad, eso no era lo que pasaba. Los que quedaban sin clasificar eran siempre los mismos diez mensajes viejos —publicidad de otras marcas, respuestas automáticas de cuentas comerciales, mensajes que son solo un adjunto sin texto—, y como el modelo los procesa en tandas de diez, esos diez rompían el JSON de la tanda entera y fallaban juntos, corrida tras corrida, sin converger nunca. Era un conjunto determinístico, no ruido aleatorio.

Hubo un intento de mejora que salió mal y conviene dejarlo anotado como lección. Se probó hacer la clasificación más robusta bajando la temperatura del modelo a cero, dándole más presupuesto de tokens para que no se corte el JSON, y corriendo el parser de respaldo siempre en vez de solo cuando el JSON falla entero. Sonaba razonable. En producción empeoró: la clasificación general bajó de alrededor del ochenta y cinco por ciento por corrida a cerca del veinte. Se revirtió el cambio completo. La lección quedó escrita: un cambio de prompt no se shippea sin verlo correr en producción primero, porque el comportamiento real del modelo no se predice leyendo el diff.

El arreglo que sí funcionó fue en dos pasos chicos. Si una tanda no logra clasificar ni un solo mensaje, se reintenta uno por uno —un solo objeto JSON siempre se puede parsear, no hay tanda que romper—. Y los mensajes que son puramente un adjunto, sin ninguna palabra, se marcan como neutrales directamente, sin gastar una llamada al modelo, porque no hay nada que clasificar ahí. Con eso, verificado contra la base real, los ochenta y cuatro mensajes entrantes con texto quedaron clasificados, cero pendientes. El pendiente de la primera fase del plan de publicación quedó cerrado.

Después vino el endurecimiento de las bibliotecas del pipeline. Ninguna de las llamadas de red que hace el sistema de publicación —ni a la API de inteligencia artificial principal, ni a su respaldo, ni a la plataforma de redes sociales, ni a la base de datos para el registro de corridas— tenía tope de tiempo. Una conexión que se cuelga bloqueaba la corrida de acciones automáticas hasta el límite de seis horas del trabajo. Se les puso a todas un límite: noventa segundos para los modelos de lenguaje, quince para el registro de corridas, cuarenta y cinco para la plataforma de redes y ciento veinte para la subida de video, que es lenta. De paso, la llamada al modelo principal ahora reintenta también cuando el pedido mismo falla por un problema de red, no solo cuando el servidor responde con un error transitorio; y el cliente de la plataforma de redes ya no revienta si le devuelven una página de error HTML en vez de JSON.

En las funciones de servidor del lane aparecieron dos pedazos de código muerto. Uno era un generador de "insights" con consejos escritos a mano —cosas como "engagement por encima del promedio, seguir así"— que quedó obsoleto desde que existe la función de insights real que contrasta contra métricas de verdad; no lo llamaba nadie, ni el frontend ni ningún proceso automático. El otro era un generador de sugerencias en el motor de reglas que tampoco usa nadie: quien consume las reglas aprendidas es el orquestador, que lee la tabla directo. Ese segundo además arrastraba un error de precedencia de operadores que hacía que el texto informativo de cada regla nunca se usara. Los dos se borraron, siguiendo la regla de la casa de que lo que no se usa se saca.

La función que hace de puente con el repositorio de código se endureció en dos puntos: ahora reintenta una vez con la referencia fresca cuando dos subidas de foto casi simultáneas chocan, y convierte a texto todo valor de parámetro antes de disparar un flujo de trabajo, porque la API rechaza un booleano.

En la integración continua y los flujos de trabajo se completó el trabajo de permisos mínimos: los veinte flujos declaran ahora explícitamente qué acceso necesitan, y los que solo hacen una llamada a una función de servidor declaran que no tocan el repositorio para nada. Se agregó control de concurrencia donde faltaba: la integración continua cancela la validación de un commit ya superado por otro más nuevo para no gastar una máquina de más, y los flujos de migraciones, de sincronización de historial y del piloto automático se serializan para que nunca se corte uno a la mitad ni choquen dos al escribir. Y en la configuración de pruebas se puso un margen de tiempo más generoso y un reintento: una prueba de pantalla que flaquea una vez por carga de la máquina pasa en el segundo intento, mientras que una rota de verdad falla las dos veces —eso mata el falso rojo que aparecía cada tanto en la integración continua.

El resto fueron correcciones de documentación: el diagrama de arquitectura seguía mostrando dos proveedores de inteligencia artificial que el ruteo real no elige desde hace más de un mes; un script de despliegue manual tenía la lista de funciones desactualizada, secretos que ya no se usan y una dirección vieja; y tres flujos de trabajo referenciaban un archivo de planificación que se borró hace tiempo. Las dos funciones que quedaban por revisar —la de reciclado de contenido y la de pauta— se miraron sin encontrar nada que ameritara tocarlas: están dormidas, esperando datos que todavía no existen porque la base se vació hace días. Todo verificado en verde en cada commit, con los despliegues de funciones y de integración continua confirmados, y las funciones recortadas corridas después del despliegue para comprobar que seguían respondiendo bien. Cada paso quedó anotado en el tablero de coordinación, sin cruces con la otra sesión.

## Parte 20 — Los diecisiete scripts del pipeline, uno por uno (2026-09-03)

Con el grueso del pase cerrado, la sesión del pipeline volvió a Pablo con "seguí con lo que quede en tu lane" y encaró lo único que faltaba mirar con lupa: cada script del pipeline de publicación, leído entero, buscando lógica rota y no solo llamadas de red sin tope. Salieron tres cosas de fondo.

La primera, en el piloto automático. El módulo tiene un invariante escrito en su propio encabezado: nunca se publica algo sin haber avisado antes. El flujo era: el orquestador aprueba una pieza y la deja agendada para la hora que elija su propia lógica de horarios —que puede ser dentro de pocas horas, el mismo día—; el piloto le empuja esa hora a la ventana de veto de la noche; y recién después manda el correo de aviso. El agujero: si el proceso se moría entre el empuje de la hora y el envío del correo —o si la llamada al servicio de correo se colgaba—, la pieza quedaba agendada y el cron de publicación, que corre cada quince minutos, la sacaba sin que nadie hubiera visto nada. Se dio vuelta el orden. Apenas el crítico aprueba, la pieza se desarma: pasa a pendiente, sin hora. Si algo se cae ahí, no se publica nada. Se manda el correo. Y solo si el correo sale bien, se re-arma para la ventana de veto. Cualquier caída en el medio deja la pieza en pendiente: molesto, porque hay que agendarla a mano desde el enlace del correo, pero no peligroso.

La segunda, en el publicador de posts programados. Cuando una publicación fallaba, el script guardaba el error en un campo de metadatos de la propuesta. El problema es que ese campo es un objeto estructurado, y la forma en que se escribía reemplazaba el objeto entero en vez de agregarle una clave. Así, cada fallo de publicación borraba en silencio otros datos que vivían ahí: de qué pieza original venía un reciclado, a qué experimento de horario pertenecía, si una aprobación había sido forzada a mano por Pablo por encima del crítico. Ahora se lee el objeto actual y se le agrega el error sin tocar el resto.

La tercera, en la herramienta de gestión manual de posts ya publicados. Después de despublicar un post en el servicio de redes, el script marcaba la propuesta como rechazada en la base —pero nunca comprobaba si esa escritura había funcionado. Si fallaba, quedaba una propuesta que la base decía "publicada" mientras el post real ya estaba de baja: exactamente el tipo de desincronización que había causado bugs visibles en el monitor semanas atrás. Ahora falla ruidosamente si esa escritura no entra. De paso, esa herramienta pasó a dejar un registro en la tabla de observabilidad —era el único script del pipeline que no lo hacía, y despublicar algo es una acción lo bastante consecuente como para querer saber quién y cuándo.

Lo demás fueron ajustes menores. El sincronizador de historial recorría las páginas de resultados del servicio de redes con un bucle sin tope: una respuesta de paginación mal formada podía dejarlo dando vueltas para siempre; se le puso un límite de cincuenta páginas. En el flujo de los reels, la foto usada se commiteaba antes de publicar; como el video no se sirve desde el repositorio sino que se sube directo al servicio, ese orden no aportaba nada y, si la publicación fallaba, dejaba la foto fuera de la carpeta de entrada sin poder re-disparar el flujo tal cual —ahora se commitea después, y si el empuje no entra es solo una advertencia, no un fallo, porque el reel ya salió. Y se pusieron topes de tiempo en el llamado a la herramienta de composición de video y en la descarga de la identidad de marca, que se hace contra un servidor con caídas documentadas.

Cinco scripts más —los dos de renderizado y publicación de historias, la gestión manual de historias, el escritor del manifiesto de "publicar ahora" y su flujo de trabajo— se revisaron sin encontrar nada real. Están bien. Todo verificado, cada script compilando limpio, la batería de pruebas y la integración continua en verde en cada commit. La lane del pipeline queda cerrada de verdad este pase: no hay más que arreglar de ese lado sin inventar hallazgos para justificar seguir.

## Parte 21 — Tercera pasada de la sesión de diálogo: un onboarding que podía tumbar toda la app en blanco (2026-09-03)

Con las dos lanes originales cerradas y verificadas, la sesión de diálogo siguió el mandato de Pablo por lo que genuinamente nadie había mirado todavía en ninguna de las dos rondas anteriores: las pantallas de acceso —inicio de sesión, el portón que envuelve toda la aplicación, la recuperación de contraseña dentro de la aplicación y la página suelta fuera del enrutador que hace el blanqueo real—, y el resto de los componentes sueltos que habían quedado sin abrir: el de reciclado de contenido, el generador de informe imprimible, la tarjeta de pauta publicitaria, los comentarios anclados a una propuesta, el diálogo de confirmación genérico, el renderizador mínimo de markdown, el tutorial de bienvenida, la página de ruta inexistente, el armazón visual que envuelve cada pantalla, y dos piezas de lógica pura: la cola de subida de fotos completa y las utilidades de exportación a CSV y JSON.

Todo se leyó completo, archivo por archivo, con el mismo criterio de siempre: no inventar hallazgos para justificar seguir trabajando, decir con honestidad cuando algo está limpio. La inmensa mayoría lo estaba. Apareció un solo problema real, y era serio por dónde vivía, no por lo grande que fuera el cambio.

El tutorial de bienvenida —la ventana que se muestra la primera vez que alguien entra a la aplicación, explicando en cuatro pasos cómo funciona el sistema— se monta en el archivo principal de la aplicación como hermano del componente que atrapa errores de pantalla, no como hijo suyo. Tiene que ser así por una razón real: necesita poder aparecer arriba de cualquier pantalla, antes incluso de que el enrutador termine de decidir qué mostrar. Pero esa posición tiene una consecuencia que nadie había advertido: si algo revienta ahí adentro, no hay ningún paraguas por encima que lo atrape.

Y adentro había tres llamadas al almacenaminame del navegador sin ninguna protección: una lectura al montarse, para decidir si mostrar el tutorial o no, y dos escrituras, una cuando se termina el tutorial y otra cuando se lo salta. El mismo repositorio ya tenía resuelto exactamente este mismo riesgo en otro lugar —el sistema de comentarios sobre una propuesta guarda el nombre de quien comenta en el mismo almacenamiento, y ese código sí está envuelto en un intento con su captura de error, con un comentario explícito que dice "modo privado o storage bloqueado, se pide el nombre de nuevo"—. El tutorial de bienvenida no tenía esa misma protección.

La consecuencia concreta: alguien que entra por primera vez a la aplicación desde una ventana de navegación privada, o con una extensión de privacidad que bloquea el almacenamiento local, dispara esa lectura sin protección apenas carga la página. Si el navegador la rechaza con un error de seguridad, la excepción no tiene nada que la atrape —ni el paraguas de errores, que queda por fuera; ni ningún otro más arriba, porque no hay ninguno—. Toda la aplicación se cae a una pantalla en blanco, sin ningún mensaje, sin ningún botón para reintentar, en la primerísima visita de esa persona.

El arreglo fue envolver las tres llamadas en su propio intento con captura, siguiendo el mismo criterio ya establecido en el resto del proyecto: si la lectura falla, se trata como si el tutorial nunca se hubiera completado y se ofrece igual; si la escritura falla, simplemente no queda guardado y el tutorial vuelve a aparecer la próxima vez —una degradación aceptable, muy lejos de tumbar la aplicación entera—.

La cola de subida de fotos, que en agosto había tenido un error real de condición de carrera (confirmar una segunda tanda mientras la primera seguía subiendo pisaba el estado de la tanda vieja), se revisó específicamente contra ese mismo patrón para confirmar que no había vuelto. Sigue sana: la cola es una lista real por identificador, no por posición, y el indicador de "ya estoy procesando" vive fuera del estado de React, así que una tanda nueva se agrega al final sin pisar nada de lo que ya estaba en curso.

Con esto, verificado con el mismo chequeo de siempre —tipos, estilo de código, las sesenta y seis pruebas automáticas, la compilación de producción— y confirmado en vivo con el despliegue real, las tres rondas de auditoría de esta sesión (el motor de diálogo, el frontend compartido, y esta tercera pasada) quedaron cerradas sin ningún pendiente propio. Todo quedó anotado en el tablero de coordinación entre las dos sesiones, que en paralelo seguía revisando la lógica interna de los scripts del sistema de publicación uno por uno, sin ningún cruce entre las dos.

## Parte 22 — Cuarta pasada, después del cierre de la otra sesión: seis hallazgos reales en una tarde (2026-09-03)

La otra sesión cerró por límite de cuenta con las dos lanes de esa tanda auditadas de punta a punta y sin ningún hilo abierto —quedó anotado con detalle en el tablero de coordinación qué se había hecho, cuál era el estado real del repositorio, y qué faltaba, todo del lado humano (el registro del dominio propio, limpiar publicaciones viejas a mano, conectar cuentas de redes adicionales). Pablo repitió el mismo mandato de siempre, dirigido esta vez a una sesión nueva. La sesión de diálogo, sin embargo, era la misma de siempre bajo un nombre distinto —la identidad real de una sesión de Claude Code queda fija en cada commit que hace, así que inventar un alias nuevo solo habría confundido el registro de quién hizo qué—, así que siguió bajo su propia identidad y arrancó una cuarta pasada por lo que ninguna ronda anterior había mirado a fondo todavía, para no repetir trabajo ya hecho.

Salieron seis hallazgos reales, uno detrás de otro, en un solo barrido.

El primero, y el más serio por dónde vive: la forma en que el sistema arma el texto final de cada pieza antes de convertirla en imagen. El copy lo escribe la inteligencia artificial y después un script lo inserta en una plantilla HTML reemplazando marcadores como "el título va acá". El método estándar de JavaScript para hacer ese reemplazo de texto tiene un comportamiento poco conocido: si el texto que se está insertando contiene ciertas combinaciones de caracteres —el símbolo de pesos seguido de un ampersand, por ejemplo—, el lenguaje las interpreta como una instrucción especial en vez de como texto literal, sin importar que la búsqueda original fuera un texto simple y no una expresión más compleja. En el peor de los casos, esa instrucción especial hace que el propio marcador sin reemplazar ("el título va acá", literalmente) se reinserte visible en la imagen final. Como el sistema publica solo, sin que nadie revise la pieza antes de que salga a Instagram o Facebook, un hook generado por la inteligencia artificial que por azar contuviera esa combinación de caracteres podría haber terminado publicado con ese error visible, en una cuenta real. Se comprobó el problema con una prueba mínima antes de arreglarlo: al reemplazar así un texto que contenía esa combinación, el resultado mostraba literalmente el marcador sin reemplazar en el medio de la frase. Estaba en tres archivos distintos, diecisiete lugares en total —el que arma los posts programados, el que arma las historias diarias, y la vista previa que se muestra en la pantalla antes de publicar—. El arreglo fue cambiar la forma de pasarle el reemplazo al método: en vez de darle el texto directo, se le da una función que devuelve ese texto, y con una función de por medio el lenguaje nunca interpreta nada como instrucción especial.

El segundo: el archivo que hace de puente entre la aplicación y el repositorio de código en GitHub —donde vive la lógica de subir una foto, o de disparar el proceso que prepara y publica una historia al toque— se había reescrito por completo hacía apenas unos días, con el objetivo de que ya no hablara directo con GitHub sino a través de una función intermedia del lado del servidor. El problema: esa reescritura pasó justo después de que otra ronda de auditoría, unos días antes, hubiera encontrado y arreglado exactamente este mismo tipo de error en otros tres archivos —una llamada de red sin ningún límite de tiempo, que si se cuelga se cuelga para siempre—. La reescritura reintrodujo el mismo error en el archivo nuevo. El caso más visible: la pantalla de "publicar ahora" consulta cada pocos segundos si el proceso de fondo ya terminó, y tiene su propio límite de tiempo total para dejar de intentar y avisar que algo salió mal —pero ese límite se revisa recién después de esperar la respuesta de cada consulta individual. Si una sola de esas consultas se cuelga sin nunca responder ni fallar, la espera nunca termina, el límite total nunca llega a chequearse, y la pantalla se queda mostrando "preparando" o "publicando" para siempre, sin ningún aviso de error, sin más salida que recargar la página a mano y confiar en que ya haya terminado. Se agregó el mismo mecanismo de corte por tiempo que ya usa el resto de la aplicación.

El tercero, en el motor de diálogo entre los tres agentes de inteligencia artificial: la función que fuerza la aprobación de una pieza —el botón que le da a Pablo la última palabra por encima del criterio del agente crítico— sí revisaba si la sesión sobre la que se estaba actuando todavía existía antes de usarla. La función hermana, la que procesa una ronda de feedback normal, hacía exactamente la misma consulta a la base de datos pero nunca revisaba el resultado, y usaba un campo de esa sesión más abajo sin ninguna protección. Si alguien intentaba continuar una sesión que ya no existe —una pestaña del navegador que quedó abierta desde antes de un vaciado completo de la base de datos, por ejemplo, que sí ocurrió una vez este mismo mes—, el error que salía era un mensaje técnico crudo en vez de una explicación clara. El sistema de todas formas no se caía —hay un mecanismo más arriba que atrapa cualquier error y siempre devuelve una respuesta— pero lo que veía Pablo era confuso en vez de útil. Se agregó la misma protección que ya tenía la función hermana.

El cuarto, en el motor que genera lecturas sobre el rendimiento real del contenido: cuando la inteligencia artificial evalúa qué tan segura está de una afirmación, devuelve un número de confianza entre cero y cien. El código que procesa esa respuesta tenía un error clásico del lenguaje: si el modelo devolvía exactamente cero —"no tengo ninguna confianza en esto"—, el código lo trataba como si el dato faltara del todo, y lo reemplazaba en silencio por cincuenta, el valor por defecto. Es exactamente lo opuesto de la regla que el propio sistema le exige a la inteligencia artificial en ese mismo prompt: nunca inventar un número que no venga de los datos reales. Una lectura con cero confianza real terminaba mostrándosele a Pablo como si el modelo hubiera dicho "cincuenta por ciento de seguro" — una cifra inventada por el propio código, no por el modelo. Se probó el arreglo con una comparación directa antes de aplicarlo: la versión vieja convertía un cero real en cincuenta; la nueva lo deja como cero, y sigue completando cincuenta solo cuando el dato realmente falta.

El quinto, en el procesamiento de documentos de la Bóveda de Conocimiento: cuando alguien reprocesa un documento ya cargado —una función real, disponible desde la pantalla—, el sistema primero borraba todos los fragmentos de texto ya indexados de ese documento y recién después insertaba los nuevos. Si ese segundo paso fallaba por cualquier motivo real —un problema de red, un error de la base de datos— el documento quedaba sin ningún fragmento: un documento que antes se podía encontrar por búsqueda semántica dejaba de poder encontrarse, en un estado peor que antes de tocar el botón, sin ningún camino de vuelta salvo volver a subir el archivo entero desde cero. Se confirmó contra la estructura real de la base de datos que los fragmentos no tienen ninguna restricción que impida que convivan dos versiones del mismo documento por un instante, así que el orden se invirtió: insertar los fragmentos nuevos primero, y solo si esa inserción se confirma, borrar los viejos por su identificador real capturado de antemano —nunca borrando por "todos los que pertenecen a este documento", para no arriesgarse a borrar de más.

El sexto no fue un arreglo sino una verificación que vale la pena dejar anotada, porque muestra el criterio en acción: en otro archivo apareció exactamente la misma forma de código que causaba el cuarto hallazgo —un número que, si es cero, se reemplaza por un valor por defecto—, esta vez en un parámetro que decide cuántos resultados devolver una búsqueda. Antes de darlo por bueno o por malo automáticamente, se revisó qué significa ese cero en ese contexto puntual: acá nadie pide honestamente "cero resultados de búsqueda" a propósito, así que tratar un cero como "no se especificó, usá el valor por defecto" es el comportamiento correcto, no un error — muy distinto de una confianza real de cero, que sí es una respuesta legítima y significativa. La misma forma de código, en dos lugares distintos, era un error real en uno y el diseño correcto en el otro. Reconocer un patrón no alcanza — hay que entender qué representa cada valor en su propio contexto antes de decidir si algo está mal.

El resto de lo revisado en esta pasada —los archivos compartidos de autenticación y de registro de corridas, las migraciones de base de datos con sus políticas de acceso, el archivo completo del motor de diálogo de más de mil doscientas líneas, el copiloto reflexivo completo, el clasificador de fotos, el puente al repositorio de código, y media docena de conectores de React entre la interfaz y el backend— se leyó entero sin encontrar nada más que valiera la pena tocar. Deliberadamente no se volvió a leer lo que la otra sesión ya había auditado a fondo con evidencia real en su propia pasada final —las funciones de bandeja de conversaciones, reciclado de contenido, pauta publicitaria, recolector de métricas y motor de reglas—, porque releer eso no habría agregado nada, solo duplicado trabajo ya hecho.

Los seis cambios de código quedaron verificados con el mismo chequeo de siempre —tipos, estilo, las sesenta y seis pruebas automáticas, la compilación de producción— y, para los cinco que tocan funciones del servidor, confirmados uno por uno con el despliegue real en verde, no solo con el código pusheado. Todo quedó anotado en el tablero de coordinación entre las dos sesiones, con el detalle completo de cada hallazgo, por si la otra sesión —o cualquier sesión futura— retoma el trabajo.

## Parte 23 — El salto de versión mayor al enrutador, y una tarde entera de limpieza obsesiva (2026-09-04)

Con el dominio propio activado y los tres pendientes del plan de publicación cerrados, quedaba una sola vulnerabilidad real de seguridad sin resolver: dos avisos de gravedad moderada en la librería de enrutamiento de la aplicación, heredados de una versión vieja de su motor interno. El arreglo automático de las herramientas de auditoría de dependencias ofrecía instalar la versión siguiente, pero avisaba con toda claridad que era un salto de versión mayor —de la sexta a la séptima— y que eso podía romper cosas. La decisión, documentada antes de tocar nada: no hacerlo a ciegas. Se investigó primero qué parte de esa librería usa realmente la aplicación —resultó ser un puñado de piezas básicas y estables: el enrutador que vive en el fragmento de la dirección en vez de en la ruta real, las rutas mismas, los enlaces, la redirección, dos lectores de la ubicación y de los parámetros de búsqueda— y ninguna de las piezas más avanzadas y cambiantes de esa librería, las que sirven para cargar datos del servidor antes de mostrar una pantalla. Con ese uso tan acotado, el riesgo de romper algo con el salto de versión era bajo, pero seguía sin poder probarse el sistema ya con la sesión iniciada, porque eso exige la contraseña real de Pablo. Quedó anotado como una decisión pendiente, no de un bug sino de criterio, para cuando se pudiera probar de verdad.

Pablo lo resolvió con una frase: dale, hacé el salto. Se hizo, y resultó ser un cambio mínimo: una sola línea en el archivo de dependencias. La librería nueva sigue publicando el mismo paquete de compatibilidad que usaba la aplicación, así que ningún archivo de código tuvo que tocarse —los mismos nombres que ya se usaban siguen funcionando igual. Lo único que cambia de comportamiento, y para mejor: al navegar entre pantallas que todavía no terminaron de cargar su parte del código, la aplicación ahora mantiene la pantalla anterior visible mientras carga la siguiente, en vez de mostrar un instante de spinner de carga. Con eso, la vulnerabilidad que de verdad importaba —la que viaja en lo que se despliega a producción, no la que solo vive en herramientas de desarrollo— quedó en cero. Se verificó de punta a punta: tipos, estilo, las sesenta y seis pruebas automáticas, la compilación, y una visita real al sitio ya desplegado, con la consola del navegador abierta, navegando entre pantallas para confirmar que no salía ningún error nuevo.

Cerrado eso, Pablo dio un mandato más amplio: limpiar, optimizar, prolijar, depurar, investigar, mejorar y arreglar todo lo que quedara, sin parar hasta terminar, y que para el final del día no quedara nada pendiente. La otra sesión de Claude Code que venía coordinando el trabajo en paralelo ya no estaba corriendo, así que esta sesión tomó el barrido completo, incluida la parte de la interfaz que normalmente le tocaba a la otra, dejando todo anotado en el tablero de coordinación por si volvía a aparecer.

El primer barrido con una herramienta de detección de código muerto encontró algo real y sustancial: treinta componentes visuales de la librería de interfaz de usuario —los bloques con los que se arma cualquier pantalla, como acordeones, menús desplegables, barras de progreso, controles deslizantes— que nunca los importaba ningún archivo real de la aplicación. Formaban un grupo cerrado: se referenciaban entre ellos mismos, pero ninguno tenía un hilo que lo conectara con una pantalla de verdad. Junto con ellos, un archivo de estilos que quedó del molde inicial del proyecto sin que nadie lo usara nunca, y un pequeño ayudante para detectar si la pantalla es de celular que solo lo usaba uno de los componentes ya descartados. Se verificó cada uno, uno por uno, antes de borrar nada —no alcanza con que una herramienta lo diga, hay que confirmarlo—, y se los borró junto con las casi veinticinco dependencias externas que solo ellos usaban. Ya había un antecedente directo de este mismo tipo de limpieza, unos días antes, con un componente de formularios que tampoco usaba nadie. Con todo borrado, la primera compilación falló: un archivo de configuración del empaquetado tenía escrito a mano el nombre de una de las dependencias justo borradas, como parte de la lista de qué agrupar junto en un mismo archivo final. Se corrigió esa lista para reflejar lo que realmente sigue en uso.

De paso, se encontró que una regla del sistema de estilo de código que revisa variables e importaciones sin usar estaba completamente apagada desde hacía tiempo —nadie se había dado cuenta de que un import muerto en cualquier archivo de la aplicación no lo iba a cachar nadie—. Se prendió, con una salvedad razonable para argumentos que a propósito no se usan. Salieron solo siete casos reales en toda la aplicación: cinco eran importaciones sueltas que sobraban, fáciles de sacar; el sexto era una constante de un archivo del sistema de notificaciones que solo se usaba para su forma, no para su valor, y se convirtió en un tipo puro en vez de un valor real, que es la manera correcta de escribir eso. Con todo esto, la cantidad de avisos del revisor de estilo de código bajó de seis a cero —una limpieza completa, no solo una reducción.

Se aprovechó también para poner al día las dependencias que ya estaban dentro de los rangos de versión que el propio proyecto permite —esas no necesitan ningún cambio en el archivo de configuración, solo bajar la versión más nueva compatible—. Entre otras, la librería de íconos subió treinta y tres versiones menores de un salto, sin que ningún ícono usado en la aplicación cambiara de nombre o desapareciera.

En el medio de este barrido apareció un error propio, cometido y corregido en la misma tarde: al sacar una dependencia marcada como "sin usar" por la herramienta automática, resultó que sí la usaba un script suelto del repositorio —una herramienta documentada para volver a cargar de cero los documentos de la Bóveda de Conocimiento, que no corre desde ningún flujo automático pero sigue siendo una herramienta real que alguien podría necesitar algún día—. La herramienta de detección no mira ese script porque no es uno de sus puntos de entrada por defecto. El error nunca llegó a romper nada en producción porque nada lo ejecuta automáticamente, pero de haber quedado así, la próxima persona que lo corriera se hubiera encontrado con que la dependencia que necesitaba ya no estaba instalada. Se restauró en minutos, con una nota explicando exactamente por qué: no es la misma dependencia que usa el procesamiento de documentos del lado del servidor —esa vive en otro entorno de ejecución completamente distinto y no depende de lo que haya en el archivo de configuración de este proyecto—; es un paquete real, instalado, que un script de Node de verdad importa.

El resto de la tarde fue una revisión más silenciosa, sin grandes sorpresas, que vale la pena dejar anotada igual porque es la que da la tranquilidad de que no queda nada suelto. Se revisaron a mano, uno por uno, más de treinta hallazgos que la herramienta de detección marcaba como "exportado pero sin uso" —la inmensa mayoría resultaron ser piezas normales de una librería de componentes de interfaz, exportadas por consistencia aunque la aplicación hoy no use cada una; o tipos de datos que documentan la forma de una función sin que haga falta importarlos por nombre en ningún otro lado, que es como funciona el sistema de tipos del lenguaje—. Dos sí eran reales y se sacaron: dos archivos de pantalla, el de inicio de sesión y el de blanqueo de contraseña, tenían cada uno una forma de exportarse a sí mismos que nadie usaba, duplicada con la forma real que sí se usa en el resto del código. Se revisaron también los flujos de trabajo automáticos de GitHub —las versiones de cada pieza externa que usan, la configuración de node, si aprovechan el cache de dependencias— sin encontrar nada para tocar: ya estaban al día de una limpieza anterior. Y una revisión final de los archivos sueltos que se sirven tal cual, sin pasar por ningún proceso de construcción, encontró dos restos reales del molde original del proyecto: un archivo de configuración de un servidor web que nunca corre en el hosting real que usa este sitio, con reglas que además apuntaban a una dirección vieja de antes de que existiera el dominio propio; y una imagen de relleno del molde inicial que ninguna pantalla real llegó a usar nunca. Los dos se borraron.

Cada uno de estos cambios, sin excepción, se verificó con la misma batería completa antes de subirlo: tipos sin errores, estilo de código en cero avisos, las sesenta y seis pruebas automáticas en verde, la compilación de producción limpia, y la integración continua confirmada en verde sobre cada commit real, no solo sobre lo que se corrió en la máquina local. El sitio real, ya desplegado con todos estos cambios encima, se visitó una vez más al final para confirmar con los propios ojos que la pantalla de acceso sigue viéndose exactamente igual que siempre, sin ningún rastro visual de que por debajo se habían sacado más de tres mil líneas de código que nadie usaba.

## Parte 24 — Las capturas que se veían mal cortadas, y un collage nuevo que arma solo el propio sistema (2026-09-04)

Pablo trajo dos pedidos en el mismo mensaje: que las plantillas de las piezas fueran más variadas y creativas, siguiendo la estética del año, sin perder la identidad de la marca; y que las capturas de las sesiones con clientes —imágenes que hoy se usan como cualquier otra foto real en las piezas— se estaban viendo mal recortadas. Pidió que se resolviera de forma automática si era posible, y que si no, le dejaran editar con una herramienta propia dentro del mismo sistema.

Antes de tocar nada se buscó la causa real del recorte, no un síntoma. La encontró rápido: las dos plantillas que arman las imágenes reales —la del post de feed y la de la historia— tenían la foto configurada para llenar todo el lienzo recortando desde el centro, sin mirar nunca la forma real de esa foto. Una foto de estudio, tomada en formato retrato, tolera bien ese recorte porque ya viene compuesta para eso. Una captura de pantalla —de una videollamada, panorámica; o de una conversación de chat, angosta y alta— no tolera nada: el recorte centrado se come contenido real de los bordes, sin ningún criterio.

El arreglo fue estructural, no un parche puntual: la foto real ya no se fuerza a llenar el lienzo entero. Ahora se ve completa siempre, sin importar su forma, y el espacio que le sobra al lienzo lo llena una copia de esa misma foto, agrandada y difuminada, de fondo —el mismo recurso visual que usan las redes sociales grandes cuando alguien sube una foto que no calza con el marco del posteo. El resultado con una foto que ya viene en la proporción exacta del lienzo es idéntico a como se veía antes, sin ningún cambio visible; con una que no calza —como las capturas de sesión que motivaron el reclamo— ya no se pierde ni un pixel de contenido real. Se probó con imágenes de prueba armadas a propósito con las dos proporciones problemáticas —una panorámica como de videollamada, una angosta como de chat— renderizadas contra las plantillas reales, y se confirmó a simple vista que el contenido completo de las dos se ve entero, sin recortar. Queda pendiente, a propósito, el mismo arreglo en el generador de reels de video: usa el mismo principio de recorte pero implementado como un filtro de edición de video en vez de código de diseño, así que el arreglo ahí es más complejo y se dejó para otro momento —los reels no son algo que el sistema publique solo, se piden a mano.

Con el recorte resuelto de raíz, se armó una plantilla nueva de verdad: un collage de dos fotos, una al lado de la otra, en vez de una sola imagen de fondo detrás del texto. Cada una de las dos fotos respeta el mismo principio de no recortar nunca nada, aplicado por separado a cada mitad. Si solo hay una foto real disponible, el espacio de la segunda no queda vacío ni repite la misma imagen: se llena con el isotipo de la marca —el trazo hecho a mano— apenas visible sobre un fondo azul, el mismo recurso decorativo que ya se había aprobado antes para piezas sin foto. El resto del diseño —la tipografía, los colores del texto, la firma al pie— es deliberadamente el mismo que ya usa la plantilla de un solo post, para que se sienta parte de la misma familia visual y no como algo ajeno pegado al costado.

Lo más importante de esta plantilla nueva no es que exista, sino que ya está conectada de verdad al sistema que publica solo: cuando una pieza de feed tiene dos o más fotos reales todavía sin usar cargadas para esa misma dimensión, el sistema elige el collage automáticamente en vez del post de una sola foto, sin que nadie tenga que elegirlo a mano. Con una sola foto disponible —que sigue siendo el caso más común— el comportamiento no cambió nada. No se disparó ninguna corrida real del sistema contra la base de datos de producción para probar esta lógica —hacerlo hubiera arriesgado una publicación real solo para verificar un detalle, algo que este proyecto viene evitando con cuidado desde hace tiempo—; la selección entre las dos plantillas es una decisión simple y lineal, revisada a fondo por lectura de código en vez de por una corrida real.

El tercer pedido de Pablo —un editor propio dentro del sistema, para el caso de que algo no se pudiera resolver solo— quedó investigado pero sin construir todavía, a propósito. La razón real, no una excusa: el pipeline que publica posts y carruseles solo, de punta a punta, no tiene ningún momento donde una persona vea la foto real antes de que salga publicada —la foto recién se elige en el momento exacto de renderizar la imagen, dentro de la corrida automática que pasa cada quince minutos—. Meterle un editor ahí significaría agregar un paso de revisión humana que hoy no existe por diseño, una decisión de fondo sobre cómo funciona el sistema, no un simple agregado técnico. El único lugar del sistema que ya tiene ese momento de revisión humana real es la pantalla de "Publicar ahora" —pero esa pantalla es para historias, no para posts de feed, y hoy solo sabe armar la historia clásica, sin la variante de collage todavía. Se le devolvió la pregunta a Pablo en vez de construir a ciegas: si el editor que pide tiene más sentido ahí —en historias, que ya tienen ese momento de revisión— o si vale la pena sumarle ese mismo momento de revisión al pipeline de posts, que hoy no lo tiene por decisión de diseño tomada tiempo atrás.

## Parte 25 — El editor que faltaba, construido donde ya había un lugar seguro para ponerlo (2026-09-05)

Pablo respondió a la pregunta pendiente con un mandato amplio, no con una elección puntual entre las dos opciones planteadas: que se resolviera todo de forma autónoma y obsesiva, sumando por encima de lo que ya funciona sin sacar nada, sumando herramientas nuevas donde hicieran falta. Eso, leído junto con la pregunta que había quedado abierta, resolvía la ambigüedad solo: construir el editor en el lugar que ya tenía un momento de revisión humana real —la pantalla de "Publicar ahora"— y dejar completamente intacto el pipeline que publica posts y carruseles solo, sin agregarle ningún paso nuevo que hoy no tiene por diseño.

Se armó una plantilla de collage para historias, hermana de la que ya existía para los posts de feed del día anterior —dos fotos reales una al lado de la otra en vez de una sola imagen de fondo, cada una con el mismo principio de nunca recortar nada—, adaptada al formato vertical angosto de una historia en vez del formato de post. La tipografía que ya tenía la historia clásica resultó estar pensada, desde siempre, para un panel de texto blanco debajo de la foto —a diferencia de los posts, donde el texto va superpuesto sobre la imagen y necesita sus propios colores para un fondo claro—, así que esta vez no hizo falta ajustar ni un color: la plantilla nueva heredó la misma paleta tal cual. Se probó con las mismas dos fotos de prueba de aspecto problemático que el día anterior —una panorámica como de videollamada, una angosta como de chat— y las dos se ven completas, sin perder nada, con el mismo acabado prolijo.

Conectar esa plantilla nueva al flujo real de "Publicar ahora" pedía tocar una cadena completa de piezas, cada una con un cambio chico y ninguno tocando el camino que ya funciona: la función que le habla a la inteligencia artificial aprendió a aceptar dos fotos en el mismo pedido en vez de una sola, sin que el camino de una sola foto —que usa casi todo lo demás en el sistema— se diera cuenta del cambio. El generador del texto de las historias aprendió una opción nueva, apagada por default, que arma un solo texto pensado para dos fotos juntas en vez de un texto por foto —contándolas como dos momentos o dos ángulos de una misma situación real, no como si fueran cosas separadas—; si alguien pide esa opción pero en la carpeta de fotos pendientes solo hay una, el sistema no revienta, simplemente sigue el camino de una sola foto de siempre. El renderizador de historias aprendió a reconocer cuándo el texto que le llega viene marcado como collage y, solo en ese caso, usa la plantilla nueva con las dos fotos; en cualquier otro caso sigue exactamente igual que antes. Y la pantalla donde Pablo prepara y publica una historia bajo demanda sumó una sola casilla nueva —visible solo cuando hay de verdad dos o más fotos cargadas para esa dimensión— que le permite elegir armar el collage en vez de la historia de una sola foto, antes de tocar "Preparar".

Nada de lo que ya funcionaba se tocó para lograr esto. El camino de siempre —una foto, una historia— sigue exactamente igual sin que nadie tenga que hacer nada distinto; la opción nueva solo aparece cuando hay material real para ella, y si algo falla al pedirla, el sistema cae solo al comportamiento de toda la vida en vez de romperse. No se disparó una corrida real de "Publicar ahora" contra la base de datos y el servicio de redes reales para probar esta cadena completa —hubiera significado arriesgar una publicación real solo para confirmar un detalle de flujo, algo que este proyecto evita con cuidado desde hace tiempo—; cada pieza de la cadena se revisó a mano, y la parte que sí se podía probar sin ese riesgo —cómo se ve la plantilla nueva con fotos reales de prueba— se probó de verdad, con los mismos ojos que se usó para confirmar la plantilla de posts el día anterior.

Quedaba un solo cabo suelto del arreglo de recorte del día anterior: el mismo problema existía también en el generador de videos cortos, pero ahí vivía adentro de un filtro de composición de video, no de una hoja de estilos, y ese arreglo se había dejado sin tocar el día anterior por una razón concreta y honesta —no había manera de instalar y probar un cambio de filtro de video en la máquina donde se estaba trabajando, y tocar a ciegas un filtro de video de un sistema que compone piezas reales para publicar no es un riesgo que valga la pena correr—. Antes de seguir dejándolo pendiente, se instaló la herramienta de composición de video que hacía falta, un paso de preparación deliberado para poder probar de verdad en vez de adivinar.

Con la herramienta ya instalada, se reprodujo primero el problema tal cual estaba, a propósito: una foto de prueba panorámica, con texto que ocupaba todo el ancho, efectivamente perdía la mitad de las letras en los bordes al pasar por el filtro viejo — la misma clase de daño real que las capturas de sesión venían sufriendo en las otras piezas. Confirmado el problema con evidencia y no solo con la lectura del código, se aplicó el mismo principio ya probado el día anterior, esta vez expresado como un filtro de video en dos ramas: una que arma el fondo exactamente como antes —recortando sin culpa, porque ahí no importa perder los bordes, es puro relleno visual— pero ahora difuminada y oscurecida; y otra que toma la foto real y la achica sin recortar nada, superpuesta justo en el centro de ese fondo. El movimiento de cámara lento que ya tenía cada video se seguía aplicando exactamente igual, ahora sobre ese resultado compuesto en vez de sobre el recorte a la fuerza de antes.

Se probó cada capa por separado antes de dar el arreglo por bueno: primero el fondo solo, para confirmar que la foto de prueba panorámica se veía completa después de todo el proceso, incluido el movimiento de cámara; después la cadena entera, con los mismos números reales que usa el sistema en producción —nueve segundos de duración, la misma resolución vertical, el mismo códec de video y de audio—, generando un archivo real y verificando con una herramienta de inspección de video que la duración, la resolución y los dos códecs eran exactamente los esperados. Por último, se generó una capa de marca transparente de la misma manera exacta en que la genera el sistema real —no una simulación armada a mano— para confirmar que la superposición de esa capa de texto sobre el fondo ya arreglado seguía funcionando con la transparencia intacta, y así fue: la foto de prueba se veía completa, con el texto de marca correctamente encimado arriba, sin que la transparencia se perdiera en el camino. Con eso, el pendiente que había quedado abierto el día anterior se cerró de verdad, probado, no solo prometido.

## Parte 26 — Un cron que dice cada quince minutos y en realidad corre cada pocas horas (2026-09-05)

Revisando el historial real de corridas automáticas para entender por qué una publicación puntual había fallado, apareció algo mucho más grande que el fallo puntual en sí. El fallo puntual era menor y sin consecuencia real: un intento de publicar una pieza real se cortó por demora al hablar con el servicio de redes, quedó registrado el error, y el propio diseño del sistema —ya pensado para esto— deja la pieza en el mismo estado de "agendada" para que la corrida siguiente la reintente sola, con un resguardo real contra publicarla dos veces si en realidad sí había salido bien la primera vez. Nada roto ahí, el sistema hizo exactamente lo que estaba pensado que hiciera.

Lo grande apareció al mirar cuánto tiempo pasaba entre una corrida automática y la siguiente. El archivo que define ese cron dice, en su propia configuración, que tiene que correr cada quince minutos. La realidad medida contra el historial de decenas de corridas reales fue otra: entre una corrida y la siguiente pasaban, la mayoría de las veces, entre casi dos horas y más de cinco horas —nunca los quince minutos que el archivo pide—. Y no era un problema de esta pieza puntual: se midieron también los otros dos cron del sistema, uno que dice correr cada tres horas y otro que dice correr cada seis, y los dos mostraban el mismo patrón de fondo —intervalos reales bastante más largos e irregulares que lo que sus propios archivos piden—, aunque el más golpeado con diferencia era, justamente, el que pide el intervalo más corto de los tres.

La causa real no está en ningún archivo de este proyecto — es un comportamiento conocido y documentado por la propia plataforma que hospeda estos cron automáticos: bajo ciertas condiciones de uso general de la plataforma, los disparadores programados con intervalos cortos no tienen ninguna garantía real de cumplirse al pie de la letra, y en la práctica pueden terminar disparando muchísimo más espaciados que lo pedido, sobre todo en cuentas sin un nivel de servicio pago con capacidad reservada. No hay ningún error de configuración que corregir en el código de este repositorio — el archivo está escrito correctamente, pidiendo exactamente lo que se quiere pedir; es la entrega real de ese pedido la que no es la que promete el propio nombre del intervalo.

El impacto real, sin exagerarlo ni minimizarlo: el sistema sigue publicando solo, sin que nadie tenga que aprobar nada, y ninguna pieza se pierde por esto. Lo que cambia es el tiempo real entre que una pieza queda lista para salir y el momento en que efectivamente sale — puede ser un rato mucho más largo que los quince minutos que la documentación del proyecto viene repitiendo en varios lugares como si fuera un hecho garantizado. Dejarlo escrito así, sin esta aclaración, sería seguir prometiendo una precisión que la plataforma real no entrega.

Existe una forma conocida de forzar una cadencia más real —un servicio externo que le vaya tocando el timbre al repositorio desde afuera en vez de confiar en el reloj interno de la plataforma—, pero ponerla en marcha significaría darle a un servicio de un tercero un permiso real para disparar acciones sobre este repositorio: exactamente el tipo de decisión sobre a quién se le da acceso que no le corresponde tomar a esta sesión en solitario. Quedó anotada como una opción real y concreta, no como algo resuelto ni como algo descartado — a la espera de que Pablo decida si vale la pena esa exposición nueva a cambio de la cadencia más ajustada.

## Parte 27 — La pestaña elegida que el sistema pisaba sin avisar (2026-09-07)

Pablo mandó un video de su pantalla mostrando algo que no funcionaba: en la sección de subir material, eligió la pestaña de una de las dimensiones —"Sociales"—, subió una captura de pantalla, y el botón para preparar una historia de esa dimensión seguía apagado, con el cartel de "primero subí una foto". Tan claro era que no pasaba nada que terminó subiendo la misma captura tres veces.

Mirando el repositorio real, la causa apareció enseguida: las tres copias de la captura —idénticas byte por byte— habían quedado guardadas en la carpeta de otra dimensión, "Empresarial", no en la de "Sociales" que él había elegido. El motivo: cuando alguien sube una foto, el sistema la mira con inteligencia artificial y propone en qué dimensión encaja mejor. Esa propuesta, en el código, no se ofrecía como sugerencia —se imponía—: sobreescribía en silencio la dimensión que la persona ya había elegido con la pestaña. El único rastro visible de ese cambio era un menú desplegable que decía "Empresarial (sugerida)", que se lee como una etiqueta informativa y no como un control que te acaba de cambiar la elección. Peor todavía, un texto de ayuda arriba afirmaba que la foto se guardaba en la carpeta de la dimensión elegida en la pestaña —lo cual era mentira, porque el destino real lo decidía ese menú por-foto—.

El arreglo devuelve el control a quien sube la foto: la dimensión de la pestaña elegida manda, siempre, como opción por defecto. La inteligencia artificial sigue mirando cada foto y, si cree que va en otra dimensión distinta a la elegida, ahora lo dice de forma explícita —un enlace debajo del menú que dice "el sistema cree que es de tal dimensión, usar esa"— y aplicarlo es un clic deliberado, nunca automático. El texto de ayuda ahora dice con todas las letras dónde se guarda cada foto y que la sugerencia del sistema es solo eso, una sugerencia.

De las tres copias que habían quedado mal guardadas, una se movió a la carpeta que Pablo quería y las otras dos —duplicados exactos, sin ningún dato propio que se pierda— se sacaron, para que el proceso automático que arma una historia por día no terminara generando dos historias idénticas a partir de la misma captura mal clasificada. No se pudo probar el flujo completo ya con la sesión iniciada —eso exige la contraseña real de Pablo—, así que la verificación quedó en los chequeos de siempre: tipos, estilo, las pruebas automáticas y la compilación, todo en verde.

## Parte 28 — La primera vez que se pudo entrar de verdad al sistema y mirarlo funcionar (2026-09-07)

Pablo instaló una herramienta que le da a la sesión de trabajo acceso a su propio navegador, con su sesión ya iniciada. Por primera vez en meses de trabajo sobre este sistema, se pudo entrar de verdad a la aplicación —no quedarse en la pantalla de acceso— y recorrerla pantalla por pantalla, mirando la consola del navegador, el tráfico de red y el estado real de cada cosa. La conclusión de fondo, primero: la aplicación está sana. Todo funciona. Lo que apareció fueron detalles, algunos con consecuencia real, ninguno grave.

El más importante: el consejo del día del copiloto venía diciendo, corrida tras corrida, "arreglá los cinco fallos del pipeline de publicación". Al mirar el registro real de corridas, esos "fallos" eran dos intentos de publicar una pieza que el servicio de redes ya tenía en cola. Cuando ese servicio detecta que el contenido exacto ya está agendado, publicándose o publicado, responde con un rechazo —pero en este caso ese rechazo no venía con el identificador del post ya existente, así que el mecanismo de reconciliación que ya existía nunca podía dispararse—. Resultado: la propuesta quedaba marcada como pendiente de publicar, el proceso automático la reintentaba en cada corrida, cada reintento chocaba con el mismo rechazo, y cada rechazo quedaba registrado como un fallo que el copiloto después le echaba en cara a Pablo. Se arregló para que ese rechazo específico —"este contenido ya está en cola o publicado"— se trate como lo que es: la pieza ya está en manos del servicio de redes, no es un fallo. La propuesta se cierra como publicada, el proceso deja de reintentarla, y el registro deja de ensuciarse.

El segundo: la consola del navegador venía advirtiendo, varias veces por carga de página, que había más de una instancia del cliente de autenticación en el mismo navegador —lo cual, sin ser un error, puede producir comportamiento indefinido cuando dos instancias compiten por refrescar el mismo token de sesión guardado—. La causa: dos archivos distintos creaban cada uno su propio cliente de conexión a la base de datos, sobre la misma clave de almacenamiento. Se unificó en uno solo. Confirmado en vivo: la advertencia desapareció.

El resto fueron mejoras de presentación, todas confirmadas mirándolas en la pantalla real: el Manual de Marca mostraba el tipo técnico crudo de cada archivo —una cadena larga e ilegible— en vez de un "Word" o "PDF" simple; el Monitor, cuando la imagen de una pieza vieja ya se había limpiado del repositorio, mostraba un recuadro gris que parecía cargando para siempre en vez de un mensaje claro de que el archivo ya no está; y la grilla de "fotos pendientes" en la sección de subir material listaba un archivo técnico invisible —un marcador de carpeta vacía— como si fuera una foto rota, poniendo un "no disponible" al lado de las fotos reales.

Y una herramienta nueva de verdad: los diecinueve documentos que ya estaban en el Manual de Marca desde antes de que existiera la clasificación automática habían quedado todos sin clasificar, porque la clasificación solo corría al subir un documento nuevo. Con todo en un solo montón, la vista organizada por categoría no servía para nada. Se agregó una acción que clasifica solo el tipo de un documento ya procesado —usando el texto que ya está guardado, sin volver a leer el archivo ni regenerar los índices de búsqueda— y un botón "Clasificar automáticamente" en el encabezado del grupo "sin clasificar". Se probó en vivo apretándolo: clasificó los diecinueve, todos bien —nueve de manual y criterio, ocho de perfiles de cliente, uno de tono de voz, uno de otros— y el grupo "sin clasificar" desapareció.

Durante todo el recorrido por el navegador no se tocó ningún botón de publicar, despublicar, borrar ni forzar una aprobación —solo navegación, inspección, y la clasificación de documentos, que es reversible: cambia una etiqueta que Pablo puede corregir a mano cuando quiera—.

## Parte 29 — El chequeo de tipos que no chequeaba nada, y una cuenta que se desconectó sola (2026-09-07)

El mismo día, más tarde, siguió el mandato de arreglar todo sin parar. El primer hallazgo fue incómodo: el arreglo anterior de la grilla de fotos había roto la pantalla de subir material en producción. El filtro nuevo usaba una constante que estaba definida en otro archivo, y como es un lenguaje que avisa de ese tipo de error antes de ejecutar, debería haberlo cazado el chequeo automático que corre en cada cambio. No lo cazó. Y ahí apareció el problema de fondo.

El chequeo de tipos del sistema de integración continua corría un comando que, contra la forma en que está armada la configuración de este proyecto, no revisa absolutamente nada —termina bien sin haber mirado un solo archivo—. Llevaba así probablemente desde siempre. Lo primero fue arreglar la pantalla rota y publicarla enseguida. Después, el fondo: se cambió el comando por el que sí revisa de verdad. Al correrlo por primera vez con todo el rigor, aparecieron alrededor de ciento diez errores de tipo que nadie había visto nunca.

La mayor parte no eran errores de lógica sino consecuencia de algo más viejo: el archivo que describe la forma de cada tabla de la base de datos para el lenguaje era un molde vacío que dejó la herramienta con la que se armó el primer esqueleto del proyecto. Nunca se había llenado. Así que el sistema, al hablar con la base, no sabía qué campos tiene ninguna tabla. Se regeneró ese archivo desde la base real: veinte tablas, con todos sus campos y tipos. Solo eso bajó los errores de ciento diez a cuarenta y dos.

Los cuarenta y dos que quedaron eran reales y se arreglaron uno por uno. Varios eran del mismo tipo: cálculos sobre números de métricas que podían venir vacíos y dar un resultado que no es un número; comprobaciones de "esto no puede estar vacío" que el lenguaje perdía de vista dentro de funciones anidadas; una prueba automática que llamaba a una función con dos argumentos cuando hace tiempo pasó a necesitar tres. Ninguno rompía la aplicación —funciona, se verificó pantalla por pantalla— pero eran deuda real que el chequeo trucho venía escondiendo. Con todo arreglado, el chequeo de verdad quedó en cero.

Mirando la aplicación de vuelta apareció un detalle de presentación: al abrir el detalle de una propuesta, el modal tenía una barra de desplazamiento horizontal y texto cortado a la derecha. La vista previa de cómo va a quedar la pieza se muestra en un recuadro que, por debajo, mide el tamaño real de la imagen final —mil ochenta puntos de ancho— y después se achica visualmente; pero ese tamaño real seguía empujando el ancho de todo el modal. Se le fijó al recuadro el ancho que ocupa de verdad después de achicarse.

Y el hallazgo que necesita a Pablo: la corrida de la historia diaria de ese día falló. La causa, en el registro: el servicio de redes respondió que la cuenta de Instagram de Mejora Continua está desconectada, que el permiso de acceso venció y hay que volver a autorizarlo. Esto pasa cada dos meses más o menos con este tipo de permisos. El día anterior había publicado bien; en algún momento entre esa noche y la tarde siguiente se cayó. Mientras no se reconecte, no publica nada a Instagram: ni la historia diaria, ni los carruseles que el sistema tenía agendados para esa noche y la siguiente, ni lo que salga del piloto automático, ni "publicar ahora". Facebook es una cuenta aparte y puede seguir andando. Lo que tiene que hacer Pablo es entrar al panel del servicio de redes y volver a conectar la cuenta de Instagram; las piezas que quedaron trabadas se publican solas en la corrida siguiente. Del lado del código se agregó que ese error puntual —cuenta desconectada— se reconozca y se explique con todas las letras en el registro y en el consejo del día, en vez de aparecer como una falla genérica sin pista de qué hacer.

## Parte 30 — Bloquear que las versiones cambien solas, y avisar bien cuando algo externo se rompe (2026-09-07)

Siguiendo el mismo día, más trabajo de fondo. Las once funciones que corren del lado del servidor traían la librería para hablar con la base de datos pidiéndola por "versión dos, la que sea" —cualquier versión menor que estuviera disponible el día del despliegue—. Dos despliegues con meses de diferencia podían terminar corriendo con librerías distintas sin que nadie lo decidiera. Se fijó la versión exacta que está corriendo hoy: mismo comportamiento, pero ahora explícito y reproducible. Se probó de punta a punta disparando dos de las funciones después de desplegar —una que analiza métricas y otra que sincroniza las conversaciones— y las dos corrieron bien, la de conversaciones hizo cincuenta y ocho escrituras reales a la base sin problema. De paso se sacó un pedazo de cálculo que se hacía y no se usaba en ningún lado.

Y se mejoró cómo el sistema avisa cuando el problema no es suyo. Cuando el servicio de redes responde que la cuenta de Instagram está desconectada, eso no es una falla del proceso de publicación: el proceso hizo bien en no intentar publicar a una cuenta caída. Antes eso quedaba registrado como un error más, indistinguible de un fallo real, y cada reintento sumaba otro. Ahora se registra aparte, con una etiqueta clara de "cuenta desconectada", la pieza queda en cola para publicarse sola cuando la cuenta vuelva, y el consejo del día del copiloto —que lee ese registro— pone una línea al principio, en rojo, diciendo "reconectá Instagram en el panel del servicio de redes". Para la historia diaria se dejó que el proceso termine marcado como fallido igual, para que el indicador quede rojo y visible, pero con el mensaje correcto en vez de uno genérico.

Sobre la política de seguridad de contenido del navegador —qué orígenes puede cargar la aplicación— se hizo una revisión detallada pero no se tocó nada. Hay dos permisos amplios que en principio convendría cerrar, pero uno de ellos lo necesita una librería que usa la sección de subir el manual de marca comprimido, y confirmar que cerrarlo no rompe nada requiere probar la aplicación abierta y con sesión iniciada, cosa que en esta sesión no se pudo porque la conexión con el navegador se cayó. Quedó documentado con el detalle exacto de qué permiso es para qué, para cerrarlo cuando se pueda verificar en vivo. Cerrar a ciegas algo que podría romper una función real, en una herramienta de un solo dueño y detrás de acceso con contraseña, no valía el riesgo por la mejora marginal.

## Parte 31 — Un texto que nunca se recortaba, dos publicaciones que podían no chequearse entre sí, y una columna que no sabía de dos dimensiones nuevas (2026-09-08)

Sesión nueva, mismo mandato de siempre repetido: investigar actualizaciones posibles, revisar de punta a punta más de una vez, arreglar todo lo que sea de verdad un problema, no parar salvo que haga falta que decida Pablo. La cuenta de Instagram seguía desconectada del servicio de redes —eso no se tocó, sigue esperando que Pablo la reconecte del otro lado.

Antes de tocar nada se revisó qué se podía actualizar y qué vulnerabilidades reales había. La única vulnerabilidad de seguridad que aparece hoy es la misma de siempre —una librería del servidor de desarrollo local, que nunca viaja a producción— y ya se había evaluado antes que no vale la pena forzar la actualización grande que la resuelve, porque el riesgo real es nulo. Se actualizaron sin drama las piezas menores que ya venían dentro del rango que el proyecto permite. Se investigaron las piezas grandes disponibles —una versión mucho más nueva de la librería de interfaz, del compilador de tipos, de la herramienta de construcción— y ninguna resuelve un problema de seguridad real ni justifica el riesgo de romper algo sin poder probarlo con la aplicación abierta y logueada.

Se agregó algo nuevo, pensado directamente a partir del problema de la cuenta desconectada: hasta ahora, el sistema solo se enteraba de que una cuenta se había caído cuando intentaba publicar de verdad y fallaba —lo que podía tardar horas, o no pasar en todo el día si no había nada agendado—. Ahora, la misma tarea que cada seis horas junta las métricas de lo publicado también le pregunta al servicio de redes por el estado de las tres cuentas conectadas, y si alguna aparece apagada, lo anota enseguida en el mismo registro que ya lee el aviso rojo del panel principal. Es un aviso de mejor esfuerzo, no una certeza absoluta —no hay garantía documentada de que esos sean los únicos campos que el servicio usa para señalar una desconexión—, pero si algún día deja de encontrar la señal, simplemente no avisa nada nuevo; nunca puede hacer fallar la tarea ni inventar un aviso falso.

De paso se puso al día la versión de Node que declaran los catorce procesos automáticos que corren en la nube de GitHub —venían pidiendo una versión ya retirada, que GitHub venía reemplazando por detrás en silencio y avisando con una advertencia en cada corrida—. Ahora piden la versión real que efectivamente usan.

Se le pidió a un ayudante que releyera con ojo crítico tres partes del sistema que ninguna sesión anterior había mirado línea por línea con esa lupa: las veinticinco migraciones de la base de datos completas, en orden; las cuatro plantillas de diseño que arman visualmente cada pieza; y la consistencia entre los veintiún procesos automáticos. Encontró cinco cosas reales, sin inventar ninguna.

La más seria: el texto grande del título de una pieza —el gancho, lo primero que se lee— nunca tuvo ningún límite de largo, a diferencia del texto de abajo, que sí se recorta desde hace tiempo. Ni la instrucción que recibe el redactor de IA le pone un tope de palabras a ese campo puntual, ni el código que arma la imagen final lo revisa. Un título largo podía superponerse con la etiqueta de la dimensión o quedar cortado en seco en la captura final —y esto pasa en piezas que se aprueban y se publican solas, sin que nadie las mire antes—. Se agregó el mismo tipo de recorte inteligente que ya usa el resto del texto —corta en una coma si hay una cerca, para no cortar a mitad de una idea— tanto para los posts y carruseles como para las historias, que no tenían ningún recorte de ningún tipo en ese archivo. Se probó armando la imagen real con un título adversario de más de veinte palabras, antes y después del arreglo, y se vio la diferencia con los propios ojos: antes se hubiera desbordado, después entra perfecto.

La segunda: el proceso que construye y publica el sitio público no dependía en absoluto de que el chequeo de calidad hubiera pasado. Los dos corren en paralelo sobre el mismo cambio, sin que uno espere al otro —así que un error real de tipos podía dejar el chequeo en rojo y aun así el sitio se publicaba igual, con el error adentro. Es exactamente el mecanismo que ya había causado un problema real hace un día, cuando una pantalla se rompió en producción pese a que en ese momento el chequeo tampoco revisaba nada. Ahora el proceso de publicación corre los mismos tres controles antes de construir nada: si alguno falla, no publica.

La tercera: el proceso que aplica cambios a la estructura de la base de datos —el que casi nunca se usa porque los cambios de estructura son delicados— todavía tenía un paso que ya se sabía roto desde hace semanas, documentado en otro proceso hermano que sí se había arreglado en su momento. Ese paso nunca se corrigió acá. Se aplicó el mismo arreglo ya probado, y de paso se le agregó una opción para simular el cambio sin aplicarlo de verdad, útil para cualquier cambio de estructura futuro. Se probó en producción real, primero en modo simulado —confirmó que el arreglo funciona, sin tocar nada— y después de verdad, aplicando un cambio pendiente real.

Ese cambio real era la cuarta cosa encontrada: la columna que guarda a qué dimensión de marca pertenece cada pieza tenía una restricción vieja que solo permitía cuatro de las seis dimensiones reales que el negocio usa desde hace semanas —faltaban las dos que se agregaron cuando se sumó la categoría de contenido social y la de profesionalización—. Hoy nadie escribe todavía ese campo, así que no rompía nada en el momento, pero era una trampa esperando: el día que se conecte la clasificación automática a esa columna, iba a fallar con un error crudo apenas apareciera una de esas dos dimensiones. Se corrigió la restricción para que acepte las seis, y de paso se agregó un atajo de búsqueda para la consulta que arma el aviso de "cuenta desconectada", que hoy revisa la tabla entera fila por fila sin ningún acceso directo.

La quinta: el proceso que arma un video corto de una sola foto no compartía ningún tipo de turno con los otros tres procesos que suben o publican contenido, pese a leer las mismas carpetas de fotos pendientes. Con el proceso de publicar posts corriendo de forma bastante irregular —a veces cada un par de horas, a veces cada varias, por cómo GitHub reparte los procesos programados bajo carga—, un video disparado a mano podía coincidir con una publicación en curso y los dos terminar usando la misma foto para dos piezas distintas. Se unificaron los cuatro procesos bajo un mismo turno: ahora nunca corren al mismo tiempo, cuestan como mucho unos minutos de espera si se cruzan.

De paso se pusieron al día seis de las herramientas oficiales de GitHub que usan los procesos automáticos —estaban tres versiones mayores atrás en varias—, después de revisar puntualmente qué cambia en cada salto y confirmar que ninguno de los cambios reales le pega a como este proyecto las usa.

Todo se probó de verdad, no solo se dio por bueno: cada cambio con los chequeos automáticos completos corriendo verde, y las corridas reales en la nube —el proceso de publicación, el de aplicar la migración, los dos rediseñados— confirmadas una por una mirando el resultado real, no asumiendo que iba a andar.

Antes de dar por cerrado todo lo anterior, se le pidió a un segundo ayudante —sin ninguna relación con el primero, sin conocer nada de lo que se había hecho salvo lo que se le contó— que revisara con desconfianza los nueve cambios recién hechos, sin darle crédito a ninguna descripción propia de lo que se había arreglado. Encontró dos cosas reales, ninguna grave por sí sola pero ambas dignas de corregir enseguida.

La primera: la nueva verificación de si una cuenta de redes se había caído —la que se agregó ese mismo día para no depender de que fallara una publicación real— tenía una falla de diseño: vivía en un lugar del código al que solo se llegaba si además había alguna pieza de feed ya publicada. Y como la base de datos está vacía desde hace días y el sistema depende de que el piloto automático apruebe algo —y lo aprueba más o menos la mitad de las veces—, en la práctica esa condición casi nunca se cumplía. La verificación que se había armado para no depender de nada más terminaba dependiendo, sin querer, de que hubiera contenido publicado. Se movió más arriba, antes de esa condición.

La segunda: el arreglo del texto que no se recortaba se había aplicado a los posts de feed y a las historias, pero quedó un tercer lugar sin tocar —el que arma el video corto de una sola foto—, que tenía exactamente el mismo problema y nadie lo había mirado. Se le aplicó el mismo arreglo, con la misma clase de límite, y se probó armando el video de verdad con un título largo antes y después.

Y de paso, revisando ese mismo arreglo, apareció que el límite elegido para el título de las historias nunca se había puesto a prueba contra el tamaño de letra más grande que usa esa plantilla —el que se usa cuando no hay foto—, solo se había probado contra el de los posts, que es bastante más chico. Al probarlo de verdad, ese límite llenaba casi toda la pantalla y quedaba pegado al pie de la imagen. Se bajó a un número más chico, el mismo que ya le pide el texto de instrucciones al redactor de IA, sin margen de más, y se confirmó con los propios ojos que ahora entra con aire de sobra, con foto y sin foto.

## Parte 32 — Otra vuelta completa, el mismo día por la tarde (2026-09-08)

Se pidió seguir buscando, esta vez arrancando de cero en vez de continuar la lista de la mañana. Antes de arrancar, un dato interesante apareció solo: la verificación de cuenta desconectada que se había agregado esa misma mañana ya había corrido de verdad en producción y devolvió que ninguna de las tres cuentas figura caída —una señal posible, no una confirmación, de que la cuenta de Instagram ya se reconectó del otro lado.

Se repartió el trabajo entre dos ayudantes en paralelo —uno revisando toda la parte visual de la aplicación de punta a punta, otro revisando todas las funciones que corren del lado del servidor— mientras se revisaba a mano lo que quedaba: los procesos sueltos y las plantillas de diseño que ninguno de los dos iba a mirar. Entre los tres aparecieron seis problemas reales, todos corregidos y confirmados en producción real antes de seguir.

El primero, de nuevo en las herramientas de gestión manual: la de posts tenía el mismo defecto de fondo que ya se había visto una vez —un corte abrupto del programa en varios puntos donde algo salía mal, que saltaba por completo el paso final donde se deja constancia del error—. La de historias tenía el mismo problema, pero peor: nunca había tenido ese registro final en absoluto, desde siempre. Las dos se corrigieron con el mismo criterio: que cualquier fallo, sea cual sea, termine pasando por un único lugar que deja la constancia.

El segundo fue el más serio de toda la tanda. El paso que guarda una pieza de contenido recién aprobada nunca revisaba si esa escritura a la base de datos realmente había funcionado. Y existe, desde hace semanas, una regla real en la base que impide que dos piezas se agenden para el mismo tema, el mismo día y el mismo formato —puesta ahí a propósito para evitar duplicados—. Si dos aprobaciones llegan casi al mismo tiempo y las dos calculan el mismo lugar antes de que cualquiera de las dos termine de guardar, la segunda choca contra esa regla y no se guarda —pero el sistema seguía diciéndole a quien mira la pantalla que esa pieza ya estaba agendada y saldría sola, cuando en realidad no existía en ningún lado. Contenido real, ya aprobado por el chequeo de calidad interno, podía desaparecer sin dejar rastro. Se corrigió para que, si eso pasa, se vuelva a calcular el lugar y se reintente una vez más antes de avisar de un error real si sigue sin poder guardarse.

De la misma familia: los tres lugares donde se actualiza el estado final de una conversación con los agentes de IA tampoco revisaban si esa actualización había salido bien. Ya existía una protección pensada justo para que una conversación fallida nunca quedara marcada como "en curso" para siempre —pero esa protección solo actúa si algo revienta con una excepción, no si la base simplemente responde que no pudo guardar. Ahora, si eso pasa, al menos queda anotado en el registro de errores en vez de perderse en silencio.

El tercero tocó la bandeja de conversaciones con clientes reales. Cuando alguien responde un comentario o un mensaje desde el sistema, ese envío es real e irreversible —ya le llegó a la persona—. Pero después de mandarlo, el sistema guarda dos registros propios para saber que ya se contestó, y si esa parte fallaba, la conversación seguía apareciendo como "sin responder" para siempre, aunque ya se hubiera contestado de verdad. El riesgo real: alguien viendo esa bandeja podría mandar una segunda respuesta real pensando que la primera nunca salió. Ahora, si ese registro falla, igual se avisa que el mensaje salió bien —porque salió bien, no hay que reportarlo como error—, pero queda una marca clara de que puede seguir viéndose como pendiente, y quien lo usa ve un aviso distinto en pantalla en ese caso puntual.

El cuarto fue en la pantalla de configuración de los tres agentes. El campo donde se guarda cada instrucción se recargaba solo cada vez que la lista de configuración cambiaba de referencia en memoria —cosa que pasa, entre otras veces, justo después de guardar—. Si alguien guardaba el texto de un agente mientras seguía escribiendo el de otro, ese segundo cambio se perdía en el instante en que el primero terminaba de guardarse, sin ningún aviso. Se corrigió para que esa recarga automática pase una sola vez, la primera, y nunca más pise lo que se esté escribiendo después.

El quinto fue en la tarjeta de "publicar una story ahora". Cambiar de pestaña de categoría mientras se estaba preparando una pieza no afectaba lo que realmente se iba a publicar —eso siempre estaba bien atado a la pieza real, no a la pestaña que se ve en pantalla—, pero el texto que se mostraba arriba sí podía confundir: podía decir una categoría distinta a la de la pieza que en realidad se estaba armando o mostrando. Se corrigió para que ese texto muestre siempre la categoría real de la pieza en curso. Al revisar ese mismo arreglo por segunda vez, apareció que quedaba un instante sin cubrir —el rato entre apretar "Preparar" y que llegue la primera confirmación real—, donde el texto todavía podía volver a mostrar la pestaña equivocada. Se cerró guardando la categoría real desde el mismo momento en que se aprieta el botón, no cuando llega la respuesta.

Una nota menor, cerrada de paso: la pantalla de monitoreo tenía dos esperas encadenadas de casi un minuto y medio en total, sin ninguna forma de cancelarlas si la persona se iba de esa pantalla antes de que terminaran —no rompía nada, pero podía intentar actualizar una pantalla que ya no estaba. Se agregó una guarda simple que también evita, de paso, un pedido de red que ya no hacía falta.

Todo esto se revisó dos veces por partes separadas: primero se armaron los seis arreglos, después se le pidió a un tercer ayudante, sin ningún contexto previo, que audite ese mismo trabajo con desconfianza real, sin creerle a ninguna descripción propia de lo ya hecho. Confirmó que cinco de los seis quedaron bien cerrados, y encontró exactamente la falla real del quinto punto —la ventana sin cubrir en la tarjeta de publicar ahora— que se corrigió enseguida, tal como se cuenta arriba.

Al rato llegó un archivo comprimido de otra herramienta de diseño con inteligencia artificial: tres documentos fechados dos días antes, con una auditoría independiente del sistema, un puñado de ideas nuevas para el diseño de las piezas de contenido, y una crítica visual tanto de lo que ya se está publicando como del panel de control, con bocetos concretos de cómo quedarían los cambios propuestos. Se leyeron los tres de punta a punta y se los contrastó contra el código real, mismo criterio ya usado la vez anterior que trajo un paquete parecido: lo que resulta ser técnico y de bajo riesgo se ejecuta directo; lo que es una decisión de contenido o de marca se le deja planteado, no se decide solo.

La auditoría independiente se tomó el trabajo real de verificar, no solo repetir lo que ya decía la bitácora del proyecto —confirmó, leyendo el código de verdad, que el chequeo de tipos corre en serio, que el archivo de tipos de la base tiene las tablas reales, que la librería de rutas ya está en la versión mayor nueva, que el manejo de la cuenta desconectada existe tal cual está documentado— y no encontró ninguna discrepancia. De las tres cosas que trajo como sugerencia nueva, dos ya estaban evaluadas y aceptadas de antes (la política de seguridad del navegador con dos permisos amplios pendientes de endurecer cuando se pueda probar en vivo, y dos huecos de seguridad menores en la librería de rutas, ambos de bajo riesgo real para este sistema en particular). La tercera sí era genuinamente nueva: avisar antes de que venza el permiso de acceso a redes sociales, no solo después de que ya se cortó.

Esa sugerencia se implementó. El servicio de redes no informa una fecha de vencimiento exacta del permiso, así que no se puede armar una cuenta regresiva precisa —en cambio, se usa el propio registro de corridas del sistema como punto de referencia real: cada vez que se detecta que una cuenta pasó de estar caída a estar conectada de nuevo, queda anotado el momento exacto de esa reconexión. Si después pasan cincuenta días o más desde esa fecha sin que la cuenta se haya vuelto a caer en el medio, aparece un aviso —uno solo, no uno por cada revisión— tanto en el panel principal como en el consejo del día, con un tono claramente distinto al de una caída real: en amarillo, no en rojo, porque todavía funciona.

La crítica visual hizo algo poco común y valioso: antes de proponer cualquier cambio, cuestionó su propia primera impresión. Descartó como problema real la densidad del panel principal —la persona que lo usa es una sola, lo mira todos los días, y prefiere poder revisar cada número antes que una pantalla más simple pero con menos control— y se quedó con dos hallazgos genuinos. Uno de los dos no necesitaba ninguna decisión de contenido: los ocho números de rendimiento del panel pesaban todos exactamente igual en pantalla, sin importar si tenían un dato real grande o si todavía no había ninguna medición. Se le dio jerarquía real, sin cambiar ningún cálculo ni sacar ningún dato: el número que ya estaba marcado como el estándar de comparación pasa a mostrarse grande y destacado, dos números de contexto inmediato quedan siempre visibles al lado, y el resto queda plegado a un clic, visible igual pero sin pesar tanto de entrada. El aviso de cuenta desconectada, de paso, quedó con más color y sin el ícono de emergencia, con el mensaje de qué hacer más directo.

El otro hallazgo genuino —que ninguna de las piezas publicadas recientemente usa una foto real, pese a que el manual de marca invirtió en un estándar fotográfico completo— se dejó sin tocar a propósito: el propio documento externo lo plantea como una pregunta, no como una propuesta lista, porque no se sabe si "nunca usar foto" es una decisión de marca ya tomada o simplemente un hueco real que nadie notó. Tampoco se tocaron las nueve ideas nuevas de diseño de pieza que traía el segundo documento —todas dentro de la paleta y tipografía de marca, sin inventar nada nuevo, pero presentadas como un menú para elegir, no como un plan ya decidido— ni la propuesta de extender el mismo criterio de jerarquía visual a otras pantallas del panel, que el propio documento ofrece como un "si querés" abierto. Las tres quedan documentadas, esperando que la persona dueña del proyecto decida.

Sobre las dos preguntas planteadas, la respuesta llegó en la misma sesión. Sobre la foto: falta contenido, no es una decisión tomada —activar la variante con foto. Investigando antes de tocar nada, apareció que el sistema de renderizado ya sabía usar una foto cuando la tenía disponible; el problema estaba un paso antes, en la parte que elige de qué categoría va a salir el próximo post. Esa elección corre en el servidor, sin ninguna forma de mirar qué fotos hay cargadas en cada carpeta —así que podía tocarle el turno a una categoría vacía mientras otra tenía fotos esperando, y la pieza salía sin imagen no porque no hubiera ninguna, sino porque le tocó a la carpeta equivocada. Se agregó una consulta real, en paralelo para cada categoría candidata, que pregunta si ya hay una foto cargada antes de decidir —sigue respetando el mismo criterio de turnos de siempre, pero ahora, entre las categorías candidatas, prefiere la que ya tiene una foto real esperando. Si ninguna tiene, o si esa consulta falla por cualquier motivo, se comporta exactamente igual que antes —nunca se arriesga a que esto rompa el armado automático de una pieza.

Sobre cuál de los nueve moldes nuevos desarrollar, la respuesta fue dejar el criterio en manos de quien estaba trabajando: "lo que sea lo óptimo". Se eligió el molde de pregunta directa —un signo de interrogación grande, casi invisible, detrás del título— por ser el que más aporta con menos riesgo real: a diferencia de los moldes que necesitan una cita real o un dato numérico (que podrían terminar inventando una fuente o una cifra que no existe, algo que este sistema tiene como regla dura no hacer nunca), este reutiliza exactamente el mismo texto que ya se genera hoy. Solo cambia el diseño cuando ese texto ya es, de por sí, una pregunta real. Se implementó con cuidado de no activar el diseño nuevo si, al recortar un título demasiado largo, el signo de pregunta quedaba cortado en el medio —en ese caso, el sistema se queda con el diseño de siempre en vez de mostrar el gancho visual sobre un texto que dejó de ser pregunta. Se probó armando la imagen real con tres títulos distintos —uno corto con pregunta, uno largo que se recorta antes de llegar al signo, y uno sin pregunta— y los tres se comportaron como correspondía, sin ningún cambio visible en el diseño de siempre para el caso más común.

Al poco tiempo, usando el sistema de verdad, aparecieron cuatro problemas juntos en un solo mensaje: que todo salía como una pieza simple, nunca como carrusel; que dar una devolución sobre algo que los agentes escribieron se sentía muy lento, como si nada se moviera en pantalla; que no había ninguna vista previa de las piezas; y un pedido nuevo, que el sistema tuviera un depósito de fotos propias del que él pudiera consultar cuál conviene usar en cada caso. Los tres primeros resultaron ser errores reales, no una sensación —se investigó cada uno hasta la causa antes de tocar nada.

El carrusel nunca se generaba porque la parte del sistema que decide el formato de la pieza buscaba la palabra exacta "carrusel" en el texto libre que escribe el primero de los tres agentes al proponer la estrategia —y ese agente casi nunca usaba esa palabra tal cual, aunque describiera perfectamente una idea de varias partes. Se le pidió que, además de su propuesta en prosa, cierre siempre con una línea clara diciendo qué formato recomienda, y ahora el sistema lee esa línea primero, en vez de andar buscando una palabra suelta en un texto libre.

Lo de "muy lento" tenía dos causas reales, las dos en la misma parte del sistema que procesa una devolución. Una: si después de corregir algo el crítico volvía a frenarlo —que es el caso más común, porque una devolución casi nunca convence a la primera— la pantalla se quedaba mostrando el veredicto y la vista previa de la ronda anterior, dando la sensación de que nada había cambiado, aunque el redactor sí había vuelto a escribir y el crítico sí había vuelto a opinar. Ahora esa parte de la pantalla se actualiza siempre, después de cada ronda, apruebe o no. La otra causa: mientras el sistema procesaba una devolución, la pantalla dejaba de consultar si había mensajes nuevos, así que todo aparecía de golpe recién cuando terminaba el proceso completo, quince a cuarenta segundos después, sin ninguna señal en el medio. Ahora la pantalla sigue consultando durante todo ese proceso, y muestra en el momento quién está trabajando —si el redactor está reescribiendo o si el crítico está evaluando de nuevo— igual que en una conversación real.

La falta de vistas previas tenía una causa bien puntual: la pantalla le pedía el diseño de la pieza directo a GitHub sin ninguna credencial, y GitHub solo permite sesenta pedidos así por hora desde la misma dirección de internet —con varias sesiones seguidas de trabajo ese límite se agotaba rápido, y la vista previa quedaba en blanco sin ningún aviso claro. Se cambió para que ese pedido pase por el mismo camino interno que ya usa el resto del sistema para hablar con GitHub, que sí tiene una credencial real cargada del lado del servidor.

Sobre el depósito de fotos, antes de tocar nada se le preguntó qué quería decir con "la que más conviene" —podía ser simplemente usar la más nueva en vez de la más vieja, o algo más ambicioso. Eligió que el sistema elija la foto según qué tan bien se relaciona con el texto real de esa pieza. Ahora, cuando hay más de una foto disponible para elegir, el sistema le muestra todas las candidatas a la inteligencia artificial junto con el texto que va a acompañar, y le pide que elija cuál o cuáles representan mejor ese contenido puntual —ya no se limita a usar la que se subió primero. Si esa consulta falla por cualquier motivo, o si solo hay una foto disponible, el sistema sigue funcionando exactamente igual que antes, sin arriesgar una pieza que ya fue aprobada.

De paso se simplificó el texto de toda la pantalla de Mesa de Diálogo —encabezados, textos de ayuda, los mensajes de aprobado y rechazado— sacando explicaciones de mecanismo interno que no le sirven a quien la usa día a día.
