...
| **Dominio propio** | 🟢 **activo**: `https://mejorasm.mejoraok.com` (hub en `/`, EDA en `/app/`). Cloudflare CNAME `mejorasm → pabloeckert.github.io` DNS-only; `hub/CNAME` en el repo; `VITE_BASE_PATH` = `/app/` en los 3 `deploy-*.yml`; cert Let's Encrypt emitido, HTTPS forzado; `pabloeckert.github.io/MejoraSM/*` hace 301 al dominio nuevo. **Único pendiente de Pablo:** agregar `https://mejorasm.mejoraok.com/app/reset.html` a los Redirect URLs de Supabase (Auth → URL Configuration) para el blanqueo de contraseña | activado 2026-09-03 |
...
| **Plan de publicación 2026** | 🟢 7 fases completas y **todos los pendientes de Pablo cerrados el 2026-09-03**: dominio propio activo, IG/FB limpio + `sync-history` reactivado (cron 6h), **LinkedIn activo** (`ZERNIO_LINKEDIN_ACCOUNT_ID` cargado — el próximo post de feed sale también a LinkedIn), FB-Ads conectado (la función `ads` anda; ver nota abajo sobre las campañas ajenas que trae Zernio). Ver "Cierre de pendientes del plan de publicación" en la bitácora | cerrado 2026-09-01, pendientes cerrados 2026-09-03 |
...
- **FB Ads conectado en Zernio — la función `ads` responde `hasAdsAccount: true` con datos reales.** Pero: `/ads/campaigns?platform=facebook` devuelve 2 campañas que **no son de Mejora Continua** — una "Traffic Campaign" pausada del ad account "Laura Ferrero (Read-Only)" y un anuncio viejo de Marketplace de un auto (2024) del ad account personal de Pablo. Zernio trae todas las cuentas de anuncios a las que el usuario de Facebook tiene acceso, no solo la de la marca. La función anda (probada: `hasAdsAccount: true`, `campaigns: [...]`, `boostCandidates: []`, `advice: ""`), pero muestra ruido. **Decisión de Pablo pendiente:** si Mejora Continua tiene (o va a tener) su propio ad account con campañas, o si hay que filtrar `ads/index.ts` por un `platformAdAccountId` específico. Hasta entonces la tarjeta de Pauta en el Dashboard muestra esas 2 campañas ajenas — no es un bug, es lo que Zernio expone.
