...
| **Posts/carruseles de feed** | 100% automáticos (Crítico aprueba → autoagenda → publica), cron `publish-scheduled-posts.yml` **pide** cada 15 min — GitHub en la práctica lo dispara cada ~2-5 horas (throttling de `schedule:` bajo carga, confirmado real 2026-09-05, ver bitácora — afecta a los 3 cron de este repo, no es un bug puntual). El pipeline sigue siendo autónomo y no se rompe por esto, solo tarda más de lo que dice el nombre. **Elección de tema**: manual (Mesa de Diálogo) **o** autónoma vía `autopilot-cron.yml` (modo libre lun/mié/vie + email de aviso con ventana de veto — ver "Autopilot" en la bitácora; schedule **activo** desde 2026-09-01 (probado end-to-end), `RESEND_API_KEY`) | reactivado 2026-08-24; autopilot agregado 2026-09-01 |
...
| **CI (`ci.yml`)** | **lint + tsc bloqueantes de verdad** — el typecheck corría `tsc --noEmit` contra un `tsconfig.json` solution-style (`"files": []`), que **no chequea nada**; se pasó a `tsc -b` el 2026-09-07 (ver bitácora "Segunda pasada de la auditoría en vivo"). `types.ts` regenerado desde la base real → `tsc -b` en **0 errores**. lint 0/0. test + build corriendo. `permissions: contents:read` + `concurrency`. `vitest.config` con `testTimeout` 15s + `retry: 1`. Los 18 workflows con `permissions` + `timeout-minutes`. **`deploy-site.yml` ya no publica en paralelo a ciegas** — corre los mismos 3 gates de `ci.yml` (tsc -b/lint/test) antes de su propio build, así un commit roto no puede llegar a producción aunque CI esté en rojo (hallazgo real 2026-09-08, ver bitácora) | 2026-09-08 |
...
| **Forzar aprobación (Mesa de Diálogo)** | Override humano — Pablo tiene la última palabra sobre el Crítico. Botón "Forzar aprobación" visible cuando el Crítico rechaza (`status: needs_review`): agenda igual el último texto real del Creativo, queda registrado como `metadata.forcedByHuman`. De paso, `continueSession` (acción "continue") ahora también crea y agenda la propuesta cuando el Crítico aprueba en una ronda de revisión — antes solo pasaba en la primera ronda | 2026-09-02 |
| **Trabajo en paralelo (multi-sesión)** | El PM sumó una 2da sesión de Claude Code. Las dos activas: `mejorasm-03` (cerró el plan de publicación 2026 + login/autopilot/wipe/Biblioteca) y `mejorasm-01` = `session_01DDbWa2ZGKMaUhBWKTDJWi4` (Mesa de Diálogo / `orchestrator` / `forceApprove`). Coordinación por `COLABORACION.md` (tablero vivo: lanes, zonas calientes, mensajes) + sección "Trabajo en paralelo" en este archivo (reglas: `pull --rebase` siempre, nunca `stash pop` a ciegas, commits chicos, avisar antes de tocar archivos calientes). El 2026-09-03 Pablo les dio a las dos el mandato "arreglar todo" en simultáneo — ver bitácora "Mandato arreglar todo" | 2026-09-02 |
...
**Cambios de schema en Supabase:** `supabase db push --linked` funciona (el historial de migraciones se reparó el 2026-09-03, ver "Historial de migraciones reparado" en la bitácora — `db push --dry-run` da `upToDate: true`). Alternativa siempre válida: `supabase db query --linked "<SQL>"` o el SQL Editor del dashboard. Una migración nueva: crear el archivo en `supabase/migrations/` con el número siguiente y `db push` (o aplicarla con `db query -f` y después `supabase migration repair --status applied <n>`).
...
> **Estado actual (2026-08-31): el login está reinstaurado** — usuario/contraseña, UNA cuenta compartida (`pabloeckert@gmail.com`). Pasó por: cerrado (2026-07-28) → abierto a propósito (2026-08-25, `019`) → cerrado de nuevo a pedido de Pablo (2026-08-31, `023` + login por contraseña con blanqueo por email). Ver "Reinstauración del login" en la bitácora. La descripción de abajo es el modelo vigente otra vez, con estos cambios respecto de la versión 2026-07-28: **una sola cuenta compartida** (Pablo y Sindy usan la misma), **sin alta de cuenta ni OTP**, **blanqueo por email** vía `/app/reset.html`.
...
1. **Frontend**: `src/components/AuthGate.tsx` envuelve las rutas en `src/App.tsx` — sin sesión de Supabase Auth, se muestra `src/pages/Login.tsx` (email/password, **sin alta de cuenta**, con "olvidé la contraseña" → `resetPasswordForEmail` con redirect a `/app/reset.html`) en vez de la app. `src/pages/ResetPassword.tsx` es el respaldo dentro del SPA para el evento `PASSWORD_RECOVERY`; `public/reset.html` es el camino principal del blanqueo (página suelta, sin router — evita el choque del token en el hash con `HashRouter`; hace `PUT /auth/v1/user` con `fetch` directo, sin SDK). Sign-out en `AppSidebar.tsx`.
...
**Cuenta de acceso:** UNA sola cuenta compartida, `pabloeckert@gmail.com` (creada 2026-07-30, la única en `app_admins`). Pablo y Sindy usan la misma. **Blanqueo de contraseña**: botón "Olvidé la contraseña" en `Login.tsx` → Supabase manda un email con un link a `<origin>/app/reset.html` (hoy `https://mejorasm.mejoraok.com/app/reset.html` — dominio propio activo desde 2026-09-03). **Para que ese link funcione, esa URL tiene que estar en la allowlist de Redirect URLs del proyecto Supabase** (Authentication → URL Configuration) — si el blanqueo no llega o rebota, es casi seguro eso. Este es el pendiente que le queda a Pablo. Dar de alta a alguien más (no necesario hoy): `INSERT INTO app_admins (email) VALUES ('...')` + esa persona hace "olvidé la contraseña" para setear la suya.
...
Los tres conviven en el **mismo sitio** de GitHub Pages (un artifact por sitio): `hub/` en la raíz, `dashboard/` y el build del EDA (`dist/`) como subpaths (`/dashboard/`, `/app/`). **Un solo workflow, `deploy-site.yml`** (2026-09-03 — antes eran 3 casi idénticos, `deploy-eda`/`deploy-hub`/`deploy-dashboard`, que ya habían empezado a divergir), reconstruye todo el `_site/` en cada deploy y se dispara por push a cualquiera de las partes (`src/**`, `hub/**`, `dashboard/**`, config del build) o a mano.
...
- **`hub/index.html`** — desde 2026-09-03 es una **puerta mínima que redirige a `/app/`** (meta refresh + `location.replace` + botón "Entrar a MejoraSM"). Antes era una página estática con tarjetas que linkeaban a la UI de upload de GitHub — pero eso contradecía el mandato "sacar GitHub de la vista" (el "Subí material" real vive en `/app/#/hub`, subida por el servidor). `hub/CNAME` declara el dominio propio para todo el sitio (lo copia `deploy-site.yml` al `_site/`).

- **`dashboard/index.html`** — monitor estático de solo lectura (lee `content/log/historial.json`).

- **`src/` (EDA)** — deployado en `https://mejorasm.mejoraok.com/app/`. Requiere login. `vite.config.ts` usa `base: process.env.VITE_BASE_PATH || "/"` — en local `/`, en prod `/app/` (seteado por `deploy-site.yml`; era `/MejoraSM/app/` hasta que se activó el dominio propio el 2026-09-03).

## Deploy

- **`hub/` + `dashboard/` + EDA (`/app/`)**: activo en GitHub Pages, sitio combinado (ver arriba). **`deploy-site.yml`** (uno solo desde 2026-09-03) arma el `_site/` entero. `deploy-biblioteca.yml` se borró el 2026-09-01; `deploy-eda`/`deploy-hub`/`deploy-dashboard` se unificaron en `deploy-site.yml` el 2026-09-03.
- **Dominio propio `mejorasm.mejoraok.com`** — 🟢 **activo desde 2026-09-03** (ver "Dominio propio activado" en la bitácora). Cloudflare CNAME DNS-only → `pabloeckert.github.io`; `hub/CNAME` en el repo (`.gitattributes` lo fuerza a LF, GitHub Pages rechaza CRLF); dominio seteado con `gh api -X PUT repos/pabloeckert/MejoraSM/pages -f cname=...`; cert Let's Encrypt emitido, `https_enforced=true`. El dominio viejo (`pabloeckert.github.io/MejoraSM/*`) hace 301 al nuevo. **Único pendiente de Pablo:** `https://mejorasm.mejoraok.com/app/reset.html` en los Redirect URLs de Supabase.
- **`supabase/functions/`**: `deploy-functions.yml` (push a `supabase/functions/**`, o manual).
- **`supabase/migrations/`**: `deploy-migrations.yml` (`supabase db push --linked --yes --debug`, manual) — **operativo desde el 2026-09-03**: el historial de `schema_migrations` se reparó (`001`-`025` registradas), `db push --dry-run` da `upToDate: true`, así que el workflow ahora es un no-op seguro hasta que haya una migración `026+` nueva. Ver "Historial de migraciones reparado" en la bitácora.
...
- **Fase E — Rol de revisor (Sindy) — 🟡 parcial: lo que no depende de la puerta de acceso, hecho (2026-08-31). El rol read-only real necesita a Pablo.** Commit `<pendiente>`.
  ...
  - **Puerta de acceso — RESUELTA por decisión de Pablo (2026-08-31).** Respuesta textual: *"un solo password usuario y contraseña y el mail de registro para recupero si pierde contraseña o se olvida para blanqueo. un rol read-only para Sindy No es necesario mas"*. O sea: **no** un esquema de roles, **sí** una sola credencial compartida con blanqueo por email. Se descartó Cloudflare Access (Pablo quiere un login en la app, no una capa de infra). Ejecutado: se reinstauró el login por usuario/contraseña de Supabase Auth (`AuthGate` + `Login.tsx` sin alta de cuenta ni OTP), blanqueo por email vía `public/reset.html`, `_shared/auth.ts` volvió a exigir JWT real, y la migración `023` cerró el RLS de las 15 tablas + bucket `vault`. **El "rol read-only para Sindy" queda descartado** — es una sola cuenta compartida, sin roles. Detalle completo en "Reinstauración del login" en la bitácora. **Lo único que necesita Pablo:** (a) agregar `https://pabloeckert.github.io/MejoraSM/app/reset.html` a los Redirect URLs del proyecto Supabase para que el blanqueo funcione; (b) tener/setear la contraseña de `pabloeckert@gmail.com` (si no la recuerda, "Olvidé la contraseña" en el login — después de (a)).
...
## Reinstauración del login — usuario/contraseña + blanqueo por email — 2026-08-31

Cerrado el plan A-E, Pablo definió la puerta de acceso que la Fase E había dejado pendiente. Respuesta textual: *"un solo password usuario y contraseña y el mail de registro para recupero si pierde contraseña o se olvida para blanqueo. un rol read-only para Sindy No es necesario mas"*. O sea: descarta el esquema de roles (nada de "revisor read-only"), quiere **una sola credencial compartida** con blanqueo de contraseña por email. Se descartó Cloudflare Access — Pablo quiere el login dentro de la app, no una capa de infraestructura aparte.

**Lo que se hizo (revierte la "Remoción deliberada del login" del 2026-08-25):**
- **Frontend.** `AuthGate.tsx` vuelve a envolver la app en `App.tsx`. `Login.tsx` reescrito y simplificado: email + contraseña, **sin alta de cuenta, sin OTP**, más un modo "Olvidé la contraseña" que llama `supabase.auth.resetPasswordForEmail(email, { redirectTo: <origin>/MejoraSM/app/reset.html })`. `ResetPassword.tsx` nuevo (respaldo dentro del SPA para el evento `PASSWORD_RECOVERY`). `AppSidebar.tsx` recupera el botón "Cerrar sesión".
- **`public/reset.html`** — página HTML suelta (no pasa por Vite/React ni por el router), servida en `/app/reset.html`. Es el camino principal del blanqueo: el link del email de Supabase (flujo implícito) vuelve con el token en el hash (`reset.html#access_token=...&type=recovery`), y con `HashRouter` ese hash chocaría con el ruteo — una página aparte lo evita por completo. Toma el token del hash y hace `PUT /auth/v1/user` con `fetch` directo (sin SDK, sin CDN, sin problema de CSP), body `{ password }`. Valores de Supabase (URL + anon key) hardcodeados — son públicos igual.
- **`ai.ts`** vuelve a mandar el `access_token` real de la sesión en `Authorization`.
- **Edge Functions.** `_shared/auth.ts` volvió a exigir un JWT real de un email en `app_admins` (o la service-role key para cron) — se sacó la rama que aceptaba la anon key pelada. Las 6 funciones redeployadas.
- **Migración `023_reclose_access_password.sql`** — revierte `019`: `is_app_admin()` (idempotente) + `"Admin full access" USING (is_app_admin())` en las **15** tablas reales (las 12 de `019` + `insights_cache`, `insight_feedback`, `proposal_comments` que se crearon después) + bucket `vault` admin-only. Aplicada con `supabase db query --linked -f` y verificada (`pg_policies` — las 15 con `Admin full access`).

**Verificado real en producción, no solo deployado:**
- REST con anon key sobre `proposals` → `[]` (RLS bloquea). Edge Function `rule-engine` con anon key → `401`.
- El cron `rule-engine-cron.yml` (service-role key) corrido a mano después de cerrar todo → `run_log` con una fila `success` real → confirma que el camino server-to-server (Edge Function + bypass de RLS) sigue intacto.
- La pantalla de login renderiza bien en `https://pabloeckert.github.io/MejoraSM/app/` (usuario/contraseña/Ingresar/"Olvidé la contraseña"), el modo blanqueo también, y `/app/reset.html` carga y detecta bien la ausencia de token ("Este link no es válido o ya venció").
- **No se pudo probar el login real ni el email de blanqueo** — necesita la contraseña de Pablo y su inbox. Igual que el límite de OTP ya documentado muchas veces.

**Pendiente, solo de Pablo (documentado también arriba en "Cuenta de acceso"):**
1. Agregar `https://mejorasm.mejoraok.com/app/reset.html` a **Redirect URLs** del proyecto Supabase (Authentication → URL Configuration) — el dominio propio se activó el 2026-09-03, este es el camino principal ahora. Sin eso, el link del email de blanqueo rebota.
...
### Limpieza de datos — dos pasadas
Pablo pidió primero clean slate manteniendo el registro de lo publicado (se borró: 24 sesiones de Mesa de Diálogo, 515 `run_log`, 16 `copilot_advice`, etc.). Después de ver el Monitor con las 12 piezas de agosto ("datos ultra viejos"), pidió **wipe total dejando SOLO el Manual de Marca**. Estado final de la base: **se mantiene** `documents` (19) + `doc_chunks` (50) + `agent_config` (3 prompts — config, no contenido) + `app_admins` (login). **Todo lo demás en 0** (`proposals`, `metrics`, `dialogue_*`, `run_log`, `copilot_advice`, `insights_cache`, `success_rules`, `proposal_comments`; `historial_cache.posts` = `[]`). Repo: `content/published/` vaciado (73 imágenes), `content/log/*.json` reseteados, `content/used/` y `content/work/` limpios.

- **`sync-history.yml` PAUSADO** (cron comentado) — mientras Pablo limpia el contenido viejo de Zernio/IG/FB a mano, el cron no debe volver a traer ese historial al Monitor (Zernio sigue reportando los posts reales hasta que Pablo los borre de las apps). Reactivar cuando esté limpio, o disparar a mano con `workflow_dispatch`.
- **`Monitor.tsx` — botón "Borrar esta pieza"** (el tacho de cada tarjeta): despublica de Facebook vía Zernio + registra Instagram como gestionado a mano (Meta no permite despublicar IG por API) + saca la fila del `historial_cache`. Un gesto. El borrado en masa de las publicaciones reales lo hace Pablo (desde las apps o pieza por pieza).
- **Instagram/Facebook/Zernio: no se tocó nada en bloque desde acá** — regla dura: no borrar/despublicar contenido vivo de la marca en masa, aunque Pablo lo pida. Se lo dijo y él quedó a cargo de esa parte.
...
**Pendiente real de Fase 1 — CERRADO el 2026-09-03** (ver "Pase 'mejorar'" en la bitácora). Lo que parecía "~15% de tandas que reconvergen" era un set determinístico de ~10 DMs viejos que fallaban el batch entero cada corrida. Fix: reintento item-por-item cuando un batch falla completo (`8ea110f`) + mensajes solo-`[Attachment]` marcados `neutral` sin LLM (`ab84da1`). Verificado: 84/84 clasificados, `unc: 0`.
...
### Fase 5 — LinkedIn (🟢 ACTIVO desde 2026-09-03)

`scripts/lib/zernio.mjs::publishPost()` agrega `{ platform: "linkedin", accountId }` cuando existe `ZERNIO_LINKEDIN_ACCOUNT_ID`. `publish-scheduled-posts.yml` / `manage-post.yml` / `sync-history.yml` pasan el env; `manage-post.mjs` acepta `linkedin` solo si su cuenta está configurada; `Monitor.tsx` muestra "LinkedIn" en los badges. `publishStory` no lo toca — LinkedIn no tiene stories. **2026-09-03:** Pablo conectó la página de LinkedIn de MC en Zernio y cargó `ZERNIO_LINKEDIN_ACCOUNT_ID = 6a99f21e77555aae01d50cf5` como secret. El próximo post/carrusel de feed sale también a LinkedIn, automático.
...
### Fase 7 — Ads de Facebook (completa, solo lectura, 2026-09-01; filtro agregado 2026-09-03)

Edge Function `ads` (`report`, deployada):
- Lee `GET /v1/ads/campaigns?platform=facebook` de Zernio + su analytics. **Filtro (`isRelevantCampaign`, 2026-09-03):** Zernio trae TODAS las cuentas de anuncios a las que el usuario de FB tiene acceso — se descartan las de solo lectura (`(Read-Only)` en el nombre) y las campañas sin actividad en el último año. Métricas: se leen de `c.metrics.*` (Zernio las anida ahí), no del tope del objeto.
...
## Forzar aprobación en Mesa de Diálogo + fix real de `continueSession` — 2026-09-02

Dos hallazgos reales de uso, reportados por Pablo en la misma sesión, sobre el mismo flujo:

**Hallazgo 1 — el feedback de una ronda de revisión parecía no hacer nada.** Pablo mandó feedback tras un rechazo del Crítico ("no . es todo lo mismo como si no hubiera tomada nada de lo que hablamos"). Causa real, en dos partes: `useContinueDialogue` (`src/hooks/useDialogue.ts`) solo invalidaba la query de la lista de sesiones, nunca la de mensajes de esa sesión puntual — y `useDialogueMessages` corta su polling de 5s apenas la sesión deja de estar `active` (fix de perf de la auditoría del 2026-08-25), así que los turnos nuevos del Creativo/Crítico de la segunda vuelta nunca llegaban a la pantalla sin un F5 manual. Se corrigió invalidando también `["dialogue-messages", sessionId]`. De paso se agregaron `onError` a `handleContinue` y `handleStart` en `MesaDialogo.tsx` — antes un fallo real (timeout de 150s, red) era indistinguible de un envío silencioso.

**Hallazgo 2 — el Crítico aprobó en la revisión y "no pasó nada".** Con el fix anterior desplegado, Pablo pudo ver la aprobación del Crítico en una ronda de feedback, pero la pieza no se agendó. Causa real: `continueSession()` (acción `"continue"` de `orchestrator`) nunca tuvo la lógica de creación de propuesta que sí tiene `runDebate()` (acción `"start"`) — una aprobación en la primera ronda autoagendaba, una aprobación en una ronda de revisión no hacía nada, sin ningún indicio visual de que fuera una limitación de diseño y no un bug. Se factorizó `createProposalFromContent(sessionId, contenido, estrategia, titleFallback)` (helper compartido, mismo bloque que ya tenía `runDebate` — elige oferta, calcula `scheduled_at`/`experimentHour` vía `pickNextSlot`, inserta en `proposals`, registra el experimento de timing en `content_experiments` si corresponde) y se aplicó también en `continueSession`, que ahora actualiza `dialogue_sessions` (`status: "approved"`, `metadata` con `proposalId`/`autoPublished`/`scheduledAt`/`oferta`) igual que `runDebate`.

**Pedido de Pablo, en la misma conversación:** *"y si quiero forzar la aprobacion, en definitiva y tengo la ultima palabra, como hago ??"* — el Crítico es un chequeo automático de criterio de marca, no la autoridad final. Se agregó una acción nueva, `forceApprove(sessionId)`, en `orchestrator`: toma el último texto real que escribió el Creativo en la sesión (apruebe o no el Crítico) y llama al mismo `createProposalFromContent` — agendándolo igual que si hubiera sido aprobado. Deja registro explícito de que fue una decisión humana: un mensaje de auditoría (`agent: "sistema"`, sin restricción de valores en esa columna) y `metadata.forcedByHuman: true` / `forcedAt` en la sesión. Frontend: `forceApproveDialogue()` (`services/ai.ts`), `useForceApprove()` (`hooks/useDialogue.ts`), botón "Forzar aprobación — publicar de todos modos" en `MesaDialogo.tsx`, visible solo cuando `session.status === "needs_review"`, con `window.confirm` explícito antes de disparar (salta el chequeo automático de marca, tiene que ser una decisión consciente).

**Colisión real con trabajo concurrente, en el medio del desarrollo:** al ir a pushear, `origin/main` había divergido enormemente — otra sesión de Claude Code (`mejorasm-03`, corriendo en paralelo, ver "Trabajo en paralelo" más abajo) había reinstaurado el login, vaciado la base, sacado Biblioteca/Laboratorio, y cerrado las 7 fases del plan de publicación 2026, todo documentado ya en este archivo. Hubo un conflicto de merge real en `orchestrator/index.ts`. En vez de resolverlo a ciegas: se guardó el commit propio en una rama de respaldo local (`backup-force-approve-87f8197`), se abortó el merge, se hizo `git reset --hard origin/main` para sincronizar contra el estado real, se leyó la bitácora para entender qué había cambiado, y se rehizo el trabajo de `forceApprove`/`createProposalFromContent`/`continueSession` desde cero contra el código real y actual — no se tocó ni se revirtió nada de lo que la otra sesión había hecho, tal como Pablo confirmó explícitamente ("toda la restructuracion la hice yo y es correcta no modifiques nada y continua con lo ya hecho").

**Probado real en producción:** `deploy-functions.yml` (`orchestrator` con la acción `forceApprove`) y `deploy-eda.yml` (el botón nuevo), los dos `success` sobre el mismo commit. No se pudo probar el botón en un navegador autenticado real (el login está reinstaurado, sin sesión disponible acá) — la verificación quedó a nivel de `tsc`/lint/62 tests/build limpios y despliegues reales en verde, no de click real. Commit `0590366`.

## Entregable de cierre de ciclo + coordinación multi-sesión — 2026-09-02/03

Pablo compartió un `.md` de cierre de ciclo (traspaso a Project Management) pidiendo una versión visual con identidad de marca. Se generó un HTML con la paleta/tipografía de `mejora-continua-brand`, publicado como artifact. Al ir a commitear una copia en `entregables/2026-09-cierre-ciclo-mejorasm.html` (la ruta que el propio documento nombra), apareció que `mejorasm-03` ya había commiteado su propia versión del mismo entregable — las dos sesiones habían recibido el mismo `.md` y generado, en paralelo y sin saberlo, su propio HTML.

**Sin sesión en vivo para coordinar** (ninguna de las dos podía mensajear a la otra — no aparecían en la lista de sesiones alcanzables una de la otra), se comparó el contenido explícitamente antes de tocar nada: la versión propia tenía el isotipo real del repo (`assets/marca/logos/isotipo-color.png`, transparente, ~61KB en base64) contra un JPEG de la otra de 122KB con metadata C2PA embebida sin uso acá, y una barra de navegación con anchors a cada sección (útil para un documento de referencia largo) que la otra no tenía; la otra versión tenía un colofón visible con la aclaración de que Bw Modelica se reemplazó por Inter/League Spartan (la propia lo tenía como comentario HTML invisible) y una etiqueta de categoría junto al dueño en cada tarjeta de "necesita acción humana". Se consolidó **una sola versión** con lo mejor de las dos (commit `41d2492`) — ni se pisó a ciegas el archivo de la otra sesión, ni quedaron dos versiones compitiendo.

**De ahí salió `COLABORACION.md`** (agregado por `mejorasm-03`, commit `da74d40`, mientras se trabajaba en la consolidación de arriba): tablero vivo de coordinación entre sesiones — filas de "quién toca qué ahora", lista de archivos calientes, y un log de mensajes entre sesiones (append-only). Reglas duras nuevas en la sección "Trabajo en paralelo" de este mismo archivo: `git pull --rebase origin main` siempre (nunca merge, nunca `stash pop` a ciegas — lección real del 2026-09-02, un `stash pop` trajo un guardado viejísimo de la época de Biblioteca y pisó este archivo, detectado y restaurado sin pérdida), commits chicos y frecuentes, avisar antes de tocar un archivo caliente, coordinar migraciones (numeración reservada en el tablero) y deploys de Edge Functions.

Se respondió en `COLABORACION.md` confirmando `orchestrator`/Deploy Functions verde y avisando el cruce puntual en `entregables/` (la única vez, según lo acordado, que esta sesión entra en el lane de `mejorasm-03` — el resto del plan de publicación 2026 es su lane). Las dos sesiones quedaron con su fila en "libre".

## Mandato "arreglar todo" — auditoría coordinada de las dos sesiones (2026-09-03)

Pablo le dio a las dos sesiones el mismo mensaje: obsesivo al detalle, arreglar todo, no parar hasta terminar, coordinar por `COLABORACION.md`, trabajar en simultáneo, solo molestarlo si hace falta intervención humana real. Reparto acordado en el tablero (`mejorasm-01`: backend de diálogo — `orchestrator`/`vault-process`/`copilot`/`insights`/`classify-photo` + sus pantallas; `mejorasm-03`: pipeline + publicación — `scripts/**`, Edge Functions `inbox`/`recycle`/`ads`/`metrics-collector`/`rule-engine`/`repo`, workflows, infra/CI; frontend común: el que lo toque primero avisa). Detalle de cada hallazgo y fix se va agregando como subsección acá.

### Historial de migraciones de Supabase reparado (2026-09-03, `mejorasm-03`)

`supabase migration list --linked` mostraba `001`-`006` como aplicadas y `007`-`025` como **pendientes**, aunque las 19 están realmente aplicadas en la base (verificado columna por columna / tabla por tabla: `documents.category`, `proposal_comments`, `insights_cache`, `content_experiments`, `inbox_items`, `proposals.is_test`, `documents.processing_status`, RLS `Admin full access` en `proposals`, `dialogue_sessions_status_check` con `'error'`, etc. — todo presente). Causa: se aplicaron con `supabase db query --linked -f`, que corre el SQL pero no escribe en `supabase_migrations.schema_migrations`. Consecuencia: `deploy-migrations.yml` (`db push`) era un landmine — habría reintentado `007`-`025`, incluido el par `019` (abre RLS) / `023` (lo re-cierra).

**Fix:** `supabase migration repair --status applied 007 … 025` — solo escribe la tabla de bookkeeping, no toca el schema. Después: `supabase migration list` = todo sincronizado (0 pendientes), `supabase db push --linked --dry-run` = `{"upToDate":true,"migrations":[]}`. `deploy-migrations.yml` ahora es un no-op seguro; una migración `026+` nueva se puede aplicar con `db push` normal o con `db query -f` + `migration repair` como hasta ahora. Cierra el pendiente "confirmar `deploy-migrations.yml`" que arrastraba desde el 2026-08-05.

### `daily-story.yml` y los workflows de contenido — rotos desde el wipe (2026-09-03, `mejorasm-03`)

`daily-story.yml` venía **fallando desde el 2026-09-02** (exit 128) y nadie lo había notado — la story se generaba y renderizaba bien, pero el paso de commit hacía `git add content/published content/used content/inbox` y `content/inbox` **dejó de existir** después del wipe de la base del 2026-09-01 (esa carpeta solo se crea cuando alguien sube una foto por el Hub, y no hubo uploads). `git add <ruta-inexistente>` sale con 128 y mata el paso → la story nunca se commiteaba ni publicaba.
...
- **CORS: comodín `*.vercel.app`** — sacado de `rule-engine` / `metrics-collector` / `repo` (el EDA nunca se deployó a Vercel; `origin.endsWith(".vercel.app")` devolvía `Access-Control-Allow-Origin` a cualquier `*.vercel.app`). Quedan `orchestrator` / `vault-process` / `copilot` / `insights` / `classify-photo` con el mismo residuo — avisado a `mejorasm-01` en el tablero (es su lane).
- **`src/lib/fetchRetry.ts` borrado** — 0 importers, era el retry helper de `raw.githubusercontent` del Monitor, obsoleto desde que el historial se movió a `historial_cache`.
- **CSP de `index.html`** — `connect-src` bajó de 6 hosts a 3 (`'self'` + `*.supabase.co` + `api.github.com`): el browser nunca llama Groq/DeepSeek/Gemini/HF directo, todo va por Edge Functions.
- **`.env.example`** — decía "Groq=Estratega, DeepSeek=Crítico, Gemini=Backup" (stale desde el ruteo automático del 2026-08-05). Reescrito: el frontend solo usa las 2 vars `VITE_`, el resto son secrets de servidor.
- **Docs stale cerradas** — `publisher` / `ai-gateway` ya no estaban en el proyecto (`functions list` = las 11); el regex de emoji de `rule-engine` ya cubre Dingbats. Actualizadas las secciones que todavía los daban por pendientes.
- **Tests nuevos** — smoke tests para `Conversaciones` y `RecycleTab` (los componentes del plan de publicación no tenían). 62 → 66. Sin bugs encontrados.
- **Baseline confirmado verde:** `tsc` / lint (0 errores) / 66 tests / build, y `node --check` en los 17 scripts.

### Auditoría de lane — backend de diálogo (2026-09-03, `mejorasm-01`)

Reparto aceptado tal cual. Primero los 3 hallazgos que dejó flaggeados `mejorasm-03`: CORS `*.vercel.app` sacado de los 5 que faltaban (`orchestrator`, `vault-process`, `copilot`, `insights`, `classify-photo`); el falso positivo de `exhaustive-deps` en `ProposalDetailDialog.tsx:149` documentado con `eslint-disable-next-line`; `scripts/load-vault-documents.mjs` se queda — mismo criterio que `cargar-clave-zernio.ps1` (herramienta operativa documentada, cero riesgo en reposo).

Después, auditoría obsesiva del resto de la lane (`vault-process`, `copilot`, `insights`, `classify-photo`, Mesa de Diálogo, Bóveda, Configuración) — 5 hallazgos reales, con evidencia, no hipótesis:

- **`orchestrator` — `sanitizeTopic()` nunca se llamaba de verdad.** La función existe hace tiempo (recorte a 500 caracteres, mínimo 3, sin bytes nulos) pero el handler real de la acción `"start"` nunca la invocaba — un topic escrito a mano pasaba directo a `startSession()` sin ningún filtro de longitud ni contenido. Peor: el `throw new ValidationError(...)` de adentro referenciaba una clase que **nunca existió en el archivo** — si alguna vez se hubiera ejecutado, habría reventado con un `ReferenceError` genérico (500) en vez de devolver el mensaje de validación real. Corregido: `sanitizeTopic()` conectada al handler (solo en modo dirigido — el modo libre ya genera un tema propio, sanitizado por descarte), y el `throw` cambiado a un `Error` común, con el chequeo de status 400/500 del catch ampliado para reconocerlo.
- **`vault-process` — `classifyDocument` perdía categorías por un detalle de normalización.** Si el LLM respondía "buyer persona" (con espacio) en vez de `buyer_persona` (el formato exacto pedido en el prompt), la limpieza de la respuesta sacaba el espacio junto con la puntuación y el resultado nunca matcheaba ninguna categoría real — caía siempre a "otro", en silencio, sin ningún error visible. Fix: normaliza espacios a `_` antes de sacar el resto de los caracteres no válidos.
- **`InsightsSection.tsx` — el botón Útil/No aplica mentía en caso de error.** Marcaba el estado local como "elegido" antes de que la mutación resolviera, y no lo revertía si el guardado fallaba — el botón seguía viéndose marcado aunque la valoración nunca se hubiera guardado en `insight_feedback`, con el único aviso real siendo un toast que desaparece solo.
- **`Boveda.tsx` — el dropzone tenía su propio `<input type="file">`, sin resetear.** Hay dos inputs de archivo reales en la pantalla: el del botón "Subir documentos" (con `ref`, se reseteaba después de subir) y uno independiente adentro del área de arrastrar-y-soltar (sin `ref`, nunca se reseteaba). Volver a elegir el mismo archivo desde el segundo no disparaba `change` — el navegador no reemite el evento si el valor del input no cambió — así que un reintento desde ahí no hacía nada, sin ningún error visible. Fix: resetear `e.target.value` directo en el handler, que cubre los dos inputs sea cual sea el que disparó el evento.
- **`Configuracion.tsx` (`SystemDecisions`) — sin rama para una aprobación forzada.** El panel de "últimas decisiones del sistema" (agregado en el plan de continuación, Fase D) no tenía en cuenta `forceApprove` (agregado después, 2026-09-02) — esas sesiones no tienen `metadata.evaluacion` (nunca pasaron por el Crítico), así que caían al último `else` y mostraban el string crudo de la base (`"approved"`) en vez de una etiqueta real, sin ningún indicio de que fue una decisión humana. Fix: rama explícita para `metadata.forcedByHuman` → "Forzada por Pablo".

Verificado en cada commit: `tsc` limpio, lint 0 errores, 66/66 tests, build limpio. `deploy-functions.yml` (`orchestrator` + `vault-process`) y `deploy-eda.yml` verdes sobre el commit final (`e90b251`). Lane cerrada, sin pendientes propios — coordinado en `COLABORACION.md` en cada paso.

### Segunda pasada: auditoría de frontend común (2026-09-03, `mejorasm-01`)

Con la lane propia cerrada y Pablo repitiendo el mandato "arreglar todo" a las dos sesiones, `mejorasm-01` tomó **frontend común** (nadie la tenía asignada) mientras `mejorasm-03` seguía con pipeline/infra. Revisados a fondo, línea por línea: `src/services/supabase.ts`, `useProposals.ts`, `Monitor.tsx`, `Conversaciones.tsx`, `PublishNowCard.tsx`, `Hub.tsx`, `Propuestas.tsx`, `Calendario.tsx`, `Auditoria.tsx`, `Dashboard.tsx`, `AppSidebar.tsx`. Dos hallazgos reales, los dos del mismo tipo — una acción real que termina en "éxito" sin haber chequeado si de verdad lo fue:

- **`Monitor.tsx` — "Borrar esta pieza" podía decir éxito con un despublicado fallido.** `useWorkflowAction().run()` nunca tira — atrapa el error adentro, muestra su propio toast, y devuelve `false`. `handleDelete()` no chequeaba ese booleano después de disparar el despublicado de Facebook / marcar Instagram a mano: si el dispatch fallaba (token sin permiso de Actions, red, rate limit de GitHub), el código seguía igual, sacaba la fila del Monitor y decía "Pieza borrada" — un **falso borrado** sobre una acción real de publicación (Facebook nunca se había despublicado de verdad). Confirmado que los otros 4 handlers del mismo archivo sí chequeaban bien el booleano — era puntual a este. Fix: aborta antes de tocar el Monitor si cualquiera de los dos dispatches falla.
- **`Propuestas.tsx` — el mismo patrón, sobre el portapapeles.** `handleCopy()` no esperaba ni chequeaba la promesa de `navigator.clipboard.writeText()` (puede rechazar: contexto no seguro, sin permiso) — el tilde de "copiado" se mostraba igual aunque nada se hubiera copiado de verdad. `ProposalDetailDialog.tsx` ya tenía el patrón correcto (`then`/`catch` + toast de error); replicado acá.

De paso, `mejorasm-03` encontró y corrigió un bug real en el propio fix de `[01]` del batch anterior: el `eslint-disable-next-line` de `ProposalDetailDialog.tsx:149` había quedado 4 líneas arriba del `}, [deps])` por el comentario multilínea de en medio — no tapaba nada, y sumaba un warning nuevo de "directiva sin uso" (8 → 6 warnings tras el fix real).

Verificado en cada commit: `tsc` limpio, lint 0 errores (6 warnings preexistentes), 66/66 tests, build limpio, `deploy-eda.yml` verde en cada push. `src/services/supabase.ts`, `Conversaciones.tsx`, `PublishNowCard.tsx`, `Hub.tsx`, `Calendario.tsx`, `Auditoria.tsx` y `Dashboard.tsx` revisados a fondo sin encontrar hallazgos — ya bien auditados en rondas anteriores. Todo coordinado en `COLABORACION.md`, con `mejorasm-03` activa en paralelo en su propio lane (pipeline/infra), sin cruces.

### Pase "mejorar" — pipeline / scripts / Edge Functions / CI (2026-09-03, `mejorasm-03`)

Con el mandato repetido, `mejorasm-03` volvió a activarse y pasó por su lane con criterio de "mejorar", no solo "arreglar bug conocido":

- **Bandeja de conversaciones — pendiente de la Fase 1 cerrado.** El "~15% de tandas que fallan el parseo y reconvergen" resultó ser, en la práctica, ~10 DMs viejos (spam de otras marcas, auto-respuestas, `[Attachment]` pelado) que eran un **set determinístico**: rompían el JSON del batch de 10 y fallaban los 10 juntos, en cada corrida, sin reconverger nunca. Un intento previo de mejora (commit `fd19db7` — temperatura 0 + presupuesto de tokens más holgado + fallback siempre) **empeoró** la clasificación general en producción (de ~85%/corrida a ~20%) y se revirtió (`5261e70`) — lección: no shippear un cambio de prompt sin verlo correr en prod primero. El fix real, en dos commits chicos: si un batch no clasifica ni uno, se reintenta item por item (un objeto JSON solo siempre parsea — `8ea110f`); los mensajes que son solo `[Attachment]` se marcan `neutral` sin gastar una llamada al LLM (`ab84da1`). **Resultado verificado contra la base real: 84/84 entrantes clasificados, `unc: 0`.**
- **`scripts/lib/**` — timeouts en toda llamada de red.** Ninguna de las llamadas a Anthropic / Groq / Zernio / PostgREST desde el pipeline tenía tope de tiempo: una conexión colgada bloqueaba la corrida de GitHub Actions hasta el límite de 6h del job. Ahora `AbortSignal.timeout` en todas (Anthropic/Groq 90s, `run_log` 15s, Zernio 45s / upload de video 120s); `claude.mjs` reintenta también si el `fetch` tira (blip de red/DNS), no solo ante 429/500; `zernio.mjs` suma `safeJson` (no revienta si Zernio devuelve un HTML de gateway 502/504). Commit `4621410`.
- **Edge Functions del lane — código muerto borrado.** `metrics-collector` tenía un `generateInsights()` con consejos hardcodeados (`"✅ Engagement rate por encima del promedio (3%)"`) — muerto desde que existe la Edge Function `insights` real (Fase A), sin ningún caller (ni frontend ni cron). `rule-engine` tenía un `getSuggestions()` / acción `"suggest"` — nadie la llama, `orchestrator` lee `success_rules` directo con `getLearnedRulesBlock()`; de paso arrastraba un bug de precedencia de operadores en el armado del texto de sugerencia (`r.action?.reason || r.action?.prefer ? ...` — el texto informativo del `reason` nunca se usaba). Los dos borrados, commit `9074e3f`, deploy verde, crons corridos OK.
- **Edge Function `repo` — endurecida.** `writeFile` reintenta una vez con el sha fresco ante un 409 (dos subidas de foto concurrentes dejan el sha viejo); `dispatchWorkflow` castea todo valor de input a string (un boolean tira 422 en la API de `workflow_dispatch` de GitHub).
- **CI + workflows.** `permissions` explícito en los 20 workflows (los 6 cron-curl con `permissions: {}`, `deploy-functions` con `contents: read` + `cancel-in-progress: false` para no matar un deploy a mitad); `concurrency` en `ci.yml` (cancela el CI de un commit ya superado), `deploy-migrations` / `sync-history` / `autopilot` (serializadas, nunca cortar a mitad); `timeout-minutes` en los 21 jobs (ninguno lo declaraba — el default de GitHub son 360 min, así que un cuelgue quemaba 6h de runner: 10 min los cron-curl y manage-*, 15 los deploy, 20 CI, 25 el pipeline de contenido). Commits `17afbf3` + `dce9ac7` + `796c25d`.
- **`vitest.config.ts` — el falso rojo del CI.** `testTimeout`/`hookTimeout` 15s + `retry: 1` — un test de página que flaquea una vez bajo carga (patrón ya documentado como "sobrecarga transitoria") pasa en el reintento; uno roto de verdad falla las dos. Commit `4a68749`.
- **Docs.** Diagrama de arquitectura de `CLAUDE.md` corregido (mostraba DeepSeek/Gemini como proveedores activos — el ruteo real desde 2026-08-05 nunca los selecciona); `scripts/deploy.sh` (lista de funciones 4→11, secrets reales, URL correcta); refs a `PLAN_AUTONOMIA.md` (archivo borrado) en 3 workflows → `CLAUDE.md`.

`ads` y `recycle` se revisaron sin hallazgos que ameriten tocar (funciones dormidas — 0 métricas / 0 posts publicados tras el wipe). Baseline verde en cada commit; `deploy-functions` y `ci` verdes sobre `9074e3f`; `rule-engine` y `metrics-collector` corridos post-deploy y respondiendo `HTTP 200` con los resultados vacíos esperados. Todo coordinado en `COLABORACION.md`, sin cruces con `mejorasm-01` (que cerró su lane + frontend común en paralelo).

### Pase "mejorar" cont. — los scripts del pipeline uno por uno (2026-09-03, `mejorasm-03`)

Segunda tanda del pase, revisando cada script de `scripts/` con criterio de lógica, no solo timeouts. Tres hallazgos reales:

- **`autopilot.mjs` — fail-safe roto** (`af50133`). `orchestrator.startSession` deja la pieza `scheduled` a la hora que elija `pickNextSlot` (que puede ser pronto — el bloque 12:00 UTC del mismo día). Autopilot le empujaba `scheduled_at` a la ventana de veto de las 23:00 y **recién después** mandaba el email. Si el runner moría entre el push y el email — o si el `fetch` a Resend colgaba —, la pieza quedaba `scheduled` y el cron de publicación (cada 15 min) la sacaba **sin que Pablo hubiera visto nada**. Rompía el invariante del módulo, escrito en su propio encabezado ("nunca se publica algo sin haber avisado"). Reescrito: apenas el Crítico aprueba, la pieza se baja a `pending`/`scheduled_at: null` (desarmada — un crash acá no publica nada); se manda el email; solo si el email sale OK se re-arma a `scheduled` en la ventana de veto. Cualquier crash intermedio la deja en `pending` — molesto, no peligroso. Timeouts en las 3 llamadas de red (orchestrator 180s, Supabase 20s, Resend 30s).
- **`publish-scheduled-posts.mjs` — `markError` pisaba `metadata`** (`0d063e9`). `PATCH { metadata: {...} }` sobre una columna `jsonb` **reemplaza el objeto entero** — así que un fallo de publicación borraba `metadata.recycled_from` (linaje de reciclado), `experimentHour` (Fase 4) y `forcedByHuman` (override del Crítico). Ahora lee el `metadata` actual y mergea.
- **`manage-post.mjs` — `markRejected` sin chequear el PATCH** (`797ac7b`). Corría después de despublicar en Zernio pero nunca miraba si el PATCH a Supabase tuvo éxito — si fallaba, la propuesta quedaba `published` en la base mientras el post real estaba de baja (mismo síntoma que los bugs de `mark-manual` ya documentados en la ronda del 2026-08-20). Ahora tira si falla. De paso, `manage-post` escribe en `run_log` — era el único script del pipeline sin rastro en `/auditoria` para una acción consecuente.
- **`sync-history.mjs`**: `fetchAllPostsForAccount` era `while(true)` sin tope — una paginación rara de Zernio podía loopear indefinido. Cap de 50 páginas + timeout 45s + parseo defensivo.
- **`reel.yml`**: la foto usada se commitea **después** de publicar, no antes — el MP4 no se sirve desde el repo (va directo a Zernio), así que el commit-primero no aportaba nada, y si el publish fallaba la foto quedaba fuera de `inbox` sin poder re-disparar. El commit final es best-effort (warning si el push no entra, el reel ya salió).
- **`render-reel.mjs`**: timeout de 180s en `ffmpeg` (un encode de 9s tarda segundos). **`generate-brief.mjs`**: timeout de 20s en el `fetch` a MejoraIdentidad (`raw.githubusercontent` tiene caídas — mejor seguir sin contexto fresco que comerse el job).

`render-story.mjs`, `publish-story.mjs`, `manage-story.mjs`, `publish-now-manifest.mjs` y `publish-now.yml` se revisaron sin hallazgos reales — están bien. `node --check` limpio en los 17 scripts; baseline (`tsc`/lint/66 tests/build) verde; CI verde en cada push.

### Tercera pasada — auth + componentes sueltos + upload/export (2026-09-03, `mejorasm-01`)

Con las dos lanes originales cerradas, `mejorasm-01` siguió por lo que genuinamente nadie había auditado todavía en ninguna ronda: las pantallas de auth (`AuthGate.tsx`, `Login.tsx`, `ResetPassword.tsx`, `public/reset.html`), los componentes sueltos que quedaron sin abrir (`RecycleTab.tsx`, `ReportDialog.tsx`, `AdsCard.tsx`, `ProposalComments.tsx` + `useProposalComments.ts`, `ConfirmDialog.tsx`, `MiniMarkdown.tsx`, `Onboarding.tsx`, `NotFound.tsx`), `AppLayout.tsx`, el resto de `useGithubUpload.ts` (`runOne`/`processQueue`, la cola de subida que ya había tenido un bug real de condición de carrera el 2026-08-25) y `src/lib/export.ts`.

Un hallazgo real:

- **`Onboarding.tsx` — `localStorage` sin proteger, montado fuera del `ErrorBoundary`.** En `src/App.tsx`, `<Onboarding />` se renderiza como hermano de `<ErrorBoundary>` (tiene que existir antes de la `Suspense` de rutas para poder mostrarse en cualquier página), así que ningún boundary de arriba lo cubre — un error ahí no tiene nada que lo atrape. Sus 3 llamadas a `localStorage` (`getItem` en el `useEffect` de montaje que decide si mostrar el tutorial; `setItem` en `handleComplete`/`handleSkip`) no estaban en try/catch, a diferencia del mismo riesgo ya cubierto en este mismo repo por `useProposalComments.ts` (`getCommentAuthor`/`setCommentAuthor`, con el comentario explícito "modo privado / storage bloqueado — el nombre se pide de nuevo"). En una sesión de navegación privada, o con el storage bloqueado por una extensión de privacidad/política del navegador, `localStorage.getItem` puede tirar `SecurityError` — y como pasa dentro de un `useEffect` de un componente sin ningún `ErrorBoundary` por encima, la excepción no se atrapa en ningún lado: tumba el render de toda la app a blanco, sin ninguna UI de recuperación, en la primera visita real de un usuario con ese tipo de configuración de navegador. Fix (commit `96633f3`): `getItem`/`setItem` envueltos en try/catch — storage bloqueado en la lectura se trata como "onboarding no completado" (se ofrece igual, sin romper nada); storage bloqueado en la escritura simplemente no persiste (el tutorial vuelve a aparecer la próxima visita, degradación aceptable en vez de un crash).

El resto de los archivos de la lista, revisado a fondo, sin hallazgos — `useGithubUpload.ts::runOne`/`processQueue` en particular se verificó específicamente contra el patrón del bug real de 2026-08-25 (cola por índice de array pisándose entre tandas) y sigue sano: la cola FIFO por `id` (no por índice) y el flag `processingRef` sin state de React siguen intactos, sin regresión.

Verificado: `tsc` limpio, lint 0 errores (6 warnings preexistentes, sin cambios), 66/66 tests, build limpio, `deploy-eda.yml` verde sobre `96633f3`. Con esto, las tres rondas de auditoría de `mejorasm-01` (backend de diálogo, frontend común, y esta tercera pasada) quedan cerradas sin pendientes propios — coordinado en `COLABORACION.md` en cada paso, sin cruces con `mejorasm-03` (que en paralelo siguió revisando la lógica de los scripts del pipeline uno por uno).

### Cuarta pasada — sweep fresco tras el cierre de `mejorasm-03` (2026-09-03, `mejorasm-01`)

`mejorasm-03` cerró su sesión por límite de cuenta con las dos lanes de esa tanda auditadas de punta a punta y sin ningún hilo abierto (ver "Estado para la próxima sesión" en `COLABORACION.md`). Pablo repitió el mandato de investigar/mejorar/arreglar dirigido a una sesión nueva; `mejorasm-01` siguió bajo su identidad real (el `Claude-Session` de los commits queda fijo a la sesión, así que fabricar un alias nuevo solo habría corrompido el registro del tablero) y arrancó un sweep fresco por superficie que ninguna ronda anterior había cubierto a fondo, para no repetir terreno ya cerrado. Seis hallazgos reales:

1. **Placeholders de render vía `String.replace()` — corrupción real de HTML publicado.** `scripts/render-scheduled-posts.mjs`, `scripts/render-story.mjs` y `src/components/PiecePreview.tsx` (17 call sites en total) sustituían `{{HEADLINE}}`/`{{SUBTEXT}}`/`{{KICKER}}`/etc. pasando el copy generado por el LLM como el *string* de reemplazo de `.replace()`. `String.replace()` interpreta patrones especiales (`$&`, `$$`, `` $` ``, `$'`) dentro de ese string incluso cuando el patrón de búsqueda es un string plano, no un regex — es un detalle de la spec del método, no depende de cómo se lo invoque. Si un hook/subtext generado por IA contenía literalmente `$&`, la sustitución reinsertaba el propio placeholder (`{{HEADLINE}}`) como texto visible en la imagen final — corrupción real en una pieza que se publica en Instagram/Facebook de forma completamente autónoma, sin revisión humana. Reproducido con un one-liner de Node antes de aplicar el fix: `"hola {{HEADLINE}} chau".replace("{{HEADLINE}}", "x $& y")` → `"hola x {{HEADLINE}} y chau"`. `render-reel.mjs` no tenía el problema (arma el HTML con un template literal, no con `.replace()`). Fix: los 17 sitios pasan a usar una función replacer (`() => valor`), que `String.replace()` nunca interpreta como patrón especial. Commit `d47bf69`.

2. **`src/services/github.ts::call()` sin timeout — un fetch colgado bloqueaba todo.** Este archivo se reescribió el 2026-09-01 (cliente liviano de la Edge Function `repo`, ya no habla directo con GitHub) — *después* del hallazgo B14 de la auditoría del 2026-08-31 ("`vault-process`/`classify-photo`/`copilot` usaban fetch pelado y podían colgarse para siempre"), y reintrodujo exactamente la misma clase de bug: sin `AbortController` ni timeout, a diferencia de `src/services/ai.ts::fetchWithTimeout`. El caso más visible: `PublishNowCard` pollea `github.getJsonFile()` en un loop con su propio `POLL_TIMEOUT_MS` — pero ese chequeo vive *después* del `await` a `getJsonFile()`. Si un solo fetch se cuelga, el `await` nunca vuelve, el chequeo de `POLL_TIMEOUT_MS` nunca se alcanza, y la UI de "Publicar ahora" queda en "preparando…"/"publicando…" para siempre, sin error, sin más salida que recargar la página a mano. Mismo riesgo en `listDir` (listado de fotos), `commitPhoto` (sube una foto real — el botón queda cargando sin fin) y `triggerWorkflow` (Reintentar/Despublicar del Monitor). Fix: mismo patrón que `fetchWithTimeout` de `ai.ts` — `AbortController` con 60s de margen. Commit `9f31f78`.

3. **`orchestrator::continueSession` sin chequear si la sesión existe.** `forceApprove()` (agregado 2026-09-02) sí tiene `if (!session) throw new Error("Sesión no encontrada")` antes de usar la fila — `continueSession()` hace el mismo fetch `.single()` pero nunca chequeaba el resultado, y usaba `session.topic` unas líneas más abajo sin guard. Un `sessionId` que ya no existe (ej. una pestaña vieja del navegador con una sesión de antes del wipe de la base del 2026-09-01, que vació `dialogue_sessions` por completo) tira un `TypeError` crudo ("Cannot read properties of null") en vez de un mensaje claro — el handler HTTP de más abajo lo atrapa igual (nunca crashea la función, siempre devuelve un JSON de error), pero lo que ve Pablo es un error genérico en vez de uno útil. Fix: mismo guard y mismo mensaje que `forceApprove`. Commit `7e66a4a`.

4. **`insights::parseInsights` — un `confidence: 0` real del LLM se pisaba en silencio por 50.** `confidence: Math.max(0, Math.min(100, Math.round(Number(x.confidence) || 50)))` — un 0 legítimo (documentado como valor válido en el propio system prompt de la función, "0-100, entero"; el `Math.max(0, ...)` que envuelve la expresión muestra que el código sí pretendía permitirlo) es falsy en JS, así que `0 || 50` da `50` — justo lo opuesto de la regla innegociable del propio prompt ("nunca inventes una cifra que no esté en el contexto"): un insight con cero confianza real se le mostraba a Pablo como si el modelo hubiera dicho 50%. Reproducido: `Number(0) || 50` → `50`; `Number.isFinite(0) ? 0 : 50` → `0` (y sigue cayendo a 50 para `undefined`/`NaN`, sin cambiar ese caso). Fix: `Number.isFinite()` en vez del `||` con fallback. Commit `cfbd411`.

5. **`vault-process` — "Reprocesar" un documento podía dejarlo sin ningún chunk.** `processDocumentInner()` borraba los chunks viejos (`doc_chunks.delete().eq("document_id", ...)`) *antes* de insertar los nuevos — si "Reprocesar" (feature real, expuesta en `/boveda` desde la auditoría del 2026-08-31) fallaba justo en el insert (blip de red, error de DB), el documento quedaba sin ningún chunk: un documento que era buscable por RAG pasaba a no serlo, peor estado que antes de tocar el botón, sin ningún camino de vuelta salvo re-subir el archivo entero. Confirmado contra el schema real (`001_initial_schema.sql`) que `doc_chunks.id` es su propia PK (`gen_random_uuid()`), sin constraint único sobre `(document_id, chunk_index)` — así que insertar los chunks nuevos antes de borrar los viejos es seguro, pueden convivir un instante sin chocar. Fix: reordenado (insertar primero, y solo si el insert confirma, borrar los viejos) + el borrado usa los `id` reales capturados antes, nunca un filtro por `document_id` (para no arriesgar borrar de más si algo cambia). Commit `2a1252e`.

6. **Chequeado y descartado — mismo patrón sintáctico, no es un bug ahí.** `vault-process::searchDocs` tiene `Math.min(Math.max(Number(limit) || 5, 1), 20)`, la misma forma `Number(x) || N`. Se investigó específicamente por la similitud con el hallazgo #4 antes de darlo por bueno: acá `limit` es una cantidad de resultados de búsqueda, y el propio call site (`searchDocs(query, limit || 5)`, ya aplicando el mismo fallback antes de llamar) confirma que `0` **no** es un valor semánticamente significativo para este parámetro — nadie pide "0 resultados" a propósito, a diferencia de `confidence: 0`, que sí es una respuesta legítima y significativa del LLM. Se deja anotado para que quede constancia de que el mismo patrón se revisó dos veces y solo una vez era un bug real — no alcanza con reconocer una forma de código, hay que evaluar qué representa el valor en cada caso.

**Revisado a fondo sin hallazgos adicionales:** `src/services/ai.ts` completo, `supabase/functions/_shared/auth.ts` y `runLog.ts`, las migraciones SQL (`023`/`024`/`025` consistentes entre sí — las tablas nuevas creadas después de `023` heredan `is_app_admin()` desde su creación, no el patrón "Allow all" viejo; `app_admins` sin políticas para anon/authenticated es intencional, documentado en el propio `auth.ts`), `orchestrator/index.ts` completo (1202 líneas, más allá del hallazgo #3), `copilot/index.ts` completo (469 líneas), `classify-photo/index.ts` completo, `repo/index.ts` completo, y los hooks `useDialogue.ts`, `useVault.ts`, `useMetrics.ts`, `useInsights.ts`, `useCopilot.ts` y `useInbox.ts` completos.

**Deliberadamente no releído:** `metrics-collector`/`rule-engine`/`ads`/`recycle`/`inbox` (Edge Functions) — `mejorasm-03` ya los auditó a fondo con evidencia real en el pase "mejorar" de esa misma tanda; releerlos habría sido duplicar trabajo ya hecho, no agregar valor real.

Verificado en cada commit: `tsc` limpio, lint 0 errores (6 warnings preexistentes, sin cambios), 66/66 tests, build limpio; los 5 commits que tocan Edge Functions confirmados uno por uno con `deploy-functions.yml` en verde (`conclusion: success`). Todo coordinado en `COLABORACION.md`, actualizando la fila propia y dejando el detalle completo de los 6 hallazgos en el tablero.

## Cierre de pendientes del plan de publicación (2026-09-03)

Pablo hizo los 3 pasos que quedaban de su lado. Estado real de cada uno:

- **IG/FB limpio → `sync-history.yml` reactivado.** Se descomentó el `schedule` (cron cada 6h) — estuvo pausado desde 2026-09-01 mientras Pablo limpiaba. Corrida real de verificación: `Historial sincronizado: 10 post(s)` — son 9 stories reales de la última semana (del cron de `daily-story`) + 1 vieja del 2026-08-03 (`facebook:cancelled`, con `**` de markdown filtrado en el copy — Pablo puede sacarla del Monitor con "Borrar esta pieza" si molesta). Cero feed posts (coherente: `proposals` vacía tras el wipe, el pipeline de feed arranca de cero).
- **LinkedIn — 🟢 ACTIVO.** Probe temporal (`/v1/accounts`, borrado apenas confirmó) — las 3 cuentas de "Mejora Continua" `enabled+active`: `facebook 6a5640333ecd8aa344fadb4b`, `instagram 6a56405a3ecd8aa344faecae`, `linkedin 6a99f21e77555aae01d50cf5`. Pablo cargó `ZERNIO_LINKEDIN_ACCOUNT_ID = 6a99f21e77555aae01d50cf5` como secret de GitHub Actions (2026-09-03 22:32 UTC — es un identificador, no credencial; el clasificador del entorno bloqueó que lo cargara la sesión, lo hizo Pablo). Verificado: `sync-history` corrió con el env nuevo sin errores (10 posts, 0 de LinkedIn todavía porque no se publicó nada ahí aún). **El próximo post de feed que publique el pipeline sale a IG + FB + LinkedIn** — `publishPost()` suma `linkedin` a las plataformas cuando ve el env. No se probó una publicación real a LinkedIn (misma cautela de siempre: no publicar contenido real solo para testear).
- **FB Ads conectado en Zernio — la función `ads` anda, y el 2026-09-03 se le agregó un filtro.** `/ads/campaigns?platform=facebook` devolvía 2 campañas ajenas (Zernio trae todas las cuentas de anuncios a las que el usuario de FB tiene acceso, no solo la de la marca): una "Traffic Campaign" del ad account "Laura Ferrero (Read-Only)" que ya no existe, y un anuncio de Marketplace de un auto (Zafira 2009) de 2024 del ad account personal de Pablo. Pablo confirmó: las dos son basura, filtrar. `ads/index.ts::isRelevantCampaign()` (commit `65a26c4`) descarta ad accounts "(Read-Only)" y campañas sin actividad en el último año. De paso se corrigió el mapeo de la respuesta — leía `c.spend`/`c.impressions`/`c.clicks` cuando Zernio las anida en `c.metrics.*`, así que daba siempre `null`. **Verificado en vivo:** `ads` responde `campaigns: []` (las 2 basura filtradas). Cuando MC corra una campaña real desde una cuenta propia y con actividad, pasa el filtro sin tocar nada.

## Dominio propio activado — `mejorasm.mejoraok.com` (2026-09-03)

Pablo hizo el punto 1 de los pendientes: agregó el registro DNS. El resto lo ejecutó esta sesión.

- **DNS (Cloudflare, lo hizo Pablo):** `CNAME mejorasm → pabloeckert.github.io`, **DNS-only (nube gris)** — igual que los otros subdominios (`app`/`crm`/`diagnostico`, todos Vercel, todos sin proxear). Proxear lo hubiera roto: GitHub Pages necesita ver el DNS real para emitir el cert. Confirmado con `nslookup`: `mejorasm.mejoraok.com` → `pabloeckert.github.io` → IPs reales de GitHub Pages (`185.199.108-111.153` + IPv6).
- **Repo (un commit, `e61e201`):** `hub/CNAME` = `mejorasm.mejoraok.com` (lo copian los 3 `deploy-*.yml` al `_site/`); `.gitattributes` nuevo con `CNAME text eol=lf` (GitHub Pages rechaza CRLF, y esta máquina tiene `autocrlf=true`); `VITE_BASE_PATH` `/MejoraSM/app/` → `/app/` en los 3 workflows (con dominio propio el subpath del nombre de repo desaparece); `autopilot.mjs::APP_URL` + `scripts/deploy.sh` → dominio nuevo. El `index.html` canonical/og y el CORS de las 11 Edge Functions ya apuntaban ahí desde el 2026-09-01.
- **GitHub Pages:** el commit tocó `hub/**` → `deploy-hub.yml` corrió solo (build completo con el nuevo base path). El CNAME del artifact **no** auto-seteó el dominio (limitación conocida del modo "workflow"), así que se seteó a mano: `gh api -X PUT repos/pabloeckert/MejoraSM/pages -f cname='mejorasm.mejoraok.com'` → cert Let's Encrypt emitido de inmediato (el DNS ya resolvía), después `gh api -X PUT ... -F https_enforced=true`.
- **Verificado en vivo:** `https://mejorasm.mejoraok.com/` (hub) y `/app/` (EDA) → 200; los assets del EDA se sirven desde `/app/assets/...` (base path nuevo activo); `https://mejorasm.mejoraok.com/app/reset.html` → 200; `http://` → 301 a `https://`; `pabloeckert.github.io/MejoraSM/app/` → 301 al dominio nuevo; preflight CORS de `orchestrator` con `Origin: https://mejorasm.mejoraok.com` → 200 con `Access-Control-Allow-Origin` correcto; cert `CN=mejorasm.mejoraok.com`, Let's Encrypt, vence 2026-12-02.
- **Supabase (hecho por Pablo, 2026-09-03):** agregó `https://mejorasm.mejoraok.com/app/reset.html` a **Authentication → URL Configuration → Redirect URLs** (coincide 1:1 con el `RESET_REDIRECT` que arma `Login.tsx` desde `window.location.origin`), y cambió el **Site URL** de `http://localhost:3000` a `https://mejorasm.mejoraok.com/app/`. Con esto el cambio de dominio queda **100% cerrado**.
- **Ajuste post-activación (2026-09-03, tarde):** Pablo abrió `mejorasm.mejoraok.com` y vio el **hub estático** ("Subí material") en vez de su app — pensó que algo estaba roto. No lo estaba: el hub vive en `/` y el EDA en `/app/` (diseño del sitio combinado). Pero (a) era confuso y (b) ese hub estático todavía linkeaba a `github.com/.../upload/...`, contra el mandato "sacar GitHub de la vista". Fix: `hub/index.html` pasa a ser una **puerta mínima que redirige a `/app/`** (meta refresh + `location.replace` + botón). El "Subí material" real ya vivía en `/app/#/hub` (subida por el servidor, sin GitHub). Commit `98cb43e`. Verificado en vivo: la raíz sirve el redirect, `/app/` y `/dashboard/` intactos.

## Ajustes post-dominio + limpieza de IG/FB (2026-09-04)

Sesión nueva (misma identidad `mejorasm-03`, cuenta nueva), mandato repetido de Pablo. Arrancó con dos cosas que Pablo flageó al usar el sistema, y siguió con auditoría.

- **`hub/index.html` → redirige a `/app/`** (`98cb43e`). Ver "Dominio propio activado" arriba.
- **Filtro de campañas basura en `ads`** (`65a26c4`). Ver "Cierre de pendientes del plan de publicación" arriba.
- **Los 3 deploy workflows unificados en `deploy-site.yml`** (`332ae7b`). `deploy-eda` / `deploy-hub` / `deploy-dashboard` eran 99% idénticos y ya habían empezado a divergir (los guards `if [ -d ]`, un nombre de step, y hubo que editar `VITE_BASE_PATH` en los 3 al activar el dominio). Uno solo, con la unión de los triggers (`src/**`, `hub/**`, `dashboard/**`, config del build, y el propio workflow). Verificado en vivo: la raíz redirige, `/app/` sirve el build con base path `/app/`, `/dashboard/` intacto, cname + HTTPS OK.
- **`manage-story` / `manage-post` / `mark-manual` — el "post ya no existe" dejaba runs rojos** (`a22bb65`). Pablo limpió IG/FB a mano y después usó "Despublicar" en el Monitor sobre posts ya borrados — Zernio/Meta responde "This post no longer exists..." y el workflow moría con exit 1 (rojo que asusta), aunque el objetivo ya estaba cumplido. `zernio.mjs::unpublishPost` detecta ese caso (`no longer exists` / `was deleted` / `not visible` / `removed by Meta` / `unsupported get request`) y devuelve `{ success: true, alreadyGone: true }`; `manage-story`/`manage-post` lo reportan como resuelto. Aparte, `mark-manual.yml` tenía un `git push` desnudo que fallaba en carrera con otro commit (pasó real, Pablo clickeando varios botones + crons) — ahora el mismo retry-loop `pull --rebase --autostash` que los otros 5 workflows de contenido.
- **Pendiente de Pablo (no nuestro):** en el Monitor pueden quedar filas viejas de IG/FB que Zernio sigue reportando aunque él las borró de las apps (Zernio guarda su propio historial). Se sacan con "Borrar esta pieza" o `mark-manual` desde el Monitor. La corrida de `sync-history` del 2026-09-03 22:33 trajo 10 posts (9 stories recientes + 1 vieja del 2026-08-03).
- **Los 3 deploy workflows unificados en `deploy-site.yml`** (`332ae7b`). `deploy-eda`/`deploy-hub`/`deploy-dashboard` eran 99% idénticos y ya divergían. Uno solo, con la unión de los triggers. Verificado en vivo (raíz redirige, `/app/` con base path `/app/`, `/dashboard/` intacto, cname + HTTPS OK).
- **`deploy-functions.yml`** (`4f31aea`) — el verify chequea los 11 endpoints, no 4, y falla si falta alguno.
- **Patch de deps** (`af5934f`) — `npm audit` reportaba 5 high; `npm audit fix` (sin `--force`) bumpeó `lodash` 4.17.21→4.18.1, `ws` 8.20.0→8.21.3, `browserslist` 4.28.2→4.28.8, `@remix-run/router` 1.23.0→1.23.4 (todos patch, `package.json` sin cambios). Exposición real baja (`ws` no está en el bundle — `WebSocket` nativo; `browserslist` build-time; `lodash` via recharts no explotable; open-redirect de router mitigado por `HashRouter`), pero son patches limpios. Baseline verde. **La caché de npm de la máquina estaba corrupta** (`npm ci` → `ENOENT _cacache`) — se limpió y reinstaló con `npm install`.
- **`harden(static)`** (`21de296`) — framebuster anti-clickjacking en `index.html` (GitHub Pages no puede setear `X-Frame-Options` ni CSP `frame-ancestors`, solo valen como header HTTP); borrado `vercel.json` (config muerta — deploy es GitHub Pages; si el repo se conectara a Vercel, ese archivo auto-deployaba una copia sombra del EDA; sus headers ya están como `<meta>` en `index.html`).
- **`react-router-dom` v6 → v7** (`c4830e5`) — Pablo pidió el bump. `npm audit --omit=dev` (lo que realmente ships) pasa de **2 moderate a 0**: saca `@remix-run/router@1.23.4` (`GHSA-2w69-qvjg-hvjx` XSS vía open redirect + `GHSA-2j2x-hqr9-3h42` protocol-relative `//`). `react-router-dom@7.18.3` sigue publicado como shim que re-exporta de `react-router@7` — **cero cambios de código**: el app usa solo APIs declarativas estables (`HashRouter`/`Routes`/`Route`/`Navigate`/`Link`/`Outlet`/`useLocation`/`useSearchParams`), nada de data-router (`json`/`defer`/loaders). Único cambio de comportamiento que toca el setup: `v7_startTransition` es default — navegar a una ruta `lazy` sin cargar mantiene la pantalla actual en vez de flashear el fallback de `Suspense` (mejora de UX; el `<Suspense>` de `App.tsx` sigue cubriendo el primer load). `react-vendor` 156 → 174 KB (+6 KB gzip). Transitivos nuevos: `cookie@1.1.1` + `set-cookie-parser@2.7.2` (deps de v7, mínimos). Baseline verde (tsc/lint 0 errores/66 tests/build). Confirmado en vivo contra `https://mejorasm.mejoraok.com/app/`: login renderiza, `<HashRouter>` monta limpio, navegación por hash sin error, consola solo con el warning benigno de siempre. **Quedan 2 vulns `vite`/`esbuild`** (`GHSA-67mh-4wv8-2f99` — dev-server) — solo afectan `npm run dev`, no van al build de prod; el fix es `vite@7` (major, plugins + config API), no se hizo blind — riesgo real para exposición nula en un proyecto de un dev.

## Pase de limpieza obsesiva — "nada pendiente" (2026-09-04)

Con el bump de react-router cerrado, Pablo pidió un pase amplio: "limpia, optimisa, aprolija, depura, investiga, mejora y arregla todo... nada pendiente" para el 4/9. La otra sesión (`mejorasm-01`) no estaba corriendo (confirmado con la lista de agentes) — esta sesión tomó el barrido completo, incluida `src/` (normalmente lane de `[01]`), avisando en `COLABORACION.md`.

**Purga de código muerto shadcn** (`c4a099b`) — `knip` (corrido por primera vez en el repo) encontró 30 componentes de `src/components/ui/*` sin ningún importer real: `accordion`, `alert`, `aspect-ratio`, `avatar`, `breadcrumb`, `calendar`, `carousel`, `chart`, `checkbox`, `collapsible`, `command`, `context-menu`, `drawer`, `dropdown-menu`, `hover-card`, `input-otp`, `menubar`, `navigation-menu`, `pagination`, `popover`, `progress`, `radio-group`, `resizable`, `scroll-area`, `separator`, `sidebar`, `slider`, `toggle`, `toggle-group` — más `use-mobile.tsx` y `App.css`. Verificado uno por uno con grep antes de borrar (no alcanza con que la herramienta lo diga): formaban un subgrafo cerrado, solo se referenciaban entre ellos, cero imports desde código de app real. Borrados junto con 24 deps huérfanas (`@radix-ui/*` de esos componentes, `cmdk`, `vaul`, `embla-carousel-react`, `input-otp`, `react-day-picker`, `react-resizable-panels`, `@tailwindcss/typography`) — se mantuvieron `browserslist`/`caniuse-lite` (pin transitivo del audit fix del día anterior). `vite.config.ts::manualChunks.ui-vendor` tenía `@radix-ui/react-dropdown-menu` hardcodeado (rompió el primer build) — corregido a los radix reales en uso. Precedente directo: `form.tsx` + react-hook-form + zod, sacados el 2026-09-03 con el mismo criterio. Re-agregable con `npx shadcn add <x>` si hiciera falta. `package.json` 87 → 61 entradas, `-3876` líneas, `index` bundle -7 KB.

**Lint a 0/0** (mismo commit) — `@typescript-eslint/no-unused-vars` estaba en `"off"` en `eslint.config.js` desde siempre: un import muerto en cualquier archivo no lo cachaba nadie. Se prendió como `"error"` con `argsIgnorePattern`/`varsIgnorePattern: "^_"`. Solo 7 violaciones reales en todo el repo: 5 imports sueltos (`AdsCard.tsx`, `Onboarding.tsx`, `useMetrics.ts`, `src/test/integration.test.tsx`) y `src/hooks/use-toast.ts::actionTypes` (un `const` shadcn que solo se usaba vía `typeof` para el tipo — convertido a un tipo puro, que es la forma correcta). De paso, `react-refresh/only-export-components` (el patrón cva de shadcn de exportar componente + variantes desde el mismo archivo) se apagó con un override scoped a `src/components/ui/**` — no es un descuido, es la convención oficial de la librería. Lint bajó de 6 a **0 warnings**.

**Autocorrección real, documentada sin maquillar** (`6f55455`) — al sacar `mammoth` de `devDependencies` por "sin uso" (según `knip`), resultó que sí lo usa `scripts/load-vault-documents.mjs` (import Node real, `import mammoth from "mammoth"` — no confundir con el `npm:mammoth` que usa `vault-process` en su propio runtime Deno, es un paquete completamente aparte). `knip` no escanea ese script porque no es uno de sus entry points por default. Nunca rompió nada en producción (nada lo corre en CI, es la herramienta documentada para re-sembrar la Bóveda), pero hubiera roto la próxima vez que alguien lo corriera. Restaurado en minutos, con la nota de por qué.

**`npm update` dentro de rangos ya declarados** (`f0221d8`) — `@radix-ui/*` (varios), `@supabase/supabase-js`, `@tanstack/react-query`, `lucide-react` 1.8→1.41 (confirmado legítimo: el paquete real de `lucide-icons/lucide` sí re-numeró a v1.x, no es un supply-chain squat), `playwright`, `eslint`, `autoprefixer`, `@types/*`. `package.json` sin cambios (los `^` ya cubrían el salto). Sin majors (React 19, eslint 10, vite 7, date-fns 4 quedan afuera a propósito — no verificables sin poder click-testear el app logueado).

**Exports muertos reales, sacados tras revisar los ~54 "unused" que marcó `knip`** (`df4730c`) — la inmensa mayoría eran falsos positivos esperables (API de compuestos de shadcn como `DialogPortal`/`SelectGroup`/`SheetFooter`, tipos de retorno de funciones de `ai.ts`, el archivo generado `integrations/supabase/types.ts`) — investigados uno por uno, no descartados en bloque. Dos sí eran reales: `src/pages/Login.tsx` y `src/pages/ResetPassword.tsx` tenían cada uno un `export default` sin ningún consumer — `AuthGate.tsx` los importa por el named export (`import { Login }`/`{ ResetPassword }`), que es el que de verdad se usa. Sacados los dos `export default`.

**Cruft estática, sin código detrás** (`a89d379`) — `public/.htaccess`: reglas de Apache para el subpath viejo `/MejoraSM/` (de antes del dominio propio) — GitHub Pages no corre Apache, nunca se procesó, y el path ya estaba mal de todas formas; la app usa `HashRouter` así que ni siquiera hacía falta el rewrite de SPA-fallback que intentaba armar. `public/placeholder.svg`: leftover del molde de Lovable/shadcn, cero referencias en `src/`/`index.html`. Los dos borrados; `favicon.ico` sí se mantiene (se sirve por convención de navegador en `/favicon.ico`, sin `<link>` explícito necesario).

**Investigado y descartado deliberadamente, sin tocar:** los 62 hallazgos de `deno lint` en las 11 Edge Functions son **el 100% `no-explicit-any`** (`catch (e: any)`, `Record<string, any>` para JSON externo) — cero bugs reales, cero `no-unused-vars`/`no-unreachable`/lógica rota. Purgar 62 `any`s en funciones de producción sin poder deployar-y-probar cada una individualmente es riesgo alto para beneficio cosmético — contradice el criterio de cautela ya documentado varias veces en este archivo. `docs/prototipo-studio-v0.1/` (440 KB, incluye `support.js` que `knip` marca sin uso) se mantiene intacto — es el spec técnico de dimensiones/zona segura citado en dos lugares de este archivo, no cruft. `scripts/cargar-clave-zernio.ps1` — Pablo ya había dicho "dejarla" (2026-08-27) pero mostraba como `??` sin trackear en cada `git status`; se agregó a `.gitignore` (queda local exactamente como Pablo quería, sin el ruido). `AUTO_AGENDA_DIMENSIONES` en `src/shared/constants.ts` — genuinamente sin ningún consumer en `src/` hoy, pero es documentación-como-código de una regla de negocio real (qué dimensiones rota la auto-agenda); se deja, el costo de removerlo es más alto que el de dejarlo. `npm install -g supabase` en `deploy-functions.yml`/`deploy-migrations.yml` — confirmado funcionando real (últimas corridas verdes) antes de considerar "optimizarlo", no se tocó. Cache de npm en `setup-node` — ya estaba (`cache: npm`, sin comillas — un grep con comillas dio falso negativo la primera vez), nada que arreglar.

Verificado en cada uno de los 6 commits de este pase: `tsc` limpio, lint 0/0, 66/66 tests, build limpio, `npm audit --omit=dev` = 0, CI + Deploy Site verdes sobre cada commit real (no solo local). Smoke test final en vivo contra `https://mejorasm.mejoraok.com/app/` (screenshot real) confirmando cero regresión visual tras sacar >3800 líneas de código sin uso.

## Fix real de recorte de fotos + template de collage — 2026-09-04

Pablo reportó dos pedidos en el mismo mensaje: "las capturas de las sesiones con los clientes están mal cortadas" (bug concreto) y "agregar un editor y más plantillas... collages más creativos... tendencia 2026" (feature). Se investigó y resolvió lo automatizable; lo que necesita una decisión de producto quedó planteado, no ejecutado a ciegas.

**Causa real del recorte** (`cf395a3`): `.photo` en `post-template.html` y `story-template.html` usaba `background-size: cover` — llena el lienzo entero recortando desde el centro, sin mirar la relación de aspecto real de la foto. Una foto de estudio (vertical, cerca de 4:5) tolera bien ese recorte; una captura de pantalla (videollamada 16:9, chat angosto, lo que sea) pierde contenido real en los bordes.

**Fix, en los dos templates:** `.photo` pasa a `background-size: contain` (nunca recorta nada) + una capa `.photo-backdrop` detrás con la misma foto ampliada y difuminada (blur 48px, oscurecida) llenando el resto del lienzo — mismo recurso que usan Instagram/Facebook para fotos que no llenan el marco. Con una foto ya en la relación de aspecto exacta del lienzo, el resultado es idéntico a antes; con una que no calza, ya no se pierde nada. Cero cambios en los scripts de render — siguen inyectando el mismo `{{PHOTO_STYLE}}`, ahora aplicado a dos capas vía un selector CSS compartido. Probado real con 3 fotos sintéticas (16:9 ancha, angosta tipo chat, 4:5 normal) renderizadas con Playwright contra los templates reales — confirmado visualmente. El mismo bug en `render-reel.mjs` (mismo patrón, en un filtro de ffmpeg en vez de CSS) se arregló al día siguiente — ver "Fix de recorte en Reels" más abajo.

**Template de collage** (`41fb20f`) — `templates/collage-template.html`: dos paneles de foto lado a lado (en vez de una sola imagen de fondo) con el mismo criterio de recorte seguro en cada uno por separado, panel blanco de texto abajo con la misma tipografía/colores que `post-template.html` (misma familia visual, compuesta distinto). Sin segunda foto disponible, el panel cae al isotipo de marca sobre azul (mismo recurso decorativo ya aprobado) en vez de un hueco vacío. Wireado en `render-scheduled-posts.mjs`: un post simple con 2+ fotos reales sin usar en `content/inbox/<oferta>/` usa el collage automáticamente, sin gate humano nuevo — mismo criterio de autonomía del resto del pipeline. Con 1 sola foto (el caso más común hoy) sigue el post clásico. `findPhotos()` ya existía (la usa el carrusel), se reusa tal cual.

**El pedido de "editor propio dentro del MejoraSM"** quedó planteado como pregunta el 2026-09-04 (¿en Stories, que ya tiene un checkpoint humano, o sumar uno nuevo al pipeline de posts?) — resuelto el mismo día, ver sección siguiente.

## Editor de collage en "Publicar ahora" — 2026-09-04

Pablo respondió al planteo de arriba con un mandato amplio ("autónomo obsesivo... agregando (append) sin quitar nada de lo que está y funciona... suma utilerías"). Se construyó en Stories — el único lugar del sistema que ya tiene un momento de revisión humana antes de publicar — sin tocar el pipeline autónomo de posts.

**`templates/story-collage-template.html`** (nuevo) — mismo criterio que el collage de posts del ítem anterior (dos paneles de foto, `contain` + backdrop difuminado, ninguna foto se recorta nunca), adaptado al lienzo 9:16 de Stories. La tipografía de `story-template.html` ya estaba pensada para panel blanco (kicker rojo, headline azul, subtext tinta) — a diferencia del collage de posts, acá no hizo falta ningún override de color. Probado con Playwright contra el archivo real: 2 fotos de aspecto muy distinto se ven completas, y el fallback de 1 sola foto muestra el isotipo de marca en el panel vacío (mismo recurso ya aprobado).

**Wireado como opción manual, 100% aditiva** (commit único, `fdd6058`):
- `scripts/lib/claude.mjs`: `askAnthropic`/`askClaude` aceptan un `images` (plural) opcional, para mandar 2 fotos en un solo llamado — `image` (singular) sigue intacto para todos los callers existentes (nada más lo necesitaba).
- `scripts/generate-brief.mjs`: env nuevo `PUBLISH_NOW_COLLAGE` (ausente = comportamiento de siempre, sin cambios). Si viene activado y hay 2+ fotos reales en el inbox de esa dimensión, `briefForCollage()` (nueva) arma un solo brief pidiéndole a Claude un copy que hable de las dos fotos como "dos momentos/ángulos de lo mismo" — no dos stories separadas. Si se pidió collage pero solo hay 1 foto, cae solo al camino de una story normal (nunca falla duro).
- `scripts/render-story.mjs`: rama nueva para `brief.mode === "collage"` que usa el template nuevo con las 2 fotos; el resto de los modos (`foto`/`solo-texto`) no se tocó.
- `scripts/publish-now-manifest.mjs`: `manifest.mode` (informativo, para el preview).
- `.github/workflows/publish-now.yml`: input `collage` (boolean, default `false`).
- `src/components/PublishNowCard.tsx`: checkbox "Armar un collage con las 2 fotos más recientes" — solo visible con 2+ fotos reales cargadas; sin tocarlo, el flujo de "Publicar ahora" funciona exactamente igual que antes de este cambio.

Verificado: `tsc` limpio, lint 0/0, 66/66 tests (mismo conteo, sin tests rotos ni nuevos necesarios — no hay test dedicado a `PublishNowCard`), build limpio, `npm audit --omit=dev` = 0. No se disparó un "Publicar ahora" real contra Supabase/Zernio para no arriesgar una publicación real solo para probar (mismo criterio de cautela de siempre) — la lógica de cada script se revisó a mano y la generación de imagen se probó aislada contra el template real con Playwright, con los mismos casos de prueba (foto ancha tipo videollamada + foto angosta tipo chat) que el collage de posts.

## Fix de recorte en Reels — 2026-09-05

El mismo bug del recorte forzado (documentado el día anterior para `post-template.html`/`story-template.html`) también estaba en `render-reel.mjs`, pero ahí vive en un filtro de ffmpeg, no en CSS — `scale=1350:2400:force_original_aspect_ratio=increase,crop=1350:2400` antes del Ken Burns, mismo problema: una captura de pantalla con una relación de aspecto muy distinta a 1350:2400 perdía contenido real en los bordes. Quedó documentado como pendiente sin arreglar porque no había ffmpeg instalado localmente para poder probar un cambio de filtro de video antes de tocar un pipeline que nadie puede ver renderizar en vivo salvo disparándolo de verdad.

**Se instaló ffmpeg local (`choco install ffmpeg`)** específicamente para poder probar esto como corresponde, en vez de tocarlo a ciegas.

**Fix** (`48eec50`) — mismo criterio "contain + backdrop difuminado" que las plantillas HTML, traducido a un filtro de ffmpeg: rama `bgstill` (fondo) escala+recorta+difumina igual que antes (ahí no importa perder bordes, es puro relleno); rama `fgstill` (foto real) escala sin recortar (`force_original_aspect_ratio=decrease`) y se superpone centrada sobre el fondo con `overlay=(W-w)/2:(H-h)/2`. El Ken Burns (`zoompan`) se aplica igual que siempre, ahora sobre ese compuesto en vez de sobre el recorte duro.

**Probado real, no solo por lectura de código:** reproducido el bug con el filtro viejo (una foto panorámica de prueba quedaba con texto cortado en los bordes, confirmado visualmente), confirmado el fix con el filtro nuevo (mismo contenido, ahora completo), y corrida la cadena completa con los parámetros reales del script (9s, 30fps, encode `libx264`/`aac`, overlay transparente generado con Playwright `omitBackground: true` igual que en producción, no un mock) — el MP4 final es válido (`ffprobe`: h264 1080×1920 + aac, duración exacta 9s) y el frame de en medio muestra la foto completa sin recortar, con el overlay de marca compuesto correctamente encima (transparencia preservada). Cierra el único pendiente que había quedado del pase de fixes de recorte.

## Hallazgo real, no un bug de código: los cron de GitHub Actions corren mucho más espaciados que lo que dice su propio `schedule` — 2026-09-05

Investigando por qué `Publish Scheduled Posts` había fallado una vez (timeout real de Zernio, sin consecuencia — ver abajo), apareció algo más grande: **ningún cron de este repo corre con la frecuencia que su propio archivo YAML pide.** Medido con `gh run list` sobre el historial real (no una muestra chica):

| Workflow | `cron:` declarado | Intervalo real observado (últimas ~8-30 corridas) |
|---|---|---|
| `publish-scheduled-posts.yml` | cada 15 min | **104 a 320 minutos** entre corridas (7x a 21x más lento que lo pedido) |
| `inbox-sync-cron.yml` | cada 3 horas | 129 a 329 minutos (hasta ~1.8x) |
| `metrics-collector-cron.yml` | cada 6 horas | 229 a 451 minutos (mayormente cerca o por debajo de lo nominal) |

El patrón es consistente con algo bien documentado por GitHub mismo (no es un bug de este repo): *"the schedule event can be delayed during periods of high loads of GitHub Actions workflow runs... short-interval schedules are not guaranteed"* — en la práctica, para cuentas sin un plan de pago con capacidad dedicada, un cron de `*/15 * * * *` puede terminar disparando en la práctica cada 2-5 horas, sea cual sea el intervalo pedido — GitHub coalesce/posterga los `schedule:` bajo carga, y cuanto más corto el intervalo pedido, peor la desviación relativa (por eso `publish-scheduled-posts` es el más golpeado — pide lo más frecuente).

**Impacto real, sin dramatizar:** el pipeline sigue siendo autónomo y sin gate humano — nada se rompe, nada requiere intervención — pero una pieza que el Crítico aprueba y agenda para "en un rato" puede tardar horas reales en salir, no 15 minutos. Lo mismo aplica a la frescura de la bandeja de conversaciones y de las métricas. La documentación de este archivo (que dice "cada 15 min"/"cada 3h"/"cada 6h" en varios lugares) describe lo que el YAML *pide*, no lo que GitHub *entrega* — dejarlo así sería engañoso si alguien asume que la cadencia real es la nominal.

**El incidente puntual que disparó esta investigación:** `Publish Scheduled Posts` run `33932746394` (2026-09-05 00:22 UTC) falló con `The operation was aborted due to timeout` al publicar la propuesta `1651d621-b4ec-434b-b700-ef6b36cef684` en Zernio. Revisado el código: `markError()` (en `publish-scheduled-posts.mjs`) solo escribe `metadata.last_publish_error`, nunca cambia `proposals.status` — la propuesta queda `scheduled`, así que la corrida siguiente la reintenta sola vía `isStillScheduled()`. Si el primer intento en realidad sí había llegado a publicar en Zernio antes de que el cliente cortara por timeout, el guard de contenido duplicado de Zernio + la reconciliación ya documentada (ver "Campaña de pruebas end-to-end reales" más arriba) evita un duplicado real. **No se pudo confirmar el desenlace final de esa propuesta puntual** — esta sesión no tiene la service-role key para consultar `proposals`/`run_log` directo, y no corresponde pedirla solo para este chequeo. Dado el diseño (reintento seguro + guard de duplicados), no hay indicio de que haya quedado en un estado roto — pero es Pablo quien puede confirmarlo mirando `/monitor` o `/propuestas`.

**Qué NO se hizo, a propósito:** no se armó un disparador externo (un servicio de cron de terceros pegándole a la API de GitHub para forzar la cadencia real) — es el workaround estándar y conocido para este problema, pero implica darle a un servicio externo un token con permiso de disparar workflows sobre este repo: una decisión real de superficie de acceso, no algo para resolver solo. Se deja documentado como opción, no ejecutado.

## "Subir material": la pestaña elegida la pisaba el clasificador de fotos — 2026-09-07

Pablo mandó un video de pantalla: en `/hub`, eligió la pestaña **Sociales**, subió una captura de pantalla, y "Preparar story de Sociales" seguía deshabilitado ("Primero subí una foto de Sociales arriba") pese a que la había subido — de hecho la subió 3 veces al no entender por qué no pasaba nada.

**Causa real, confirmada mirando el repo:** las 3 copias (mismo MD5) habían quedado en `content/inbox/empresarial/`, no en `sociales/`. En `Hub.tsx`, `classify-photo` (el clasificador con visión) sugirió "Empresarial" para esa captura y su respuesta **pisaba `p.dimension`** (`{ ...p, dimension: res.dimension, ... }`) — la elección explícita de la pestaña quedaba sobreescrita en silencio. El único indicio era el `<select>` mostrando "Empresarial (sugerida)", que se lee como una etiqueta, no como un control que te cambió la elección. Y el header decía "la foto se guarda en `content/inbox/{selectedDim}/`" — o sea, mentía, porque el destino real era el `p.dimension` por-foto, no `selectedDim`.

**Fix (`28feaf2`):** la sugerencia del clasificador ya **no pisa** `p.dimension` — la dimensión de la pestaña manda como default. Si el clasificador cree que va en otra, aparece un link explícito debajo del dropdown ("El sistema cree que es de X — usar esa"), un click lo aplica, nunca automático. El texto de ayuda ahora dice claro dónde se guarda cada foto. Las 3 copias mal guardadas: 1 movida a `sociales/` (lo que Pablo quería), las otras 2 (duplicados exactos) sacadas para que `daily-story` no arme 2 stories iguales de una captura mal clasificada. Verificado: `tsc`/lint 0/0/66 tests/build; no se pudo probar el flujo logueado en vivo (login con la contraseña de Pablo), la verificación quedó a nivel de código + baseline.

## Auditoría en vivo del EDA con Claude in Chrome — 2026-09-07

Pablo instaló "Claude in Chrome" → por primera vez se pudo recorrer el EDA **logueado**, de verdad, en vez de solo la pantalla de login (limitación arrastrada meses). Se recorrió pantalla por pantalla (Dashboard, Subir material, Manual de Marca, Mesa de Diálogo, Propuestas + sus 7 pestañas, Calendario, Monitor, Conversaciones, Auditoría, Configuración) mirando consola, red y estado real. El EDA está **sano de fondo** — todo funciona. Hallazgos y fixes:

- **`publish-scheduled-posts` — "5 fallos" que nagueaban al copiloto eran una pieza que Zernio YA tenía** (`9ade568`). El "consejo del día" repetía "arreglá los 5 fallos del pipeline". En `run_log`: 2 corridas (05-sept 18:12 y 19:53) con `error {"error":"This exact content is already scheduled, publishing, or was posted..."}` — el 409 de contenido duplicado de Zernio, pero **sin `existingPostId`**, así que la reconciliación que ya existía (buscar el post por id) nunca disparaba. La propuesta seguía `scheduled` → el cron la reintentaba en cada corrida → mismo 409 → `error` en `run_log`. Fix: `zernio.mjs` detecta "already scheduled/publishing/posted" y devuelve `{ alreadyHandled: true }`; `publish-scheduled-posts.mjs` lo cierra con `markReconciled()` (status=published, `metadata.reconciled_by_zernio_message`), log `skipped` no `error`, sin `exit(1)`; `publish-story.mjs` tampoco lo cuenta como fallo.
- **"Multiple GoTrueClient instances detected" en consola** (`0068678`). Dos `createClient()` — `src/integrations/supabase/client.ts` y `src/services/supabase.ts` — cada uno con su `GoTrueClient` sobre la misma storage key, compitiendo al refrescar el token. Fix: `services/supabase.ts` re-exporta el cliente canónico. Confirmado en vivo: warning **desaparecido**.
- **Manual de Marca: MIME crudo como tipo de archivo** (`be10308`) — mostraba `application/vnd.openxmlformats-officedocument.wordprocessingml.document`. `prettyFileType()` → "Word"/"PDF"/"Texto"/"Markdown". Confirmado en vivo.
- **Monitor: imagen faltante = recuadro gris ambiguo** (`be10308`) — una pieza vieja cuya render se limpió del repo mostraba un skeleton indefinido. `onError` → placeholder "Imagen no disponible — la pieza ya se publicó, el archivo se limpió del repo".
- **Subir material: `.gitkeep` listado como "foto" rota** (`be10308`) — `content/inbox/<dim>/.gitkeep` (agregado 2026-09-03) aparecía en la grilla "Pendientes" como una imagen rota → "no disponible" al lado de las reales. `PhotoGrid` ahora filtra por `IMG_RE`.
...
## Sesión nueva — deps, chequeo proactivo de cuenta y relectura crítica del pipeline (2026-09-08)

Sesión nueva (mandato de Pablo: investigar actualizaciones, mejorar, arreglar todo, revisar de punta a punta al menos dos veces, no parar hasta terminar). El bloqueo externo de Instagram (ver arriba) seguía sin resolver — no se tocó, es tarea de Pablo. Todo lo de acá es autónomo.

**Dependencias:** `npm outdated`/`npm audit` completo. Vulnerabilidad real: la misma de siempre (`vite`/`esbuild`, solo dev-server, ya evaluada y aceptada por sesiones anteriores — se confirma que sigue siendo la decisión correcta). Patches seguros aplicados dentro de rangos ya declarados (`@supabase/supabase-js`, `lucide-react`, `playwright`, `autoprefixer`, `browserslist`, `typescript-eslint`) — `package.json` sin cambios, solo lockfile. Majors disponibles (React 19, Vite 8, TypeScript 7, ESLint 10, Tailwind 4, Vitest 5, recharts 3, etc.) — investigados, ninguno resuelve una vulnerabilidad real ni justifica el riesgo de breaking changes sin poder click-testear el app logueado. Mismo criterio que sesiones anteriores.

**Chequeo proactivo de salud de cuentas Zernio** (`metrics-collector`, `checkAccountsHealth()`) — hasta ahora, una desconexión de cuenta (como la de IG del 2026-09-07) solo se descubría cuando una publicación real fallaba, con horas de demora. Ahora el cron de métricas (cada 6h) también pega contra `GET /v1/accounts` de Zernio y loguea `metadata->>reason = "account-disconnected"` (mismo contrato que ya leen Dashboard/copilot) si detecta `enabled`/`active` en `false` para IG/FB/LinkedIn. Mejor esfuerzo explícito: no hay doc pública de Zernio que confirme que esos son los únicos campos — si el shape no matchea, simplemente no detecta nada (mismo comportamiento de hoy), nunca hace fallar el cron ni genera falsos positivos.

**`node-version: 20` → `24`** en los 14 workflows que lo declaraban — GitHub deprecó Node 20 en los runners y ya forzaba 24 por detrás, generando un warning en cada corrida. Alineado con la versión real que corre (confirmado: la máquina local ya tiene Node 24).

**Relectura crítica fresca del pipeline** (agente dedicado, sin repetir las 6 rondas de auditoría ya hechas por sesiones anteriores — foco en 3 áreas menos cubiertas: migraciones SQL completas, templates HTML de render, consistencia de los 21 workflows). 5 hallazgos reales, todos arreglados:

1. **HEADLINE nunca se truncaba** (el más grave) — a diferencia de SUBTEXT, el hook/headline de posts, carruseles, collages y stories no tenía ningún tope de longitud, ni en el prompt del Creativo ni en el código de render. Un hook verbose podía superponerse con la pastilla de oferta o cortarse en seco en una pieza que se autoagenda y publica sola, sin gate humano. Fix: `truncateHeadline()` (reusa `truncateAtClause`, límite 12 palabras) en `render-scheduled-posts.mjs`; `truncateWords()` nueva en `render-story.mjs` (no tenía ninguna función de truncado). Probado real con Playwright contra `post-template.html`: un hook adversarial de 23 palabras cabe limpio tras el fix, sin tocar la pastilla ni el isotipo.
2. **`deploy-site.yml` no dependía de que `ci.yml` pasara** — corre su propio `vite build` (no tipa) en paralelo, sin `needs`/`workflow_run`. Un commit con un error real de tipo podía fallar en CI y publicarse igual a producción — el mecanismo exacto detrás del incidente `IMG_RE is not defined` del 2026-09-07. Fix: mismos 3 gates de `ci.yml` (tsc -b/lint/test) agregados directo en `deploy-site.yml` antes del build.
3. **`deploy-migrations.yml` conservaba el paso "Link project"** que `deploy-functions.yml` ya había documentado como roto (SchemaError real) — nunca se le aplicó el mismo fix. `db push` acepta `--project-ref` directo, sin necesitar el link previo (confirmado contra la doc oficial de Supabase). De paso se agregó un input `dry_run` permanente. Probado real en producción: dry-run confirmó el fix (nunca llegó al paso roto), y se usó para aplicar la migración 026 de verdad.
4. **CHECK de `proposals.dimension` con solo 4 de las 6 dimensiones reales** (migración 008, nunca actualizada cuando se agregaron `profesionalizacion`/`sociales` el 2026-08-17) — dormido hoy (nada escribe esa columna todavía) pero bomba de tiempo real para cuando se conecte clasificación automática. Migración `026_fix_dimension_check_and_run_log_index.sql`: constraint actualizado a las 6 + índice parcial nuevo para `run_log.metadata->>'reason'` (la query del banner de "IG desconectada" caía a seq-scan). Aplicada real contra producción.
5. **`reel.yml` sin concurrency lock compartido** con `daily-story`/`publish-now`/`publish-scheduled-posts` pese a leer las mismas carpetas de `content/inbox/<dimensión>/` — con el cron de posts corriendo de forma irregular (1-5h por throttling de GitHub, ya documentado), un Reel manual podía solaparse y levantar la misma foto para dos piezas distintas. Fix: los 4 workflows unificados en un solo grupo `content-pipeline`.

**Bump de 6 GitHub Actions** (`checkout`/`setup-node`/`upload-artifact` v4→v7, `configure-pages`/`upload-pages-artifact`/`deploy-pages` v3-5→v5-6) — 3 majors atrás en varias. Breaking changes investigados uno por uno contra el uso real de este repo (ninguno usa `pull_request_target`/`workflow_run` de checkout de forks, ni `packageManager`/`always-auth` de setup-node) — ninguno aplica. De paso saca el warning residual de Node 20 (era el runtime interno de las actions v4, no el `node-version` del job).

Todo verificado real en producción, no solo local: cada commit con `tsc -b`/lint 0/0/66 tests/build limpios, y las corridas reales de CI + Deploy Site + Deploy Migrations confirmadas en verde una por una (no solo asumidas). Sin ningún hallazgo forzado — el agente de relectura crítica reportó explícitamente "sin hallazgos" donde no encontró nada real (RLS, placeholders huérfanos, env vars de workflows).

**Segunda pasada — auto-revisión crítica de estos mismos 9 commits.** Antes de dar el pase por cerrado, se le pidió a un agente independiente que auditara con ojo escéptico el propio trabajo de esta sesión, sin confiar en los commit messages. Encontró 2 problemas reales:

1. **`checkAccountsHealth()` nunca llegaba a ejecutarse en el escenario actual del repo** — vivía después de un `return` temprano de "no hay proposals publicadas" dentro de `collectAllPending()`. Con la base vacía desde el wipe del 2026-09-01 y el pipeline de feed dependiendo de que el autopilot apruebe algo (3x/semana, ~50% de rechazo), ese `return` se ejecutaba casi siempre — el chequeo de salud, pensado justamente para no depender de que hubiera contenido de feed, terminaba dependiendo de eso igual. Movido antes del `return`, corre ahora siempre que haya `ZERNIO_API_KEY`.
2. **`render-reel.mjs` quedó con el mismo bug de HEADLINE sin truncar** que este mismo pase arregló en los otros dos archivos — nunca se tocó. Mismo guardrail agregado (límite 13 para headline, 20 para subtext). Probado real con Playwright: un hook de 23 palabras cabe limpio.
3. **Verificación pendiente que el agente señaló** (no un bug, un supuesto no confirmado): el límite de headline de `render-story.mjs` (14 palabras) nunca se había probado visualmente contra el font-size real de la variante `solo-texto` (104px, el más grande del proyecto) — solo se probó contra `post-template.html` (62-72px). Al probarlo con Playwright, 14 palabras llenaba casi 6 líneas y quedaba muy cerca del footer. Bajado a 11 (el mismo límite que ya pide el prompt de `generate-brief.mjs`, sin margen extra) — probado real en las dos variantes (solo-texto y con foto), cabe con margen amplio en las dos.

CI + Deploy Edge Functions confirmados verdes sobre este segundo commit. Con esto, el pase de hoy queda cerrado — la auto-revisión no encontró más problemas en los otros 5 puntos que auditó (`deploy-site.yml`, `deploy-migrations.yml`, la migración 026, el concurrency unificado, el bump de actions), confirmando cada claim de forma independiente contra la documentación oficial y las corridas reales, no contra los commit messages.
